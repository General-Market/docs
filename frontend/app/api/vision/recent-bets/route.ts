// Mirror of /api/bets/recent under /api/vision/* — placed here because the
// original path returns 404 on Vercel despite the file existing in the deployed
// tree. The vision/* tree builds and ships reliably, so this guarantees the
// route is reachable. The frontend hook reads from this path.
...
// Look back ~30 minutes — covers join + settle for every source.
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const log of settledLogs) {
      ...
      // won → display gross payout. lost → display deposit if known, else payout.
      const displayAmount = won ? payout : deposit > 0n ? deposit : payout
    ...
    console.error('[api/vision/recent-bets]', err?.message || err)
