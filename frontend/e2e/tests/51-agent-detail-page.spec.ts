...
test.describe('Agent Detail Page', () => {
  test('page loads without crash', async ({ page }) => {
    ...
    const response = await page.goto(`/agent/${TEST_ADDRESS}`, { waitUntil: 'domcontentloaded', timeout: 60_000 })
  ...
  test('agent header renders with address', async ({ page }) => {
    test.setTimeout(120_000)
    await page.goto(`/agent/${TEST_ADDRESS}`, { waitUntil: 'domcontentloaded', timeout: 60_000 })
  ...
  test('no 500/404 error on page', async ({ page }) => {
    test.setTimeout(120_000)
