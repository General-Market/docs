/**
 * Oracle process management helpers for resilience E2E tests.
 * Kill, restart, and health-check individual oracle nodes.
 */
...
/** Anvil account private keys for oracles (accounts 1-3). */
const ORACLE_KEYS: Record<number, string> = {
...
/** Read .pids.info and find PID for oracle-{id}. */
export function getOraclePid(id: number): number | null {
  if (!existsSync(PIDS_INFO_PATH)) return null;
  const lines = readFileSync(PIDS_INFO_PATH, 'utf-8').split('\n');
  for (const line of lines) {
    const match = line.match(new RegExp(`^oracle-${id}:(\\d+)$`));
    if (match) return parseInt(match[1], 10);
  }
  return null;
}

/** Update .pids.info with a new PID for oracle-{id}. */
function updatePidsInfo(id: number, newPid: number): void {
  const content = existsSync(PIDS_INFO_PATH)
    ...
    : '';
  // Remove old entry for this oracle
  const lines = content.split('\n').filter(l => !l.startsWith(`oracle-${id}:`));
  lines.push(`oracle-${id}:${newPid}`);
...
/** Find the actual PID of oracle-{id} by checking P2P port (9000+id) with lsof. */
...
/** Kill oracle-{id} with SIGKILL and wait until the process is dead and port is freed. */
export async function killOracle(id: number): Promise<void> {
  // Try PID from .pids.info first, then fall back to finding by port
  let pid = getOraclePid(id);
  ...
  if (isProcessAlive(pid)) throw new Error(`oracle-${id} (PID ${pid}) didn't die after SIGKILL`);
...
/** Restart oracle-{id} by spawning the binary with the same args as start.sh. */
export async function restartOracle(id: number): Promise<void> {
  ...
  if (contracts.BLSCustody) args.push('--oracle-custody-settlement', contracts.BLSCustody);
  ...
  const keyFile = `/tmp/oracle-key-${id}.txt`;
  writeFileSync(keyFile, ORACLE_KEYS[id]);

  const env: Record<string, string> = {
    ...process.env as Record<string, string>,
    ORACLE_PRIVATE_KEY_PATH: keyFile,
    ORACLE_PEERS: peers.join(','),
    ORACLE_SETTLEMENT_RPC_URL: 'http://localhost:8546',
    ORACLE_SETTLEMENT_CHAIN_ID: '421611337',
    ORACLE_BRIDGE_PROXY_ADDRESS: contracts.BridgeProxy || '',
    DATA_NODE_URL: 'http://localhost:8200',
  };

  const logFile = join(LOGS_DIR, `oracle-${id}.log`);
  const logFd = openSync(logFile, 'a');

  const child = spawn(
    join(PROJECT_ROOT, 'target', 'release', 'oracle'),
    args,
    {
      cwd: PROJECT_ROOT,
      env,
...
/** GET oracle health endpoint. Returns null only if process is dead (network error).
 *  Parses both 200 (healthy) and 503 (alive but degraded, e.g. 0 peers) responses. */
export async function getOracleHealth(id: number): Promise<OracleHealthResponse | null> {
  try {
    const res = await fetch(`http://localhost:${10000 + id}/health`, {
      signal: AbortSignal.timeout(3_000),
    });
    // Parse both 200 and 503 — oracle is alive in both cases
    // Only return null on network error (process actually dead)
    const text = await res.text();
    try {
      return JSON.parse(text) as OracleHealthResponse;
    } catch {
      return null;
    }
  } catch {
...
/** Poll until oracle-{id} is healthy and has at least 1 connected peer. */
export async function waitForOracleHealthy(id: number, timeoutMs: number): Promise<void> {
  ...
  throw new Error(`oracle-${id} not healthy after ${timeoutMs}ms`);
}

/** Get consensus.success_total for oracle-{id}, or 0 if unreachable. */
export async function getConsensusTotal(id: number): Promise<number> {
  const health = await getOracleHealth(id);
  return health?.consensus?.success_total ?? 0;
}

/** Wait until ALL oracles have at least 1 successful consensus round (warmup). */
export async function waitForConsensusWarmup(ids: number[], timeoutMs: number): Promise<void> {
  ...
  for (const id of ids) {
    ...
    const total = await getConsensusTotal(id);
    if (total === 0) {
      throw new Error(`oracle-${id} never achieved a successful consensus round within ${timeoutMs}ms`);
    }
  }
}

/** Poll until oracle-{id}'s consensus.success_total >= baselineTotal + minIncrease. */
export async function waitForConsensusProgress(
  ...
  throw new Error(
    `oracle-${id} consensus didn't progress: expected >= ${target}, got ${final} (baseline=${baselineTotal}, minIncrease=${minIncrease})`,
  );
}
