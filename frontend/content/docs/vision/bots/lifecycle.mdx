...
Poll the oracle API for active blocks on a loop. The machine asks the same question every thirty seconds: is there something to bet on? The answer is almost always yes.

```python
import requests
import time

def poll_blocks(api_url: str, poll_interval: int = 30) -> list[dict]:
    while True:
        try:
            resp = requests.get(f"{api_url}/vision/blocks", timeout=10)
            if resp.ok:
                blocks = resp.json().get("blocks", [])
                active = [b for b in blocks if not b.get("paused")]
                if active:
                    return active
...
| `paused` | `bool` | Whether the block is paused |
...
| Empty `blocks` array | No active blocks exist | Sleep and retry |
...
bets = generate_predictions(block["market_count"])
```
...
bitmap = encode_bitmap(bets, block["market_count"])
...
```
keccak256(bitmap_bytes) == bitmapHash passed to joinBlock()
```
...
Call `joinBlock` on the Vision contract to enter the block with your sealed commitment.

```python
def join_block(vision_contract, block_id, deposit, stake, bitmap_hash, account, w3):
...
receipt = join_block(vision, block_id, DEPOSIT, STAKE, bitmap_hash, account, w3)
```
...
| Error | Cause | Fix |
|-------|-------|-----|
| `BlockNotFound` | Invalid block ID | Re-fetch blocks from API |
| `BlockPaused` | Block has been paused | Skip this block |
| `AlreadyJoined` | Already have a position in this block | Skip or use `deposit()` to add more |
...
<Info>
If you submit the bitmap too early, the oracle responds with HTTP 404: "Player not found in block." Retry after a few seconds.
</Info>
...
```python
def submit_bitmap(api_url, player, block_id, bitmap_hex, expected_hash, retries=3):
...
| 400 | expected_hash != on-chain | Hash doesn't match `PlayerPosition.bitmapHash` | Ensure you submitted the same hash in `joinBlock` |
...
```python
def update_bitmap(vision_contract, block_id, new_bitmap_hash, account, w3):
    """Update sealed commitment on-chain."""
    tx = vision_contract.functions.updateBitmap(
        block_id, new_bitmap_hash
    ).build_transaction({
        "from": account.address,
        "gas": 100_000,
        "gasPrice": w3.eth.gas_price,
...
update_bitmap(vision, block_id, new_hash, account, w3)
time.sleep(6)
submit_bitmap(API_URL, account.address, block_id,
...
```solidity
function updateBitmap(uint256 blockId, bytes32 newBitmapHash) external;
```
...
| Error | Cause | Fix |
|-------|-------|-----|
| `NotJoined` | No position in this block | Join first with `joinBlock()` |
...
```python
def claim_rewards(vision_contract, block_id, from_tick, to_tick,
                  new_balance, bls_signature, account, w3):
    tx = vision_contract.functions.claimRewards(
        block_id, from_tick, to_tick, new_balance, bls_signature
    ).build_transaction({
        "from": account.address,
        "gas": 500_000,
        "gasPrice": w3.eth.gas_price,
...
```python
def get_balance_proof(api_url, block_id, player):
    resp = requests.get(
        f"{api_url}/vision/balance/{block_id}/{player}",
        timeout=10,
    )
    if not resp.ok:
...
proof = get_balance_proof(API_URL, block_id, account.address)
# proof = { block_id, player, balance, stake_per_tick, bls_signature }
```
...
| Error | Cause | Fix |
|-------|-------|-----|
| `NotJoined` | No position in this block | Cannot claim without joining |
...
Exit a block entirely. Recover what remains. The balance -- whatever it is -- is the final verdict on your theory about the future.

```python
def withdraw(vision_contract, block_id, final_balance,
             bls_signature, account, w3):
    tx = vision_contract.functions.withdraw(
        block_id, final_balance, bls_signature
    ).build_transaction({
        "from": account.address,
        "gas": 500_000,
        "gasPrice": w3.eth.gas_price,
...
proof = get_balance_proof(API_URL, block_id, account.address)
withdraw(vision, block_id, int(proof["balance"]),
...
- Position is deleted (`delete _positions[blockId][player]`).
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                for block in blocks:
                    block_id = block["id"]
                    if block_id in self.joined_blocks:
                        continue
                    if block.get("paused"):
                        continue

                    # Check if already joined on-chain
                    position = self.get_position(block_id)
                    if position["balance"] > 0:
                        self.joined_blocks.add(block_id)
                        continue

                    # Full join flow
                    bets = self.generate_predictions(block["market_count"])
                    bitmap = encode_bitmap(bets, block["market_count"])
                    bitmap_hash = Web3.keccak(bitmap)

                    self.approve_usdc(DEPOSIT)
                    self.join_block(block_id, DEPOSIT, STAKE, bitmap_hash)
                    time.sleep(6)
                    self.submit_bitmap(block_id, bitmap, bitmap_hash)
                    self.joined_blocks.add(block_id)

                # Periodically claim rewards for all joined blocks
                for block_id in list(self.joined_blocks):
                    try:
                        self.claim_if_profitable(block_id)
                    except Exception as e:
                        print(f"Claim error for block {block_id}: {e}")
...
<Info>
The bot maintains a local `joined_blocks` set to avoid duplicate joins. On restart, it checks on-chain positions via `getPosition()` to rebuild this set. The machine remembers where it has already placed its faith.
</Info>
...
```python
def deposit_more(vision_contract, block_id, amount, account, w3):
    # Approve first
...
| Join | `AlreadyJoined` | Skip block, track in `joined_blocks` |
