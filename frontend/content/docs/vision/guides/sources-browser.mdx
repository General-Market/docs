...
- **Source detail** (`/source/[sourceId]`) -- individual source with live markets, price charts, block info, and leaderboard
...
| **Action buttons** | Markets, Block, Details -- all link to the source detail page |
...
Click any source card (or the Markets/Block/Details buttons) to reach the dedicated source page at `/source/[sourceId]`.
...
A horizontal bar below the hero shows the live state of the active block for this source:
...
| **Players** | Active players in this block |
...
On the right side (sticky on desktop), the block entry panel lets you join the block directly from the source detail page:
...
Below the markets table, a leaderboard shows the top-performing players in this source's block, ranked by cumulative profit.
...
- USGS earthquake magnitude stayed FLAT --> resolved based on block rules
