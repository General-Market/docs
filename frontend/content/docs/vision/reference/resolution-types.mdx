...
Resolution types are stored as raw `uint8` values in the off-chain block config (never stored on-chain). The oracle resolver interprets them at tick resolution time. The Solidity enum in `IVision.sol` defines the first 8 types for reference; types 8-13 are oracle-level extensions.
