...
**Fix:** Verify the block ID is correct by calling `getBlock(blockId)` first. Block IDs are sequential integers starting from 0. If you are using a block ID from the API, confirm it matches the chain you are connected to. A block ID valid on one chain will not exist on another.
...
**Fix:** The block has been temporarily suspended by oracle consensus (via BLS-signed `pause`). Wait for the oracles to unpause the block. You can monitor block status by polling `getBlock(blockId)` and checking the `paused` field. If you believe the pause is in error, contact the block creator or the oracle operators.
...
**Fix:** Each source can only have one block. Check if a block already exists for this source by looking up the sourceId mapping. If you need to update the block configuration, use `updateBlockConfig` on the existing block instead of creating a new one.
...
**Fix:** Ensure you are calling the function from the correct address. For block operations, only the address that created the block can update its markets or metadata. For fee collection, only the `feeCollector` address (set at contract deployment) can call `collectFees`. If you need to act on behalf of a block creator, the creator must submit the transaction themselves.
...
**Reverts when:** The `depositAmount` passed to `joinBlock` is less than the `stakePerTick` value. Every player must deposit at least one tick's worth of stake.
...
**Fix:** Check your current balance by calling `getPosition(blockId, yourAddress)` and reading the `balance` field. Only withdraw up to that amount. If you recently claimed rewards, the balance may have changed.
...
**Fix:** Provide a non-zero amount. The minimum meaningful deposit is the `stakePerTick` value for your block.
...
**Fix:** You can only join a block once per address. If you need to increase your balance, call `deposit(blockId, amount)` instead. If you need to change your bitmap, call `updateBitmap(blockId, newBitmapHash)`. If you want to rejoin after leaving, you must first call `withdraw` to clear your position.
...
**Fix:** Call `joinBlock` first before performing any player operations. Verify you are connected with the same wallet address that joined the block. If you previously withdrew from the block, your position was deleted and you must rejoin with `joinBlock`.
...
**Fix:** Read the player's current position with `getPosition(blockId, playerAddress)` and check the `lastClaimedTick` field. Set `fromTick` to `lastClaimedTick + 1`. If you are using the oracle API, request a balance proof for the tick range starting after your last claimed tick.
...
**Fix:** Set `tickDuration` to a value between 1 second and 2,592,000 seconds (30 days). Common values: 300 (5 minutes), 3600 (1 hour), 86400 (1 day). The tick duration is immutable after block creation, so choose carefully.
