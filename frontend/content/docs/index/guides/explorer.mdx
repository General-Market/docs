...
Real-time consensus health, order throughput, peer-to-peer connectivity, DTF fund metrics, Vision block activity, and chain performance -- all in one place. The dashboard does not interpret. It reports.
...
The prediction market engine. Blocks open. Players enter. Markets resolve. This tab shows the machinery behind it.

```
+-------------------------------+-------------------------------+
|      BLOCK VOLUME             |     BLOCK POOL STATS          |
|                               |                               |
...
|  +--------+ +------+ +-----+ |  [==== TfL ====]        $3.2K|
|                               |  [=== UEFA ===]         $2.8K|
|  45 total blocks             |       horizontal bar chart    |
|  [====== Active 93% ======]  |                               |
|  [= Paused 7% =]             |                               |
+-------------------------------+-------------------------------+
|      BLOCKES BY SOURCE        |     NETWORK ACTIVITY          |
|                               |                               |
|  [======= Ryanair =======] 8 |       ___/---\___/---          |
|  [====== TfL ======]       6 |       area chart               |
...
Active vs paused blocks. Total player count. The ratio tells you how alive the prediction market is.

### Block Pool Stats

Total TVL across all Vision pools. Average players per block. A horizontal bar chart ranks the top 5 data sources by TVL. The money concentrates where the conviction is.

### Blocks by Source

How many blocks each data source runs. Sources with more blocks have more active markets. Popularity is not equally distributed. It never is.
...
|-------|------|-------------|
| `poll_block_ts` | string | Timestamp of this snapshot |
...
| Vision block paused | That specific prediction market is not accepting new entries. Other markets are unaffected. |
