...
Live contract addresses across L3 (Chain ID `111222333`) and Settlement (Chain ID `14601`). Source of truth is `deployments/active-deployment.json`; this page is a convenience copy.

<ContractAddressesRegistry />

<Warning>
Contract addresses are loaded from `deployments/active-deployment.json` and may differ between environments (testnet, staging, production). Verify against the active deployment file before interacting with contracts directly.
</Warning>
...
| Contract | Address | Description |
|----------|---------|-------------|
| **Vision** | `0x8aF92d1D6397c0073f47D1f754983A2Da51C06C8` | Prediction market contract for block management and player operations. |
...
The deployment JSON is the source of truth, updated by deploy scripts after each deployment.
...
To interact with the Index L3 chain from a wallet like MetaMask (~2 min):
...
All contracts use UUPS (Universal Upgradeable Proxy Standard) proxies. The addresses on this page are permanent; only the implementation behind each proxy changes on upgrade.
