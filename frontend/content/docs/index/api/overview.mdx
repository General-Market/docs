---
title: "API Endpoints Index"
description: "Introduction to the General Market API"
---

# API Endpoints Index

JSON over HTTPS. No authentication on read endpoints.

Read access to prices, Dex Traded Fund (DTF — basket of on-chain assets with one NAV) shares, portfolios, backtests, and lending positions.

## Architecture

Every browser request passes through a Next.js proxy. The browser never touches the backend directly. The data node speaks HTTP; the browser demands HTTPS; the proxy resolves the contradiction.

<OverviewArchitecture />

**Proxy layer flow:**

```
            Browser ---- HTTPS ----> /api/itp-price
                                         |
                                     (server-side)
                                         |
                                     HTTP -----> data-node:8200/itp-price
                                     HTTP -----> L3 RPC (fallback)
```

Two proxy patterns:

```
  1. DEDICATED ROUTES          Each has its own logic, caching, validation
     /api/itp-price            Tries data-node, falls back to on-chain RPC
     /api/itp-enrichment       Aggregates data-node + CoinGecko + DeFiLlama
     /api/deployment           Reads deployment JSON from filesystem
     /api/faucet               Directly calls L3 + Settlement RPC (writes)
     /api/explorer/health      Token-authenticated proxy to data-node
     /api/market/history       Proxy to data-node market price history

  2. CATCH-ALL PROXY           Transparent pass-through for any path
     /api/dn/[...path]         Forwards to DATA_NODE:8200/{path}
                               Supports SSE streaming (backtest progress)
                               5-minute timeout for long simulations
```

## Base URL

```text
https://generalmarket.io/api
```

The data node runs on port 8200 behind this proxy. All paths in this reference are relative to the base URL above.

## Response Format

JSON. No wrappers, no envelopes. The data is the response.

```json
{
  "asset": "BTC",
  "price": 97432.15,
  "timestamp": 1708600000
}
```

Errors return an HTTP status code and a JSON body with `error`:

```json
{
  "error": "Asset not found"
}
```

## Authentication

None. Read endpoints are public. The explorer health endpoint uses a server-side token injected by the proxy — invisible to callers.

## Rate Limiting

No strict limits. Abuse will be throttled.

## Complete Endpoint Table

```
  METHOD   PATH                          UPSTREAM          DESCRIPTION
  ------   ----------------------------  ----------------  --------------------------------
  GET      /api/itp-price                Data Node + RPC   ITP NAV with on-chain fallback
  GET      /api/itp-enrichment           DN + CG + DeFi    Holdings, founders, TVL, funding
  GET      /api/deployment               Filesystem        Contract deployment addresses
  GET      /api/market/history           Data Node         Historical market price series
  GET      /api/explorer/health          Data Node (auth)  System health metrics
  POST     /api/faucet                   L3 + Settlement   Testnet USDC mint + gas drip
  GET      /api/dn/{path}                Data Node          Catch-all proxy (supports SSE)
```

Vision-specific routes (see [Vision API Reference](/vision/api/overview)):

```
  METHOD   PATH                          UPSTREAM          DESCRIPTION
  ------   ----------------------------  ----------------  --------------------------------
  GET      /api/vision/blocks           Oracle API        List active prediction blocks
  POST     /api/vision/bitmap            All Oracles       Fan-out bitmap submission
  GET      /api/vision/leaderboard       Oracle API        Player PnL rankings
  GET      /api/vision/snapshot          Data Node         Market price snapshots
  GET      /api/vision/snapshot/meta     Data Node         Source health + asset counts
```

## Endpoint Groups

<CardGroup cols={2}>
  <Card title="Prices" icon="chart-line" href="/index/api/prices">
    Single asset, bulk fetch, cached fast-path. Latest USD prices for every supported ticker.
  </Card>
  <Card title="DTFs" icon="layer-group" href="/index/api/itps">
    NAV, AUM rankings, holdings snapshots, founder and TVL enrichment.
  </Card>
  <Card title="Portfolio" icon="wallet" href="/index/api/portfolio">
    Positions, historical value, trade history — keyed by wallet address.
  </Card>
  <Card title="Simulation" icon="flask" href="/index/api/simulation">
    Backtest engine. Run, stream, sweep, compare. Historical data from 2020.
  </Card>
  <Card title="Morpho" icon="building-columns" href="/index/api/morpho">
    Lending positions, market state, history. Borrow against DTF shares; monitor health factor.
  </Card>
  <Card title="Infrastructure" icon="server" href="/index/api/infrastructure">
    Deployment config, explorer health, testnet faucet, data-node catch-all proxy.
  </Card>
</CardGroup>

## Quick Start

<CodeGroup>
```bash cURL
curl https://generalmarket.io/api/price?asset=BTC
```

```javascript JavaScript
const res = await fetch("https://generalmarket.io/api/price?asset=BTC");
const data = await res.json();
console.log(data.price);
```

```python Python
import requests

resp = requests.get("https://generalmarket.io/api/price?asset=BTC")
data = resp.json()
print(data["price"])
```
</CodeGroup>

```json Response
{
  "asset": "BTC",
  "price": 97432.15,
  "timestamp": 1708600000,
  "source": "binance"
}
```
