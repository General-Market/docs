...
Contract addresses, health checks, testnet faucet, market history, and the catch-all data-node proxy.

| Endpoint | Returns |
|---|---|
| `/deployment` | Contract addresses from a deployment JSON |
| `/explorer/health` | Consensus, peer, and throughput metrics |
| `/market/history` | Historical prices for one source + asset |
| `/faucet` | Mints testnet USDC + optional gas drip |
| `/dn/{...path}` | Catch-all data-node proxy (supports SSE) |
...
Returns the contract addresses for the requested deployment JSON. The frontend reads this at runtime to know which addresses to call.
...
Next: [Get Explorer Health](#get-explorer-health) for runtime consensus metrics.

---

## Get Explorer Health

Returns consensus metrics, peer counts, and order throughput. Token-authenticated; the proxy injects the `EXPLORER_TOKEN` server-side.
...
{
  "network": {
    "poll_block_ts": "2026-03-14T12:00:00Z",
    "quorum_met": true,
...
```json
{
  "snapshots": [
    {
      "poll_block_ts": "2026-03-14T11:55:00Z",
      "quorum_met": true,
      "worst_status": "healthy",
...
```
  poll_block_ts              Timestamp of the health poll
...
Next: [Get Market Price History](#get-market-price-history) for asset-level series.

---

## Get Market Price History

Returns historical USD prices for one source and asset as `{timestamp, value}` points.
...
Next: [Testnet Faucet](#testnet-faucet) to fund a test wallet.

---

## Testnet Faucet

Mints testnet USDC on L3 and optionally drips Sonic native token for settlement gas. Testnet only.
...
Next: [Data Node Proxy](#data-node-proxy-catch-all) for any data-node route without a dedicated wrapper.

---

## Data Node Proxy (Catch-All)

Forwards any path under `/api/dn/` to the data node, query string intact. Supports SSE streaming for backtest progress and other long-running endpoints.
...
Next: [API Endpoints Index](/index/api/overview) for the full route map.
