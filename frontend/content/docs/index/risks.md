...
- **Fills wait on oracles too.** Every order is batched, filled, and settled by oracle consensus. If the network stalls, orders sit. Your money is recoverable on a timer, not by goodwill: cancel any PENDING order yourself at any time, and after an order's deadline plus 24 hours, **anyone** may trigger the refund (`claimExpiredOrder`) — nobody can strand your funds. Details: [What happens to my order?](/docs/index/order-lifecycle) (~5 min).
