---
title: DTF, lending, and bridge risks
navTitle: Risks
description: Oracle-fed NAV, permissionless liquidation, rate spikes to 100% APR, and a bridge that waits on oracles.
order: 10
group: System
mode: explanation
---
...
- **Fills wait on oracles too.** Every order is batched, filled, and settled by oracle consensus. If the network stalls, orders sit. Your money is recoverable on a timer, not by goodwill: cancel any PENDING order yourself at any time, and after an order's deadline plus 24 hours, **anyone** may trigger the refund (`claimExpiredOrder`) — nobody can strand your funds. Details: [How orders fill](/docs/index/order-lifecycle) (~5 min).
...
- **Dust truncation on the way out.** Converting an 18-decimal L3 amount to 6-decimal settlement USDC truncates anything below $0.000001. Bounded, but real. The decimal split: [Settlement and the bridge](/docs/index/settlement-and-bridge) (~5 min).
...
```gmseealso
[{"title": "Earn yield or borrow against DTFs", "href": "/docs/index/lending"}, {"title": "How orders fill", "href": "/docs/index/order-lifecycle"}, {"title": "Settlement and the bridge", "href": "/docs/index/settlement-and-bridge"}]
```
