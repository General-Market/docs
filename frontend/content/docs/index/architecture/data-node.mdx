...
The Data Node is the Rust service that feeds every other component a price.

- **Inputs:** 91 external sources (CoinGecko, Binance, USGS earthquakes, Deutsche Bahn delays, and 87 more) plus 8 internal collectors that poll L3 and Settlement.
- **Storage:** PostgreSQL, 25 migrations, 15+ tables.
- **Outputs:** 50+ REST endpoints, a WebSocket feed for Vision prices, and an SSE stream for backtester results — all on port 8200.
- **Consumers:** frontend, oracle nodes, Vision bots, Social Bot, explorer.

If the Data Node stops, every other service stops with it.
...
Each source runs as an independent Tokio task. `spawn_resilient` restarts a task on panic. All writes pass through one `BlockWriter` channel that coalesces inserts before they reach PostgreSQL.
...
In addition to the 91 external sources, the Data Node runs internal collectors that poll the L3 and Settlement chains, compute NAV, and track oracle health.
...
| **Block Engine** | `block_engine.rs` | Vision block lifecycle management | Continuous |

### Chain Pollers Detail

Both chains are polled in parallel. Results land in an in-memory cache, which the API serves directly. The chain is the source of truth; the cache exists for read latency.
...
Each source has a Rust client, a config JSON, and a `SyncEngine`. They are grouped into 16 categories.
...
25 migrations, 15+ tables, grouped into 5 functional clusters.
...
| `block_tables` | 024 | Vision block state and lifecycle |

---

## REST API

The HTTP layer runs on Axum: 50+ endpoints across six functional groups, with CORS and gzip handled by Tower middleware.
...
| `/market/block-history` | GET | Block-level price history |
...
| `/vision/block/:block_id/history` | GET | Price history for a block |
| `/vision/ws` | GET | WebSocket upgrade for live prices |
| `/blocks/recommended` | GET | Recommended blocks for new users |
| `/blocks/config/:hash` | GET | Block configuration by content hash |
| `/blocks/signed` | GET | Signed block attestations |
...
The Vision WebSocket lets a client subscribe to Vision block IDs and receive prices as each source ticks.

<DataNodeWebSocketFlow />

**Protocol:**
- Client sends `{"subscribe": [42, 77]}` to start receiving prices for block IDs 42 and 77
- Client sends `{"unsubscribe": [42]}` to stop receiving prices for block 42
- Server pushes `{"type": "prices", "blockId": 42, "ts": "...", "markets": [...]}` on each source tick

---

## Simulation Engine

The backtester is a 2,759-line module that runs historical DTF strategies against cached market data.
...
The `SimDataCache` loads into memory at startup and reloads every 24 hours.
...
<Info>
The Data Node uses the same NAV formula as `Index.sol._getCurrentPrice()` and reads `_itpInventory` directly from L3. Both implementations must agree; mismatch is a bug.
</Info>
...
`qty[i]` is the per-share quantity from `_itpInventory[itpId]` on-chain. `price[i]` is the latest validated price from the data pipeline.

The chain poller reads inventory via `getITPState()`, multiplies by prices, caches the result in `ChainCache`. The API serves it. Sub-second latency end to end.

---

## CLI Reference

Six subcommands ship in one binary. In production you only need `serve`.
...
Starts the HTTP server and every collector. This is the only command that runs in production.
...
One binary, one VPS, behind nginx.
