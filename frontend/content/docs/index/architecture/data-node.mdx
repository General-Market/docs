...
Every source is an independent Tokio task. When one panics, `spawn_resilient` restarts it. The source does not know it died. All writes flow through a shared `BlockWriter` channel that coalesces inserts -- because PostgreSQL has limits, even if the world does not.
...
Subscribe to block IDs. Receive prices as they arrive. Unsubscribe when you have had enough.
