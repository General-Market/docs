...
<CardGroup cols={2}>
  <Card title="Oracle Nodes (3x)" icon="server">
    Three Rust nodes that agree every second. Round-robin leaders, BLS multi-party signing, Byzantine fault tolerance. They block orders, sign them, and submit confirmed blocks on-chain. One second per cycle. No exceptions.
  </Card>
  ...
  <Card title="Vision Bot" icon="robot">
    Python. Strategy framework. Automated bets on Vision blocks through the BotRegistry. The machine does not hesitate.
  </Card>
...
<Warning>
  **Oracle-AP isolation (FR13):** Oracle nodes cannot communicate directly with the AP. The AP discovers confirmed blocks exclusively through on-chain `TradeRequest` events. This architectural boundary prevents collusion between consensus participants and the trade executor.
</Warning>
