...
Written in Rust (68K LOC), the oracle nodes are the consensus layer of the protocol. They block orders, coordinate BLS signing across peers, and submit confirmed blocks on-chain. One-second cycles. Round-robin leaders. No meetings.
...
- Propose blocks and coordinate BLS multi-party signing
- Submit confirmed blocks on-chain with aggregated signatures
...
Agreement is not optional. The protocol requires it on every block.
...
The leader builds the proposal, aggregates signatures, and submits the block. If a leader fails, the cycle is lost and the next leader takes over. No negotiation. No second chances.
...
A buyer and a seller of the same asset in the same block cancel each other out. Only the remainder reaches the exchange. This is not clever -- it is obvious. But obvious things must still be built.
...
<Warning>
  Oracle nodes **cannot communicate directly** with the AP. The AP discovers confirmed blocks exclusively through on-chain `TradeRequest` events. This boundary exists because if the signers and the executor could coordinate off-chain, the system would have a single point of collusion.
</Warning>
