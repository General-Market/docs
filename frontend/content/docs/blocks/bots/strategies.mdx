...
<Info>
All strategies here generate a list of `"UP"` / `"DOWN"` bets per market. Pass the result to `encode_bitmap()` from the [Bitmap Encoding](/docs/blocks/bots/bitmap-encoding) reference to produce the packed bytes.
</Info>
