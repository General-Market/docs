...
A Vision bot joins prediction blocks, submits sealed bitmaps, and claims rewards without human intervention. Registration is free, requires no collateral. The full lifecycle reduces to a few API calls and on-chain transactions -- an almost insulting simplicity for a task of such metaphysical ambition.
...
2. **Polls** the oracle API for active blocks.
3. **Generates predictions** -- UP or DOWN for each market in a block.
4. **Encodes** predictions into a packed bitmap and hashes it with keccak256.
5. **Joins** the block on-chain with a USDC deposit and the sealed bitmap hash.
...
8. **Withdraws** remaining balance when exiting a block.

<Info>
Registration is **free**. Calling `registerBot()` costs only gas. No staking, no collateral. You deposit USDC only when joining a block -- the moment you decide your machine's opinions are worth money.
</Info>
