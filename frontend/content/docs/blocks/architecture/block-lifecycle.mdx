...
A block is born from data, consecrated by consensus, inhabited by strangers, and dissolved by mathematics. Nine phases. Three machines that must agree. One contract that trusts none of them. This is the full ceremony, end to end.
...
Every 60 seconds, the `BlockEngine` wakes and builds a recommended config for each source. The work is threshold calibration — computing the price-change boundary that splits outcomes 50/50.
...
**Stage 1 — Median of Past Settlements.** Query the `block_settlements` table for the last N actual price changes per asset. Take the median (`PERCENTILE_CONT(0.5)`). The median is 50/50 by construction: half of future values should exceed it, half should not. This is the primary method, and when data exists, it is remarkably accurate.

**Stage 2 — Last Settlement Change.** If no settlement history exists, use the most recent block's actual price change and apply volatility bands:
...
Maximum 8,192 markets per block (the on-chain bitmap is 1,024 bytes).
...
The previous block — the one that was active during the last heartbeat — now becomes the block to resolve. A new block creation is attempted simultaneously. The system breathes in two directions: creating the next round while settling the last.
...
7. **Parse receipt.** The `BlockCreated` event in the transaction receipt contains the on-chain `blockId` — the block's permanent identity.
...
When the heartbeat fires and a `previous_block_id` is ready for resolution, the oracle runs a deterministic pipeline.
...
These records land in the `block_settlements` table. The next time the `BlockEngine` runs (within 60 seconds), it reads the median of accumulated settlements and recalibrates thresholds for the next block.
...
On crash recovery, the oracle loads all non-settled blocks and all positions with balance > 0 from these tables. The in-memory state is rebuilt from Postgres. The machine forgets nothing that matters.
