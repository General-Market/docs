...
Vision tracks 90+ external data sources across 16 categories — crypto prices, train delays, earthquake magnitudes, GitHub stars, broken ice-cream machines, transit disruption scores, and more. Each source produces a numeric value; each value can back a UP/DOWN/FLAT market.

- **Sources:** 90+, in `data-node/src/market_data/sources/`
- **Categories:** 16 (Finance, Economic, Regulatory, Tech, Entertainment, Geophysical, Transport, Academic, Nature, Space, etc.)
- **Update frequency:** ~1 minute to weekly, per source
- **Adding one:** 6 files must be updated — see [Adding a Data Source](https://github.com/generalmarket/index/blob/main/data-node/ADDING_A_DATA_SOURCE.md) (~30 min)

## The Data Pipeline

Every Vision market is backed by a real-world data source. The Data Node collects, validates, and serves price feeds from 90+ external APIs.

<DataPipelineFull />

## Source Categories at a Glance

Vision sources span 16 categories. The edge tends to live in the less-watched ones — buoy readings, nuclear reactor outputs, transit disruption scores.
...
A source produces a numeric value — temperature, wildfire count, Kp index, anything measurable. Players predict whether the value goes UP, DOWN, or stays FLAT over a tick period. The protocol treats every numeric input the same way.

<SourceToMarketFlow />

The same pattern works for every source. The Data Node returns a number; the oracle compares start price to end price; the resolution type decides UP/DOWN/FLAT.

## Market ID Convention

Every market has a string ID formed from its source prefix and asset name:
...
Each source is a Rust module in the Data Node implementing the `MarketDataSource` trait:
...
Every source has built-in rate limiting. The Data Node uses a sliding-window limiter that automatically backs off as upstream limits are approached.
...
See [Adding a Data Source](https://github.com/generalmarket/index/blob/main/data-node/ADDING_A_DATA_SOURCE.md) (~30 min) for the full guide. The six steps:
