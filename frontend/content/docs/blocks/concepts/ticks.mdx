...
Every tick, the system resolves all markets in a block. It is a small reckoning, repeated endlessly:
...
A tick is a discrete time window -- a unit of waiting. The block's `tickDuration` defines how long each window lasts. Tick numbering starts from `createdAtTick` (the tick number when the block was created). Time is sliced into intervals, and in each interval, reality delivers its verdict.
