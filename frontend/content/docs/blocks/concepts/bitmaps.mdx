...
For example, a block with 3 markets only uses bits 0-2 of byte 0. Bits 3-7 are ignored regardless of their value. Silence is not an opinion.
...
If a block has zero markets (`marketIds` is empty), the bitmap is 0 bytes. This is a valid but degenerate case -- no markets resolve and all players' balances remain unchanged. A game with no questions has no losers, which is also a way of having no winners.
