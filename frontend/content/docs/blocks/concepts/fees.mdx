...
When a player calls `withdraw` to exit a block, the fee is computed on **total profit** over the lifetime of the position:
...
To join a block, the player's `stakePerTick` must be at least `MIN_STAKE_PER_TICK`. Ten cents. The price of admission to a prediction market. The price of having an opinion and backing it with something other than words:
