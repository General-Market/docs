...
Adding a new source = implement one Rust trait, wire it into the data-node, deploy the on-chain block pool, register in the frontend. ~2 hours end-to-end for a simple HTTP API.

**Steps**

- **Step 0** — design the feed (what to track, static vs dynamic discovery, sync interval)
- **Step 1** — implement `MarketDataSource` trait in Rust
- **Step 2** — write the config JSON
- **Step 3** — register in data-node (`mod.rs`, `main.rs`, `api.rs`, optional API key wiring)
- **Step 4** — add to `DeployAllVisionBlocks.s.sol`
- **Step 5** — frontend wiring (`market-categories.ts`, `sources.ts`, logo, grid, modal)
- **Step 6** — compile checks and curl the live API

The mechanics are routine. The hard part is choosing *what* to track and how to split it into markets.
...
Answer three design questions before writing code.
...
The same mistakes recur. Take this list seriously the first time.
...
Copy into your PR description and tick as you go.
