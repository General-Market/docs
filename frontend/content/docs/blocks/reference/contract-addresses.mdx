...
Vision contracts are deployed on the **Index L3** Arbitrum Orbit chain (chain ID `111222333`). Verify every address against the deployment file before you send funds.
...
|----------|---------|-------------|
| **Vision** | `0x4F1BDD073932828bf2822F6dCAD1121Da41ED1Ef` | Main entry point for block management, player operations, bot registry, and fee collection. |
...
| `MIN_STAKE_PER_TICK` | 100,000 (0.1 USDC) | Minimum stake per tick to join a block |
| `BPS_DENOMINATOR` | 10,000 | Basis point denominator |
| `MAX_TICK_DURATION` | 30 days | Maximum allowed tick duration when creating a block |
| `MAX_NAME_LENGTH` | 64 bytes | Maximum block name length |
| `MAX_DESCRIPTION_LENGTH` | 280 bytes | Maximum block description length |
