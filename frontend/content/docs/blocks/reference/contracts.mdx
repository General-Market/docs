...
Vision's on-chain layer is three contracts deployed on Index L3 (chain ID `111222333`). Every state-changing operation on `Vision.sol` is gated by BLS signature verification from oracle consensus. There are no admin overrides — the code does not permit them.
...
```
 ...
 |  +------------------+   +---------------------------------------+  |
 |  | joinBlock()      |   | createBlock()      -- BLS sig check   |  |
 |  | updateBitmap()   |   | createBlockAndJoin() -- BLS sig check |  |
 |  | deposit()        |   | updateBlockConfig() -- BLS sig check  |  |
...
| Contract | LOC | Role |
|---|---|---|
| **Vision.sol** | ~820 | Main prediction market. Block lifecycle, player positions, dual-balance accounting, BLS-gated operations. |
...
The main contract inherits from `ReentrancyGuard` and `BLSVerifier`. It manages the full lifecycle of prediction blocks: creation, player entry, reward distribution, and withdrawal.
...
A block is a continuous prediction market tied to a real-world data source. Each block runs on a tick-based clock — predictions lock before each tick resolves.
...
```
  ...
  During lock: joinBlock(), updateBitmap(), updateBlockConfig() all revert TickLocked().
```

### Deferred Config Promotion

Block configuration updates do not take effect immediately. They are staged via `updateBlockConfig()` and promoted at the next tick boundary.
...
Creates a new block for a data source. Idempotent -- if the `sourceId` already has a block, returns the existing `blockId` without reverting.

```solidity
function createBlock(
    bytes32 sourceId,        // keccak256(source_id_string)
    bytes32 configHash,      // keccak256 of ABI-encoded block config
...
**BLS domain tag:** `"CREATE_BLOCK"`

**BLS message:**
```
keccak256(abi.encode(
    block.chainid, address(this), "CREATE_BLOCK",
...
**BLS domain tag:** `"UPDATE_BLOCK_CONFIG"` (distinct from `CREATE_BLOCK`)
...
Join an existing block with a deposit, stake rate, and prediction bitmap.
...
Change your prediction bitmap mid-block. Enforces lock window.
...
Top up your position balance in an already-joined block.

```solidity
function deposit(uint256 blockId, uint256 amount) external;
```
...
Exit a block entirely. BLS-gated -- oracles sign the final balance.
...
```solidity
function pause(uint256 blockId, bytes calldata blsSignature, uint256 referenceNonce, uint256 signersBitmask) external;
function unpause(uint256 blockId, bytes calldata blsSignature, uint256 referenceNonce, uint256 signersBitmask) external;
```
...
function getBlockIdBySourceId(bytes32 sourceId) external view returns (uint256);
function currentTickId(uint256 blockId) external view returns (uint256);
function getPosition(uint256 blockId, address player) external view returns (PlayerPosition memory);
...
```solidity
struct Block {
    address creator;           // address that created the block
    bytes32 sourceId;          // keccak256(source_id_string)
...
```
  ...
  | Field               | Bytes  | Notes                             |
  +---------------------+--------+-----------------------------------+
  | creator             |   20   | first user who called createBlock |
...
// Verify multiple proofs in block
...
```
  ...
  createBlock()
       |---> BlockCreated(blockId, sourceId, creator, configHash, tickDuration, lockOffset)

  updateBlockConfig()
       |---> BlockConfigUpdated(blockId, nextConfigHash, nextLockOffset)

  _promoteConfigIfNeeded()  [internal, fires lazily]
       |---> BlockConfigPromoted(blockId, oldConfigHash, newConfigHash, atTick)

  pause() / unpause()
       |---> BlockPausedEvent(blockId)
       |---> BlockUnpaused(blockId)

  joinBlock()
       |---> PlayerJoined(blockId, player, stakePerTick, bitmapHash, configHash)

  deposit()
       |---> PlayerDeposited(blockId, player, amount)

  updateBitmap()
       |---> BitmapUpdated(blockId, player, newBitmapHash, configHash)

  claimRewards()
       |---> RewardsClaimed(blockId, player, amount)

  withdraw()
       |---> PlayerWithdrawn(blockId, player, amount)

  forceWithdraw()
       |---> ForceWithdrawn(blockId, player, amount)
...
| Error | Trigger |
|---|---|
| `BlockNotFound()` | Block does not exist or config hash mismatch |
| `BlockPaused()` | Block is paused by oracle consensus |
| `BlockAlreadyExists()` | sourceId already has a block |
...
| `AlreadyJoined()` | Player already has a position in this block |
| `NotJoined()` | Player has no position in this block |
...
<Info>
For detailed error resolutions and fix suggestions, see the [Error Codes](/docs/blocks/reference/error-codes) reference.
</Info>
...
```
  ...
  +-----------------------------+----------+-------------------------------------+
  | createBlock()               | YES      | Create block for a sourceId         |
  | createBlockAndJoin()        | YES      | Atomic create + join                |
  | updateBlockConfig()         | YES      | Stage config update for next tick   |
  | joinBlock()                 | no       | Join a block with deposit + bitmap  |
  | updateBitmap()              | no       | Change prediction bitmap            |
  | deposit()                   | no       | Top up position balance             |
  | claimRewards()              | YES      | Claim winnings for tick range       |
  | withdraw()                  | YES      | Exit block, receive final balance   |
  | forceWithdraw()             | YES      | Emergency exit by oracle consensus  |
  | pause()                     | YES      | Pause a block                       |
  | unpause()                   | YES      | Unpause a block                     |
...
```
  ...
  | Function                    | Returns                                    |
  +-----------------------------+--------------------------------------------+
  | getBlock(blockId)           | Block struct                               |
  | getBlockIdBySourceId(srcId) | uint256 blockId                            |
  | currentTickId(blockId)      | uint256 current tick (with underflow guard)|
  | getPosition(blockId, player)| PlayerPosition struct                      |
...
```
  ...
  |                        | createBlockAndJoin()      |
  | "UPDATE_BLOCK_CONFIG"  | updateBlockConfig()       |
  | "CLAIM"                | claimRewards()            |
  | "WITHDRAW"             | withdraw()                |
...
```
  ...
  2. Join a block
     joinBlock(blockId, ...) ----------------------------------->  _debitBalance(user, deposit)
                                                                   create PlayerPosition
                                                                   emit PlayerJoined

  3. Update predictions
     updateBitmap(blockId, ...) -------------------------------->  update bitmapHash
                                                                   emit BitmapUpdated

  4. Claim rewards (per tick range)
                              <--- oracles compute new balance
     claimRewards(blockId,         and sign BLS proof
  ...
  5. Withdraw
     ...
     withdraw(blockId,             and sign BLS proof
