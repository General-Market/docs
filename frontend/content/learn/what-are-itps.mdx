...
keywords: ["ITP", "index tracking product", "crypto ETF", "on-chain ETF", "DeFi index fund", "tokenized index", "ERC-4626 vault", "crypto index fund"]
date: "2026-03-17"
...
tldr:
  - "ITPs are <strong>on-chain ETFs</strong> — one token holds a basket of up to 100 crypto assets with fixed weights"
  - "Settlement in <strong>~30 seconds</strong> on Arbitrum — no T+1 waiting, no custodian delays"
  - "Management fees up to <strong>10% annualized</strong>, 70/30 split between creator and protocol — <strong>recurring revenue per ITP deployed</strong>"
  - "107 backtested ITPs: <strong>median 33.5% annualized</strong>, 75% positive. Best Sharpe: <strong>1.355</strong> (RWA TVL Weighted)"
---
...
Anyone can create an ITP. Pick assets from 100+ supported tokens, assign weights (e.g. 30% ETH, 20% BTC, 50% stablecoins), deploy in a single transaction. The ITP starts at $1 NAV. Each share holds a fixed quantity of each underlying asset, calculated from the weights and prices at the moment of creation.
...
2. **Rebalancing.** Maintaining 20% ETH / 20% BTC requires manual adjustment when prices shift. The math is not hard. Doing it every week is. An ITP handles this automatically. Discipline outsourced to a contract.
...
5. **Composability.** An ITP is an ERC-20 token. It can be transferred, used as collateral, integrated into other protocols. Ten separate token positions are ten separate management problems. One ITP is one token that plays well with everything else on-chain.
...
**The ETF parallel.** Vanguard did not invent investing. It made index funds permissionless — anyone could access diversified exposure at minimal cost, and the sheer volume of "anyone" made Vanguard enormous. ITPs do not invent crypto baskets. They make them permissionless. No SEC filing. No authorized participant agreements. No $100M minimum viable AUM. The creation cost is one transaction.

This means the long tail can exist. Niche baskets, thesis-driven allocations, community portfolios, meme-sector indexes — products that would never survive the overhead of traditional fund creation can exist on-chain because the overhead is approximately zero. The question is not whether someone will create an "AI Infrastructure" ITP or a "Latin American DeFi" ITP. The question is how many will exist before the year ends.

Every platform that hosts user-generated content discovered the same truth: the long tail generates more aggregate value than the head. The ETF industry's head is SPY and QQQ. The ITP ecosystem's value is in the ten thousand baskets that would never exist if creation required a legal team.
...
Four steps between you and a diversified crypto position. Five if you count the moment of decision, which is always the hardest.

**Step 1: Get USDC on Arbitrum.** Bridge from Ethereum using the [Arbitrum Bridge](https://bridge.arbitrum.io/), or buy USDC on an exchange and withdraw directly to Arbitrum. If you already have USDC on Arbitrum, skip this step. The protocol speaks USDC. Nothing else.

**Step 2: Connect your wallet.** Go to [generalmarket.io](https://generalmarket.io). Connect MetaMask, WalletConnect, or any EVM-compatible wallet. No account creation. No KYC. No email. The wallet is the identity.

**Step 3: Browse ITPs.** Each ITP displays its holdings, weights, current NAV, total AUM, and creator address. Read them like you would read a portfolio — because that is what they are. Someone expressed a thesis in token weights. You are deciding whether to share it.

**Step 4: Buy.** Enter the USDC amount. Confirm the transaction. Receive ITP shares in your wallet. Settlement completes in one cycle — roughly 30 seconds. Your shares appear. The NAV begins to float. You are now holding a basket of crypto assets in a single token.

**Or: Create your own.** Pick tokens from 100+ supported assets. Set weights. Deploy in one transaction. Your ITP launches at $1.00 NAV. Your thesis, tokenized. If others buy in, you have built something between a portfolio and a small declaration of belief. If nobody buys in, you still hold your own conviction at the price you intended. Either way, the thing exists on-chain, verifiable, with your address on it.
