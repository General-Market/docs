...
function FlowDiagram({ steps, caption }: { steps: Step[]; caption?: string }) {
  return (
    <div className="my-10 -mx-4 md:-mx-8">
      <div className="bg-surface border-t-[3px] border-b border-black border-b-border-light px-6 md:px-10 py-8">
        <div className="flex items-center justify-center gap-0 overflow-x-auto py-4">
          {steps.map((step, i) => (
            <div key={i} className="flex items-center gap-0 shrink-0">
              <motion.div
                className="group relative bg-white border border-border-light hover:border-black hover:bg-black transition-colors duration-200 px-6 py-4 min-w-[130px] text-center cursor-default"
              ...
              >
                ...
                {step.sub && <p className="text-micro text-text-muted group-hover:text-text-muted transition-colors mt-1">{step.sub}</p>}
                <span className="absolute -top-2.5 left-3 text-[9px] font-mono text-text-muted bg-surface px-1 group-hover:text-text-muted">{String(i + 1).padStart(2, '0')}</span>
              </motion.div>
              {i < steps.length - 1 && (
                <motion.div className="flex items-center px-1.5" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.1 + 0.2 }}>
                  <div className="w-6 h-[1px] bg-border-medium" />
                  <div className="w-0 h-0 border-t-[3px] border-t-transparent border-b-[3px] border-b-transparent border-l-[5px] border-l-border-medium" />
                </motion.div>
              )}
            </div>
          ))}
