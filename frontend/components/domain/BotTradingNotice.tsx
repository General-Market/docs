...
export function BotTradingNotice({ className = '', dismissible = false }: BotTradingNoticeProps) {
  ...
  return (
    <div className={`animate-fade-in bg-muted border border-border-light rounded-md p-4 ${className}`}>
      <div className="flex items-start gap-3">
        {/* Info icon */}
        <svg
          className="w-6 h-6 text-text-primary flex-shrink-0 mt-0.5"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
          aria-hidden="true"
        ...
        <div className="flex-1 min-w-0">
          ...
          <a
            ...
            className="inline-flex items-center gap-1 text-sm text-brand hover:text-brand-dark mt-2 transition-colors"
          >
        ...
        {dismissible && (
          <button
            ...
            className="press text-text-muted hover:text-text-primary transition-colors p-1"
