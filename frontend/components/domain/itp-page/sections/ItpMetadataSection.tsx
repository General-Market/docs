    ...
    <section className="py-8">
      ...
      {isUnofficial && (
        <div className="flex items-center gap-3 mb-6">
          <span className="inline-flex items-center text-[11px] font-semibold uppercase tracking-[0.06em] text-[#7c6f3e] bg-[#f5f0e0] px-2 py-1 rounded">
            {t('community_index')}
          </span>
          <span className="text-sm text-text-secondary">
            {t('creator')}:
            <span className="font-mono text-text-primary ml-1.5">{truncateAddress(creator)}</span>
          </span>
        </div>
      )}
      ...
      {metadata?.description && (
        <div className="mb-6">
          <h2 className="text-xl font-bold text-text-primary mb-3">{t('description_title')}</h2>
          <p className="text-sm text-text-secondary leading-relaxed">{metadata.description}</p>
        </div>
      )}
      ...
      {metadata?.websiteUrl && (
        <div className="mb-6">
          <a
            ...
            className="inline-flex items-center gap-1.5 text-sm font-semibold text-brand-dark hover:text-brand transition-colors"
          >
            {t('website')}
      ...
      {embedUrl && (
        <div className="mb-2">
          <h3 className="text-sm font-semibold text-text-secondary uppercase tracking-[0.06em] mb-3">{t('video')}</h3>
