...
export function VaultStats() {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 py-5 border-b border-border-light stagger" data-fade-in>
      {stats.map((stat, idx) => (
        <div
          key={stat.label}
          className={`py-3 px-4 md:px-6 animate-fade-up ${idx > 0 ? 'md:border-l border-border-light' : 'md:pl-0'} ${idx >= 2 ? 'border-t md:border-t-0 border-border-light' : ''}`}
        >
