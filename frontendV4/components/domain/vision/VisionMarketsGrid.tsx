...
function CryptoLogo({ assetId, symbol, size = 16 }: { assetId: string; symbol: string; size?: number }) {
  ...
  if (hasError) {
    return (
      <div
        className="rounded-full bg-muted flex items-center justify-center text-text-muted font-bold"
...
function SourceCard({ source, assetCount }: { source: SourceSchedule; assetCount: number }) {
  ...
  return (
    <div className="border border-border-light bg-white p-3 min-w-[180px] flex-shrink-0">
      ...
      <div className="space-y-1 font-mono text-xs text-text-muted">
        <div className="flex justify-between">
          <span>Assets</span>
          <span className="text-text-secondary">{assetCount.toLocaleString()}</span>
        </div>
        <div className="flex justify-between">
          <span>Last sync</span>
  ...
  return (
    <div
      className={`p-2 border cursor-default relative group ${
        isUp
          ? 'border-green-200 bg-surface-up'
          : isDown
            ? 'border-red-200 bg-surface-down'
    ...
    >
      ...
      {changePct !== null && (
        <div className={`font-mono text-[10px] ${isUp ? 'text-color-up' : 'text-color-down'}`}>
...
function SectionHeader({ source, count }: { source: SourceSchedule; count: number }) {
  const displayName = SOURCE_DISPLAY_OVERRIDES[source.sourceId] || source.displayName
  return (
    <div className="flex items-center gap-3 px-3 py-2 bg-muted border-b border-border-light sticky top-0 z-10">
      <div className={`w-2.5 h-2.5 rounded-full ${STATUS_COLORS[source.status] || 'bg-zinc-300'}`} />
      <span className="font-mono text-sm font-bold text-text-primary">{displayName}</span>
      <span className="font-mono text-xs text-text-muted">{count.toLocaleString()} assets</span>
...
function SubSectionHeader({ label, count }: { label: string; count: number }) {
  return (
    <div className="flex items-center gap-2 px-4 py-1.5 bg-card border-b border-border-light">
      <span className="font-mono text-xs text-text-muted">{label}</span>
      <span className="font-mono text-[10px] text-text-muted/50">{count.toLocaleString()}</span>
    </div>
  )
}
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="mb-3 flex-shrink-0">
        <div className="flex items-baseline gap-4">
          <h2 className="text-2xl font-bold text-text-primary font-mono">Market Data</h2>
          {totalAssets > 0 && (
      ...
      <div className="flex items-center gap-3 mb-3 flex-shrink-0">
        <input
          ...
          className="bg-muted border border-border-medium text-text-primary font-mono text-sm px-3 py-1.5 rounded-lg focus:outline-none focus:border-border-dark w-64"
        />
          ...
          <div className="flex flex-col items-center justify-center h-full gap-6 py-20">
            <div className="text-center font-mono">
              ...
              <button
                ...
                className="px-6 py-2 bg-brand/10 border border-brand/30 text-brand font-mono text-sm hover:bg-brand/20 transition-all rounded-lg"
              >
                Retry
              </button>
            </div>
