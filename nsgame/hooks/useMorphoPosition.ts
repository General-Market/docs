...
export function useMorphoPosition(market?: MorphoMarketEntry): UseMorphoPositionReturn {
  ...
  if (collateralAmount !== undefined && debtAmount !== undefined && oraclePrice !== undefined) {
    ...
    // maxBorrow / maxWithdraw are per-market and require the market's own oracle price
    // at Morpho's 1e36 scale. Trust the REST backend (computes with the correct per-market
    // oracle); refetchPosition() is called after borrow/repay for immediate freshness.
    const maxBorrow = safeBigInt(restData?.max_borrow)
    const maxWithdraw = debtAmount === 0n ? collateralAmount : safeBigInt(restData?.max_withdraw)
