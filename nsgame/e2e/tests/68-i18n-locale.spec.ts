...
test.describe('i18n Locale Routing', () => {
  test('default locale (en) loads without prefix', async ({ page }) => {
    ...
    const contentVisible = await page
      .locator('h1, h2, [data-testid="source-card"]')
      .first()
      .isVisible({ timeout: 30_000 })
      .catch(() => false)
    if (!contentVisible) {
      console.log('Homepage content not visible after 30s — page may still be hydrating')
      return
    }
  ...
  test('explicit /ko/ locale loads without crash', async ({ page }) => {
    test.setTimeout(120_000)
    const response = await page.goto('/ko/', { waitUntil: 'domcontentloaded', timeout: 90_000 })
    expect(response?.status()).toBeLessThan(500)
    ...
    const contentVisible = await page
      ...
      .catch(() => false)
    if (!contentVisible) {
      console.log('/ko/ page content not visible after 30s')
      return
    }

    // Body should have non-trivial content (not a blank crash page)
    const bodyText = await page.locator('main, body').first().textContent({ timeout: 15_000 }).catch(() => '') ?? ''
    expect(bodyText.length).toBeGreaterThan(50)
  })

  test('explicit /ja/ locale loads without crash', async ({ page }) => {
    test.setTimeout(120_000)
    const response = await page.goto('/ja/', { waitUntil: 'domcontentloaded', timeout: 90_000 })
    expect(response?.status()).toBeLessThan(500)
    ...
    const contentVisible = await page
      ...
      .catch(() => false)
    if (!contentVisible) {
      console.log('/ja/ page content not visible after 30s')
      return
    }
  ...
  test('invalid locale falls back gracefully', async ({ page }) => {
    ...
    const response = await page.goto('/xx/', { waitUntil: 'domcontentloaded', timeout: 90_000 })
    ...
    const hasContent = await page.locator('body').textContent({ timeout: 15_000 }).catch(() => '') ?? ''
