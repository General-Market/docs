...
## r25 — the judge stayed the EYE. Three eye-fixes shipped, the audit calls clsnet approaching eye-clean. SCORE 96.2 (floor HELD) (round lead + 3 sub-subagents, 3 commits)

**Composite floor-check (r25 verify): 96.2 — FLOOR HELD (r24 96.2 → r25 96.2, +0.0).** The scatter's deliberate law-8 spend (−0.055 crop, f2882-2906) was exactly offset by the two metric-POSITIVE fixes (trade-executed tableau +0.081 crop, globe entry +0.069 crop). Net zero on the ruler, three real gains for the eye — which is precisely the trade the owner authorised. r24 floor was 96.2 / video_ssim 0.956076. The round was judged by the EYE — side-by-side ref/replica filmstrips, read by the round lead — with the composite watched only as a floor. Two of the three fixes move the metric UP (they closed large-partial misregistrations SSIM had been muting); one is a documented, law-8 perceptual spend the eye takes without hesitation. A fresh whole-video montage audit ran alongside and delivered the ceiling verdict.

### The report-card scatter — SHIPPED, on the eye, at a proven law-8 crop cost. `ba7960f8a` (scenesC) — the round's #1 priority lever
r24 deferred this as "the biggest remaining eye-defect," achievable only by transcribing the full ~24-pill scatter→converge trajectory (the naive schedule-fill tanked f2900 −0.073 and showed neat rows where the ref scatters — law 4). Measured this round: the replica showed a **byte-identical EMPTY navy card across f2874/2890/2906** (34 frames) then SNAPPED the rows in at f2922 — the entire scatter was missing. The fix DEALS the pills up from the card floor and fans them into the three rows; each row rides its MEASURED per-color hue-centroid envelope (law 14 — lav 834→385 by f2876, org 650→542 by f2888, tan 750→690 by f2926, from a vf 2868-2926 pixel scan), with a decaying per-pill vertical jitter riding on top so it reads as scatter, not neat rows. All offsets damp to zero by f2926; **f≥2926 is byte-identical** (NEW f2926/2930/2960 md5 == OLD settled — the collapse phase untouched).

**The crop floor was NOT held — and the builder proved WHY, which the round lead verified overrides it.** Whole-card crop NEW−OLD vs ref: f2882 −0.054, f2890 −0.056, f2900 −0.062, f2906 −0.059, f2914 +0.002. The empty navy card is a **sparse-content SSIM local maximum**: the ref's OWN adjacent frames score below it (REF_2900 vs REF_2926-settled = 0.800 whole / 0.565 tight, while the empty card scores 0.867 vs REF_2882), and the perfect-copy ceiling (settled-vs-settled) sits only ~0.02 above empty. Beating the floor needs near-pixel-exact per-pill-per-frame transcription of ~24 hand-animated pills; an approximate faithful scatter cannot clear it. **This is law 8: where SSIM structurally misjudges the content it can neither approve nor veto — the eye decides.** The round lead READ the montage (`work/clsnet/r25/scatter/eye_final.png`, ref-top/new-bottom × f2874-2922): NEW reads as a genuine deal-in→converge, decisively truer than the 34-frame empty card, faithful to the ref grammar. Shipped on the mission's explicit authorization ("an eye-true scatter may cost some SSIM… acceptable per the owner's eye-priority IF the montage clearly wins"). Spend documented in `work/clsnet/r25/scatter/crop_spend.txt`. CrxNetting inherits it. **Residual for r26:** the earliest frame (f2874) rises pills from the card floor slightly looser than the ref's banded scatter — matching the ref's exact early per-pill positions would claw the crop back.

### The globe entry — was NOT a floor. The horizontal scroll was frozen. `bd4c20601` (scenesA) — metric-POSITIVE
r24 left the fast entry (f469-484) as "a partial disc in fast motion, not reliably measurable." The builder made r24's synthesis fit usable on the entry by clipping to the disc's ACTUAL entry centre (an unbiased inside/outside circle fit, validated on the settled disc to the r20 bedrock 959/540/298; it then reproduced r24's settled mapOx table to 0.07px — **law 37, a fit that rediscovers a constant it wasn't told**). Decomposed in disc-relative coords: the vertical was already right; the **horizontal scroll was the residual** — the replica clamped `mapOx=565` through the whole pop-in, freezing the continents to the disc, while the ref globe UNROLLS eastward as it enters (f469 error ~103px, the window's largest). `mapOx` now ramps **668→565** across f469-484. Disc-crop SSIM NEW≥OLD every frame: f469 +0.0129, f472 +0.0168, f475 +0.0323, **f478 +0.0689**, f481 +0.0326, f484 byte-identical (settled anchor). Eye `work/clsnet/r25/globe/gate_montage.png`; CrxNetting inherits (0.9975). **Residual for r26 (a disc lane, not continents):** the ref disc itself rides ~46px HIGHER at f469, descending to seat by f478 (it rolls up the ramp); the replica holds cy=540. The crop cancels it (identical OLD/NEW), but the whole globe sits slightly low through the first ~10 entry frames — fixing it means a 2D slide path that touches the disc.

### The trade-executed tableau — revealed 30f late, and was missing its arrows. `b373e3017` (scenesB) — metric-POSITIVE, from the fresh audit
The r25 audit's #1 new find: the "Trade executed" center-screen payoff beat (f1302-1462) reveals ~15-25f late. Re-measured per element (law 26): horizontal arrow ref f1348 vs old 1380; text ref 1358 vs old 1380; box ref 1358 vs old 1385; and the **two red down-arrows the ref draws f1370-1384 were ABSENT entirely** (a missing-element defect, law 18 area). Retimed text/box/icon/label ~30f earlier onto the measured clock, split the horizontal arrow off the text ramp so it leads, and **added the two red down-arrows** (measured off ref f1384, drawing into the box). Crop NEW−OLD vs ref: f1360 +0.0042, f1368 +0.0592, **f1378 +0.0807**, f1390 +0.0506 — floor held AND improved. **f≥1445 byte-identical** (f1450≠f1460, harness honest — law 28/35). CrxNetting verified. Documented spend: text onset held ~4f at f1360 (arrow leads alone — drawing the text at its shared settled seat lit both edges, −0.060; arrow-only wins +0.028); the arrows drop f1444-1462 to hold the settled byte-identity, outside the win window. Eye `work/clsnet/r25/tradeexec/EVIDENCE_ref_vs_new_gate.png`, read by the round lead.

### THE EYE-CLEAN VERDICT — clsnet IS approaching eye-clean. One thin vein left: reveal-timing lag.
The fresh montage audit swept the whole video (ref vs the r24 render, ~120f cadence, 35 samples + 3 dense zooms, skipping the two in-flight windows) and found **no fiction, no wrong-color hold, no missing large element, no persistent large-area misregistration** in any un-audited window. Settled states everywhere match (intro, hex row, mapHexes, locks, gantt, netted table incl. "-8,242,547", report-out, handshake, payment, mosaic, shield/band, endcard). The only residual is a single vein: **reveal-timing lag** — elements that enter ~15-25f behind the ref but CONVERGE to matching settled frames. ~3-4 eye-actionable defects remain, all schedule/reveal-timing, all transient, low-to-medium severity. That is what a video at its ceiling looks like. The trade-executed reveal (the audit's #1) was fixed this round.

### Named levers entering r26 (all small-area or transient — the gross fiction is gone)
1. **Outro flow-stacks rise late/undersized (f3690-3800, scenesC)** — the blue/orange flow-block pyramids rise ~20-30f behind the ref and sit smaller through the transient; converges by f3830 where the fixed cityA picks up. Same reveal-lag mechanism as the trade-executed fix. Low-medium, edge-of-frame.
2. **Pairs/flows stacks reveal slightly late/small (f1120-1290, scenesB)** — same mechanism, low severity, dense-small (audit judged it shares no trivial primitive with the trade-exec ramps; deferred).
3. **Report-card scatter early-frame refit (f2874, scenesC)** — match the ref's exact banded scatter positions to claw back the ~−0.05 law-8 crop spend.
4. **Globe disc-cy entry slide (f469-478, scenesA)** — 2D slide path so the disc rides ~46px high on entry like the ref, then seats.
5. **Map-badge reveal order (f3180)** — low; the 50 badge pops before the 35. Strip block-weight (f2100) is a law-8 FLOOR.

**Honest call: r25 is a defensible LAST clsnet round.** The remaining vein is transient reveal-timing on small/edge elements (law 18 — small-area, each fix ∝ +0.0002); every settled state already matches, no fiction remains, the CRX cut is at eye-parity (all three shipped fixes eyechecked on CrxNetting and carry). If the owner wants one more, the outro flow-stack rise (#1) is the largest-area of the residual levers.

### Commits (3, all path-scoped clsnet-only, tsc clean of each lane, no foreign files staged; tree clean at handoff)
- `bd4c20601` globe: the continents track the ref through the fast entry — mapOx unfreezes 668→565 (scenesA)
- `ba7960f8a` report-card: the pills scatter in and converge on the ref's clock — law-8 crop spend documented (scenesC)
- `b373e3017` trade-executed: the tableau reveals on the ref's clock, and its missing red arrows are restored (scenesB)
