#!/bin/bash
# Render ClsNet-Replicate stills to work/clsnet/r15/<tag>_<frame>.png through the ONE global lock.
# Usage: still.sh <tag> <frame> [frame...]
cd /Users/maxguillabert/Downloads/index/video
W=.claude/rounds/work/clsnet/r15
TAG=$1; shift
PUB="$PWD/.claude/rounds/pubdir/clsnet"
tries=0
while ! mkdir /tmp/replica-render.lock 2>/dev/null; do
  # self-heal a stale lock ONLY if nothing is actually rendering
  ...
  tries=$((tries+1)); [ $tries -gt 180 ] && { echo "LOCK TIMEOUT"; exit 1; }
  sleep 20
done
...
for FR in "$@"; do
  npx remotion still src/index-replicas.ts ClsNet-Replicate "$W/${TAG}_${FR}.png" \
    --frame=$FR --public-dir="$PUB" --concurrency=1 --log=error 2>&1 | tail -2
  echo "rendered $W/${TAG}_${FR}.png"
done
# reap headless chrome BETWEEN stills (lock released, no live render now)
