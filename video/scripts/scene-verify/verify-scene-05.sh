#!/bin/bash
# verify-scene-05.sh — Verify scene 05 only (faster than full video)
...
SCENE_CLIP="$ANALYSIS_DIR/scene_clips/scene_05.mp4"
COMP_ID="ReplicateScene05"
ATTEMPT="/tmp/scene-05-attempt-$$.mp4"
...
# Scale to match
ATTEMPT_SCALED="/tmp/scene-05-scaled-$$.mp4"
...
for i in 0 1 2 3 4; do
  T=$(python3 -c "print(round(50.684 + 23.172 * $i / 5, 2))")
  ffmpeg -i "$SCENE_CLIP" -ss $(python3 -c "print(round(23.172 * $i / 5, 2))") -frames:v 1 /tmp/_skf_ref_$i.png -y 2>/dev/null || continue
  ffmpeg -i "$ATTEMPT" -ss $(python3 -c "print(round(23.172 * $i / 5, 2))") -frames:v 1 /tmp/_skf_att_$i.png -y 2>/dev/null || continue
...
cat > "$ANALYSIS_DIR/scene_05_breakdown.json" << JSONEOF
{
  "scene": 5,
  "score": $SCORE,
