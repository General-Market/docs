#!/bin/bash
# verify-scene-03.sh — Verify scene 03 only (faster than full video)
...
SCENE_CLIP="$ANALYSIS_DIR/scene_clips/scene_03.mp4"
COMP_ID="ReplicateScene03"
ATTEMPT="/tmp/scene-03-attempt-$$.mp4"
...
# Scale to match
ATTEMPT_SCALED="/tmp/scene-03-scaled-$$.mp4"
...
for i in 0 1 2 3 4; do
  T=$(python3 -c "print(round(14.481 + 24.858 * $i / 5, 2))")
  ffmpeg -i "$SCENE_CLIP" -ss $(python3 -c "print(round(24.858 * $i / 5, 2))") -frames:v 1 /tmp/_skf_ref_$i.png -y 2>/dev/null || continue
  ffmpeg -i "$ATTEMPT" -ss $(python3 -c "print(round(24.858 * $i / 5, 2))") -frames:v 1 /tmp/_skf_att_$i.png -y 2>/dev/null || continue
...
cat > "$ANALYSIS_DIR/scene_03_breakdown.json" << JSONEOF
{
  "scene": 3,
  "score": $SCORE,
