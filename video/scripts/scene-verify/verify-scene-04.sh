#!/bin/bash
# verify-scene-04.sh — Verify scene 04 only (faster than full video)
...
SCENE_CLIP="$ANALYSIS_DIR/scene_clips/scene_04.mp4"
COMP_ID="ReplicateScene04"
ATTEMPT="/tmp/scene-04-attempt-$$.mp4"
...
# Scale to match
ATTEMPT_SCALED="/tmp/scene-04-scaled-$$.mp4"
...
for i in 0 1 2 3 4; do
  T=$(python3 -c "print(round(39.339 + 11.311 * $i / 5, 2))")
  ffmpeg -i "$SCENE_CLIP" -ss $(python3 -c "print(round(11.311 * $i / 5, 2))") -frames:v 1 /tmp/_skf_ref_$i.png -y 2>/dev/null || continue
  ffmpeg -i "$ATTEMPT" -ss $(python3 -c "print(round(11.311 * $i / 5, 2))") -frames:v 1 /tmp/_skf_att_$i.png -y 2>/dev/null || continue
...
cat > "$ANALYSIS_DIR/scene_04_breakdown.json" << JSONEOF
{
  "scene": 4,
  "score": $SCORE,
