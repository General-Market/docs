// Conveyor Builder — distilled from masahito's "box flow system" CodePen, a
// drag-to-build factory of conveyor belts, pneumatic tubes and little boxes
// with pixel faces. The editor machinery (dev mode, add/delete, config IO,
// pointer dragging, the setInterval physics loop) is gone. What remains is the
// *result*: a small flat top-down factory where boxes ride belts, get sucked up
// through tubes, drop, and circulate on a seamless loop. All motion is derived
// from useCurrentFrame — no physics, no events, no interactivity.

import React, { useMemo } from "react";
import {
  AbsoluteFill,
  useCurrentFrame,
  useVideoConfig,
  interpolate,
  Easing,
} from "remotion";

// ── Palette lifted from the source CSS ───────────────────────────────────────

const FLOOR = "#b4b3b3"; // .wrapper background
const BACKDROP = "#797979"; // body background
const CONTROL = "#1b8aab"; // --control (belt frame / tube accent)
const TUBE_BODY = "#5a6b72"; // pneumatic pipe body
const TUBE_RIM = "#3f4d52";
const BELT_FRAME = "#2d3338";
const BELT_TREAD_A = "#3c454b";
const BELT_TREAD_B = "#525d64";
const BOX_COLORS = ["#42c6d2", "#797979", "#ffffff"] as const; // box --bg options

// The original is laid out on a 2000×2000 wrapper but the visible config sits in
// a ~1400×820 band. We model the scene in its own coordinate space and scale it
// to fill 1920×1080.

const WORLD_W = 1440;
const WORLD_H = 810;

// ── Geometry primitives ──────────────────────────────────────────────────────

type Belt = {
  x: number; // left edge
  y: number; // top of tread surface (boxes ride here)
  w: number; // width
  dir: 1 | -1; // +1 → boxes move right, -1 → move left
  speed: number; // px per loop-second
};

type Tube = {
  // a tube is a vertical lift: boxes enter at the bottom, rise to the top
  x: number; // center x
  bottom: number; // y where a box enters
  top: number; // y where a box exits
  w: number; // pipe width
};

// One closed circulation: a belt carries boxes to a lift tube, the tube raises
// them, they drop onto the belt above, ride back the other way, and a second
// tube lowers... we fake the lower with a "drop chute" so the loop is legible.
// Each lane is a self-contained ring so the whole field loops perfectly.

type Lane = {
  beltLow: Belt; // lower belt
  beltHigh: Belt; // upper belt (opposite direction)
  liftTube: Tube; // right/left lift connecting low → high
  dropTube: Tube; // returns high → low
  boxCount: number;
  colorSeed: number;
};

// Static set of belts purely for visual density (no boxes ride these — they sit
// in the background to fill the factory floor like the original's clutter).
const DECOR_BELTS: Belt[] = [
  { x: 60, y: 120, w: 360, dir: 1, speed: 90 },
  { x: 980, y: 150, w: 300, dir: -1, speed: 70 },
  { x: 1050, y: 470, w: 320, dir: 1, speed: 80 },
  { x: 70, y: 520, w: 280, dir: -1, speed: 60 },
  { x: 600, y: 700, w: 260, dir: 1, speed: 95 },
];

const LANES: Lane[] = [
  {
    beltLow: { x: 180, y: 360, w: 620, dir: 1, speed: 150 },
    beltHigh: { x: 180, y: 230, w: 620, dir: -1, speed: 150 },
    liftTube: { x: 820, bottom: 360, top: 230, w: 46 },
    dropTube: { x: 168, bottom: 360, top: 230, w: 46 },
    boxCount: 7,
    colorSeed: 0,
  },
  {
    beltLow: { x: 560, y: 640, w: 700, dir: -1, speed: 130 },
    beltHigh: { x: 560, y: 500, w: 700, dir: 1, speed: 130 },
    liftTube: { x: 540, bottom: 640, top: 500, w: 46 },
    dropTube: { x: 1278, bottom: 640, top: 500, w: 46 },
    boxCount: 8,
    colorSeed: 3,
  },
];

const BELT_THICKNESS = 22;
const BOX_SIZE = 26;
const LOOP_SECONDS = 10; // matches 600 frames @ 60fps

// Each lane forms a ring whose perimeter is travelled at constant speed. We map
// a normalized phase p∈[0,1) onto a position + which segment the box is on.

type RingPoint = {
  x: number;
  y: number;
  z: number; // draw order: tubes (in transit) sit above belts
  squash: number; // landing plop, 1 = none
  expression: "neutral" | "joy" | "surprise" | "sleepy";
};

function ringLengths(lane: Lane) {
  const beltLen = lane.beltLow.w - 60; // usable run, minus tube footprints
  const liftLen = lane.liftTube.bottom - lane.liftTube.top;
  const dropLen = lane.dropTube.bottom - lane.dropTube.top;
  const total = beltLen + liftLen + beltLen + dropLen;
  return { beltLen, liftLen, dropLen, total };
}

function ringPoint(lane: Lane, p: number): RingPoint {
  const { beltLow, beltHigh, liftTube, dropTube } = lane;
  const { beltLen, liftLen, dropLen, total } = ringLengths(lane);
  const d = ((p % 1) + 1) % 1 * total;

  const lowSurface = beltLow.y - BELT_THICKNESS / 2 - BOX_SIZE / 2;
  const highSurface = beltHigh.y - BELT_THICKNESS / 2 - BOX_SIZE / 2;

  // Segment 1: ride the low belt from the drop tube to the lift tube. The
  // ring topology fixes the travel direction; the belt's tread direction is a
  // separate visual choice authored per lane.
  if (d < beltLen) {
    const t = d / beltLen;
    return {
      x: dropTube.x + (liftTube.x - dropTube.x) * t,
      y: lowSurface,
      z: 1,
      squash: plop(t),
      expression: "neutral",
    };
  }

  // Segment 2: lift tube raises the box (low → high).
  let d2 = d - beltLen;
  if (d2 < liftLen) {
    const t = d2 / liftLen;
    const eased = Easing.inOut(Easing.cubic)(t);
    return {
      x: liftTube.x,
      y: liftTube.bottom - (liftTube.bottom - liftTube.top) * eased - BOX_SIZE / 2,
      z: 3,
      squash: 1,
      expression: "joy",
    };
  }

  // Segment 3: ride the high belt back the other way. The box leaves the lift
  // at liftTube.x and rides to the drop tube at dropTube.x.
  d2 -= liftLen;
  if (d2 < beltLen) {
    const t = d2 / beltLen;
    return {
      x: liftTube.x + (dropTube.x - liftTube.x) * t,
      y: highSurface,
      z: 1,
      squash: plop(t),
      expression: "neutral",
    };
  }

  // Segment 4: drop tube lowers the box (high → low).
  d2 -= beltLen;
  const t = d2 / dropLen;
  const eased = Easing.inOut(Easing.cubic)(t);
  return {
    x: dropTube.x,
    y: dropTube.top + (dropTube.bottom - dropTube.top) * eased - BOX_SIZE / 2,
    z: 3,
    squash: 1,
    expression: "surprise",
  };
}

// Gentle squash near the start of a belt run, mimicking the original's `plop`
// landing animation when a box meets a surface.
function plop(t: number): number {
  if (t > 0.12) return 1;
  const u = t / 0.12;
  // 1.18 → 0.92 → 1.0 quick bounce
  return interpolate(u, [0, 0.5, 1], [1.16, 0.9, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
}

// ── Visual components ─────────────────────────────────────────────────────────

const BeltView: React.FC<{ belt: Belt; treadOffset: number }> = ({
  belt,
  treadOffset,
}) => {
  const stripe = 18;
  // Tread scroll: shift the repeating gradient by the surface offset. Direction
  // follows belt.dir so the visible tread moves the same way the boxes do.
  const bg = `repeating-linear-gradient(90deg, ${BELT_TREAD_A} 0px, ${BELT_TREAD_A} ${stripe}px, ${BELT_TREAD_B} ${stripe}px, ${BELT_TREAD_B} ${stripe * 2}px)`;
  const offset = (treadOffset * belt.dir) % (stripe * 2);
  return (
    <div
      style={{
        position: "absolute",
        left: belt.x,
        top: belt.y - BELT_THICKNESS / 2,
        width: belt.w,
        height: BELT_THICKNESS,
        borderRadius: 4,
        background: BELT_FRAME,
        boxShadow: `0 4px 0 rgba(0,0,0,0.18), inset 0 0 0 2px ${CONTROL}`,
        overflow: "hidden",
      }}
    >
      {/* moving tread */}
      <div
        style={{
          position: "absolute",
          inset: 3,
          borderRadius: 2,
          backgroundImage: bg,
          backgroundPositionX: `${offset}px`,
        }}
      />
      {/* end rollers */}
      <div style={rollerStyle("left")} />
      <div style={rollerStyle("right")} />
    </div>
  );
};

const rollerStyle = (side: "left" | "right"): React.CSSProperties => ({
  position: "absolute",
  top: 0,
  bottom: 0,
  width: 8,
  [side]: 0,
  background: `linear-gradient(180deg, ${CONTROL}, ${BELT_FRAME})`,
  borderRadius: 4,
});

const TubeView: React.FC<{ tube: Tube }> = ({ tube }) => {
  const height = tube.bottom - tube.top + BELT_THICKNESS;
  return (
    <div
      style={{
        position: "absolute",
        left: tube.x - tube.w / 2,
        top: tube.top - BELT_THICKNESS / 2,
        width: tube.w,
        height,
        borderRadius: tube.w / 2,
        background: `linear-gradient(90deg, ${TUBE_RIM} 0%, ${TUBE_BODY} 28%, #7d8c93 50%, ${TUBE_BODY} 72%, ${TUBE_RIM} 100%)`,
        boxShadow: `inset 0 0 0 2px ${TUBE_RIM}`,
        zIndex: 2,
      }}
    >
      {/* segment rings every 24px, matching the tube's 24px modules */}
      {Array.from({ length: Math.floor(height / 24) }).map((_, i) => (
        <div
          key={i}
          style={{
            position: "absolute",
            left: 0,
            right: 0,
            top: i * 24 + 10,
            height: 2,
            background: "rgba(0,0,0,0.22)",
          }}
        />
      ))}
      {/* glossy highlight */}
      <div
        style={{
          position: "absolute",
          top: 4,
          bottom: 4,
          left: tube.w * 0.32,
          width: 3,
          borderRadius: 2,
          background: "rgba(255,255,255,0.35)",
        }}
      />
    </div>
  );
};

const Face: React.FC<{ expression: RingPoint["expression"] }> = ({
  expression,
}) => {
  const ink = "#1c1c1c";
  // simple pixel-art-ish faces echoing the original sprite expressions
  const eye = (cx: number) => (
    <rect x={cx} y={9} width={3} height={3} fill={ink} />
  );
  let mouth: React.ReactNode;
  switch (expression) {
    case "joy":
      mouth = <path d="M9 16 q4 4 8 0" stroke={ink} strokeWidth={2} fill="none" />;
      break;
    case "surprise":
      mouth = <rect x={11} y={15} width={4} height={4} fill={ink} />;
      break;
    case "sleepy":
      mouth = <rect x={9} y={17} width={8} height={2} fill={ink} />;
      break;
    default:
      mouth = <rect x={9} y={16} width={8} height={2} fill={ink} />;
  }
  return (
    <svg
      width={BOX_SIZE}
      height={BOX_SIZE}
      viewBox="0 0 26 26"
      style={{ position: "absolute", inset: 0 }}
    >
      {eye(8)}
      {eye(15)}
      {mouth}
    </svg>
  );
};

const BoxView: React.FC<{ pt: RingPoint; color: string }> = ({ pt, color }) => {
  return (
    <div
      style={{
        position: "absolute",
        left: pt.x - BOX_SIZE / 2,
        top: pt.y,
        width: BOX_SIZE,
        height: BOX_SIZE,
        background: color,
        borderRadius: 3,
        boxShadow: "0 3px 4px rgba(0,0,0,0.25), inset 0 0 0 2px rgba(0,0,0,0.12)",
        transform: `scaleY(${pt.squash}) scaleX(${1 + (1 - pt.squash) * 0.6})`,
        transformOrigin: "center bottom",
        zIndex: pt.z * 10,
      }}
    >
      <Face expression={pt.expression} />
    </div>
  );
};

// ── Composition ───────────────────────────────────────────────────────────────

export const ConveyorBuilder: React.FC = () => {
  const frame = useCurrentFrame();
  const { width, height, durationInFrames } = useVideoConfig();

  // Loop clock in seconds, wrapping at LOOP_SECONDS for a seamless cycle.
  const loopFrames = durationInFrames; // 600
  const tLoop = ((frame % loopFrames) / loopFrames) * LOOP_SECONDS; // 0..10

  // Scale the world to cover the frame (contain inside, then fill via cover).
  const scale = useMemo(
    () => Math.max(width / WORLD_W, height / WORLD_H),
    [width, height],
  );
  const offsetX = (width - WORLD_W * scale) / 2;
  const offsetY = (height - WORLD_H * scale) / 2;

  // Precompute box phases per lane so boxes are evenly spaced and already
  // mid-journey on frame 1.
  const lanes = LANES.map((lane) => {
    const beltScrollSec = tLoop;
    const points: { pt: RingPoint; color: string }[] = [];
    for (let i = 0; i < lane.boxCount; i++) {
      const phase0 = i / lane.boxCount;
      // travel one full ring per LOOP_SECONDS so everything closes on the loop
      const p = phase0 + tLoop / LOOP_SECONDS;
      const pt = ringPoint(lane, p);
      const color = BOX_COLORS[(i + lane.colorSeed) % BOX_COLORS.length];
      points.push({ pt, color });
    }
    return { lane, points, beltScrollSec };
  });

  // Decor belt tread offset, in px, derived from the same loop clock.
  const decorOffset = (px: number) => (px * tLoop) % 1000;

  return (
    <AbsoluteFill style={{ backgroundColor: BACKDROP }}>
      {/* Factory floor */}
      <div
        style={{
          position: "absolute",
          left: offsetX,
          top: offsetY,
          width: WORLD_W,
          height: WORLD_H,
          transform: `scale(${scale})`,
          transformOrigin: "top left",
          background: FLOOR,
          boxShadow: "inset 0 0 120px rgba(0,0,0,0.18)",
        }}
      >
        {/* subtle floor grid for the engineered look */}
        <div
          style={{
            position: "absolute",
            inset: 0,
            backgroundImage:
              "linear-gradient(rgba(0,0,0,0.05) 1px, transparent 1px), linear-gradient(90deg, rgba(0,0,0,0.05) 1px, transparent 1px)",
            backgroundSize: "40px 40px",
          }}
        />

        {/* Background decoration belts */}
        {DECOR_BELTS.map((b, i) => (
          <BeltView key={`decor-${i}`} belt={b} treadOffset={decorOffset(b.speed)} />
        ))}

        {/* Lanes: drop/return tube drawn behind belts, lift tube in front */}
        {lanes.map(({ lane }, i) => (
          <React.Fragment key={`tube-back-${i}`}>
            <TubeView tube={lane.dropTube} />
          </React.Fragment>
        ))}

        {/* Lane belts */}
        {lanes.map(({ lane, beltScrollSec }, i) => (
          <React.Fragment key={`belts-${i}`}>
            <BeltView belt={lane.beltHigh} treadOffset={beltScrollSec * lane.beltHigh.speed} />
            <BeltView belt={lane.beltLow} treadOffset={beltScrollSec * lane.beltLow.speed} />
          </React.Fragment>
        ))}

        {/* Lift tubes in front of belts so rising boxes pass through them */}
        {lanes.map(({ lane }, i) => (
          <TubeView key={`tube-front-${i}`} tube={lane.liftTube} />
        ))}

        {/* Boxes */}
        {lanes.map(({ points }, li) =>
          points.map((b, bi) => (
            <BoxView key={`box-${li}-${bi}`} pt={b.pt} color={b.color} />
          )),
        )}
      </div>

      {/* Vignette to seat the scene as a background */}
      <AbsoluteFill
        style={{
          pointerEvents: "none",
          background:
            "radial-gradient(120% 120% at 50% 45%, transparent 55%, rgba(0,0,0,0.28) 100%)",
        }}
      />
    </AbsoluteFill>
  );
};
