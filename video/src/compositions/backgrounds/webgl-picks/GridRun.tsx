// Grid Run — Matthias Hurrle's (@atzedent) ray-marched lattice corridor.
// The original was a live ShaderToy-style editor page. Here it is wired into
// a fullscreen ThreeCanvas quad with frame-driven time. The shader carries
// its own camera drift, so interaction uniforms (move, wheel) are pinned at
// zero — Remotion needs determinism, not a mouse.
...
// ── Fragment — verbatim from the source, minus the #version line which
//    ShaderMaterial injects when glslVersion === GLSL3 ──────────────────────
...
vec3 dirVec(vec2 uv, vec3 p, vec3 t, float z) {
    vec3 up=vec3(0,1,0),
    f=N(t-p),
    r=N(cross(up,f)),
    u=N(cross(f,r));
...
const ShaderQuad: React.FC<{ time: number }> = ({ time }) => {
  ...
  const uniforms = useMemo(
    ...
    // Re-create uniforms when canvas resizes so resolution stays correct
