// Liquid metal button — black pill, two rotated icons leaning toward the
// center, and a chromatic-flow disk where the paper-design shader used to
// live. paper-design isn't installed; the equivalent metallic noise is
// rebuilt in ~40 lines of GLSL.

import React, { useMemo, useRef } from "react";
import {
  AbsoluteFill,
  useCurrentFrame,
  useVideoConfig,
} from "remotion";
import { ThreeCanvas } from "@remotion/three";
import * as THREE from "three";

const BG = "#1d1d1d";
const DARK = "#000";
const LIGHT = "#444";
const ICON = "#65615f";

// ── Fragment shader: flowing fbm field, sampled three times for chromatic
//    shift, then tonemapped through a metallic gradient. The paper-design
//    look comes from contrast — light pools against deep shadow with hints
//    of warm/cool. ─────────────────────────────────────────────────────────

const FRAGMENT = /* glsl */ `
precision highp float;
varying vec2 vUv;
uniform float uTime;

float hash(vec2 p) {
  return fract(sin(dot(p, vec2(127.1, 311.7))) * 43758.5453123);
}
float noise(vec2 p) {
  vec2 i = floor(p);
  vec2 f = fract(p);
  vec2 u = f * f * (3.0 - 2.0 * f);
  return mix(
    mix(hash(i), hash(i + vec2(1.0, 0.0)), u.x),
    mix(hash(i + vec2(0.0, 1.0)), hash(i + vec2(1.0, 1.0)), u.x),
    u.y
  );
}
float fbm(vec2 p) {
  float v = 0.0;
  float a = 0.5;
  for (int i = 0; i < 5; i++) {
    v += a * noise(p);
    p = p * 2.05 + vec2(17.0, 31.0);
    a *= 0.55;
  }
  return v;
}

void main() {
  vec2 uv = vUv;
  vec2 p = (uv - 0.5) * 2.5;
  float t = uTime * 0.22;

  vec2 flow = vec2(t, t * 0.7);
  float a = fbm(p + flow);
  vec2 q = vec2(fbm(p + a + 4.0), fbm(p + a - 2.0));
  float r = fbm(p + 0.7 * q + flow * 0.5);
  float g = fbm(p + 0.7 * q);
  float b = fbm(p + 0.7 * q - flow * 0.5);

  // Pour the three channels into a single luminance, then drape a metallic
  // gradient over it for the polished-chrome quality
  float lum = clamp(0.33 * (r + g + b), 0.0, 1.0);
  lum = smoothstep(0.15, 0.85, lum);

  vec3 dark = vec3(0.08, 0.08, 0.11);
  vec3 mid = vec3(0.55, 0.56, 0.6);
  vec3 hi = vec3(0.98, 0.99, 1.02);

  vec3 metal = mix(dark, mid, smoothstep(0.0, 0.5, lum));
  metal = mix(metal, hi, smoothstep(0.55, 0.95, lum));

  // Faint chromatic tint from the channel divergence — keeps it from
  // looking like flat greyscale
  vec3 tint = vec3(r - g, g - b, b - r) * 0.18;
  metal += tint;

  // Vignette toward the disk's edge so the rim reads as metal, not paper
  float vig = 1.0 - smoothstep(0.7, 1.05, length(uv - 0.5) * 2.0);
  metal *= mix(0.5, 1.0, vig);

  gl_FragColor = vec4(clamp(metal, 0.0, 1.0), 1.0);
}
`;

const VERTEX = /* glsl */ `
varying vec2 vUv;
void main() {
  vUv = uv;
  gl_Position = vec4(position.xy, 0.0, 1.0);
}
`;

const ShaderQuad: React.FC<{ time: number }> = ({ time }) => {
  const matRef = useRef<THREE.ShaderMaterial>(null);
  const uniforms = useMemo(
    () => ({ uTime: { value: 0 } }),
    [],
  );
  if (matRef.current) {
    matRef.current.uniforms.uTime.value = time;
  }
  return (
    <mesh>
      <planeGeometry args={[2, 2]} />
      <shaderMaterial
        ref={matRef}
        vertexShader={VERTEX}
        fragmentShader={FRAGMENT}
        uniforms={uniforms}
        depthTest={false}
        depthWrite={false}
      />
    </mesh>
  );
};

// ── Attachment icon ─────────────────────────────────────────────────────────

const AttachmentIcon: React.FC<{ size: number }> = ({ size }) => (
  <svg
    viewBox="0 0 20 20"
    fill="currentColor"
    width={size}
    height={size}
    style={{ flexShrink: 0, transform: "rotate(70deg)" }}
  >
    <path d="M6.0678 2.16105C7.46414 1.61127 9.04215 2.29797 9.59221 3.69425L12.7983 11.8339C13.1238 12.6606 12.7177 13.5952 11.891 13.9208L11.8149 13.9511C10.9883 14.2765 10.0535 13.8695 9.72795 13.0429L8.02678 8.7255C7.92565 8.46868 8.05228 8.17836 8.30901 8.07706C8.56594 7.97586 8.85624 8.10236 8.95744 8.35929L10.6586 12.6767C10.7819 12.9894 11.1359 13.1436 11.4487 13.0204L11.5248 12.9901C11.8377 12.8669 11.9908 12.5129 11.8676 12.2001L8.66155 4.06046C8.31383 3.17839 7.31727 2.74467 6.43498 3.09171L6.28069 3.15226C5.39843 3.49996 4.96466 4.4974 5.31194 5.3798L9.18108 15.2011C9.75314 16.6533 11.3938 17.3667 12.8461 16.7948L13.0766 16.705C14.5288 16.1329 15.2432 14.4913 14.6713 13.039L12.308 7.03898C12.2069 6.78212 12.3325 6.49177 12.5893 6.39054C12.8461 6.28961 13.1365 6.41605 13.2377 6.67277L15.601 12.6728C16.3753 14.6389 15.4089 16.8601 13.4428 17.6347L13.2133 17.7255C11.2472 18.4998 9.025 17.5342 8.25041 15.5683L4.38225 5.74698C3.83217 4.35052 4.51801 2.77168 5.91448 2.22159L6.0678 2.16105Z" />
  </svg>
);

const SparklesIcon: React.FC<{ size: number }> = ({ size }) => (
  <svg
    viewBox="0 0 30 30"
    fill="currentColor"
    width={size}
    height={size}
    style={{ flexShrink: 0, transform: "rotate(180deg)" }}
  >
    <path d="M14.217,19.707l-1.112,2.547c-0.427,0.979-1.782,0.979-2.21,0l-1.112-2.547c-0.99-2.267-2.771-4.071-4.993-5.057L1.73,13.292c-0.973-0.432-0.973-1.848,0-2.28l2.965-1.316C6.974,8.684,8.787,6.813,9.76,4.47l1.126-2.714c0.418-1.007,1.81-1.007,2.228,0L14.24,4.47c0.973,2.344,2.786,4.215,5.065,5.226l2.965,1.316c0.973,0.432,0.973,1.848,0,2.28l-3.061,1.359C16.988,15.637,15.206,17.441,14.217,19.707z" />
    <path d="M24.481,27.796l-0.339,0.777c-0.248,0.569-1.036,0.569-1.284,0l-0.339-0.777c-0.604-1.385-1.693-2.488-3.051-3.092l-1.044-0.464c-0.565-0.251-0.565-1.072,0-1.323l0.986-0.438c1.393-0.619,2.501-1.763,3.095-3.195l0.348-0.84c0.243-0.585,1.052-0.585,1.294,0l0.348,0.84c0.594,1.432,1.702,2.576,3.095,3.195l0.986,0.438c0.565,0.251,0.565,1.072,0,1.323l-1.044,0.464C26.174,25.308,25.085,26.411,24.481,27.796z" />
  </svg>
);

const InnerSquareIcon: React.FC<{ size: number; color: string }> = ({
  size,
  color,
}) => (
  <svg
    viewBox="0 0 1024 1024"
    width={size}
    height={size}
    style={{ transform: "rotate(-45deg)", fill: color }}
  >
    <path d="M843.968 896a51.072 51.072 0 0 1-51.968-52.032V232H180.032A51.072 51.072 0 0 1 128 180.032c0-29.44 22.528-52.032 52.032-52.032h663.936c29.44 0 52.032 22.528 52.032 52.032v663.936c0 29.44-22.528 52.032-52.032 52.032z" />
    <path d="M180.032 896a49.92 49.92 0 0 1-36.48-15.616c-20.736-20.8-20.736-53.76 0-72.832L807.616 143.616c20.864-20.8 53.76-20.8 72.832 0 20.8 20.8 20.8 53.76 0 72.768L216.384 880.384a47.232 47.232 0 0 1-36.352 15.616z" />
  </svg>
);

// ── Composition ─────────────────────────────────────────────────────────────

export const LiquidMetalButton: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const time = frame / fps;

  const BUTTON = 480; // px — bigger than the source's 38.5rem so it reads on 1080p
  const ICON_SIZE = 220;

  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      {/* The black pill, pinned to the left edge */}
      <div
        style={{
          position: "absolute",
          left: 0,
          top: "50%",
          transform: "translateY(-50%)",
          display: "flex",
          justifyContent: "flex-end",
          alignItems: "center",
          gap: 160,
          padding: 40,
          width: "65%",
          height: BUTTON + 80,
          backgroundColor: DARK,
          borderRadius: "0 50vh 50vh 0",
          boxShadow: `inset 0 3px 0 0 ${LIGHT}`,
          color: ICON,
        }}
      >
        <AttachmentIcon size={ICON_SIZE} />
        <SparklesIcon size={ICON_SIZE} />

        {/* Liquid metal disk */}
        <div
          style={{
            position: "relative",
            width: BUTTON,
            height: BUTTON,
            borderRadius: "50%",
            flexShrink: 0,
            display: "grid",
            placeItems: "center",
          }}
        >
          {/* Conic outline ring */}
          <div
            style={{
              position: "absolute",
              inset: 0,
              borderRadius: "50%",
              padding: 6,
              background:
                "conic-gradient(from 180deg, blue, purple, red, purple, blue)",
              filter: "grayscale(1)",
              WebkitMask:
                "linear-gradient(#000 0 0) content-box, linear-gradient(#000 0 0)",
              WebkitMaskComposite: "xor",
              mask: "linear-gradient(#000 0 0) content-box, linear-gradient(#000 0 0)",
              maskComposite: "exclude",
            }}
          />

          {/* Inner disk with shader + inner highlight */}
          <div
            style={{
              position: "absolute",
              top: "50%",
              left: "50%",
              transform: "translate(-50%, -50%)",
              width: BUTTON - 30,
              height: BUTTON - 30,
              borderRadius: "50%",
              overflow: "hidden",
              boxShadow: "inset 0 3px 3px 3px rgba(255, 255, 255, 0.3)",
            }}
          >
            <ThreeCanvas
              width={BUTTON - 30}
              height={BUTTON - 30}
              gl={{ antialias: false, powerPreference: "high-performance" }}
            >
              <ShaderQuad time={time} />
            </ThreeCanvas>
          </div>

          {/* The square icon overlay */}
          <div
            style={{
              position: "relative",
              zIndex: 2,
              display: "grid",
              placeItems: "center",
            }}
          >
            <InnerSquareIcon size={130} color={ICON} />
          </div>
        </div>
      </div>
    </AbsoluteFill>
  );
};
