...
// SVG viewBox 0 0 2058 871. All path data copied verbatim from original.
// Timeline: 1s delay, "explode" label at 1s, "flight" label at 1.3s.
// 22 confetti images, SplitText chars, CustomBounce poly, Physics2D confetti,
...
const ASSET_DIR = "compositions/gsap-smooth";
...
const POLY_SRC = staticFile(`${ASSET_DIR}/3D-poly.png`);

// ── Timing ───────────────────────────────────────────────────────────────────
// Original: gsap.timeline({ delay: 1 }), labels "explode" at 1, "flight" at 1.3
// These are RELATIVE to the timeline start (which is already delayed by 1s).
// Absolute seconds: explode = 1+1 = 2s, flight = 1+1.3 = 2.3s

const DELAY = 1.0;
const EXPLODE = DELAY + 1.0;
const FLIGHT = DELAY + 1.3;
...
// CustomBounce "myBounce" with { strength: 0.6, squash: 3 }
...
// #path (pink curve, strokeWidth 12, round caps): from("#path", { dur: 0.5, drawSVG: 0 }, "explode")
// #path_2 (green squiggle, strokeWidth 3): from("#path_2", { dur: 0.8, drawSVG: 0 }, "flight")

const PATH_1 =
  "M538.5 556.481C565.165 497.803 621.326 446.333 685.817 449.897C742.695 453.011 791.23 499.923 807.851 554.295C822.28 601.485 806.483 665.329 758.267 675.637C710.051 685.944 669.808 635.984 657.212 588.396C638.16 517.04 652.243 437.987 694.679 377.68C737.115 317.374 806.881 277.006 880.5 270.501";
const PATH_2 =
  "M973.861 226.794C1015.92 240.459 1041.39 136.212 1005.93 135.899C977.513 135.649 990.28 214.204 1046.61 229.17C1089.82 240.65 1168.88 147.886 1092.89 84.6262C1022.57 26.0944 1052.01 288.336 1197.12 209.704";
...
// GSAP: from(".sprinkle", { scale: 0, rotation: 360, ease: "back.out" }, "explode")
...
const SPRINKLES: Sprinkle[] = [
  {
    type: "path",
    d: "M733.022 222.569L709.927 203.163C709.819 203.073 709.659 203.087 709.569 203.194L690.163 226.289C690.073 226.397 690.087 226.556 690.194 226.647L713.29 246.052C713.397 246.143 713.557 246.129 713.647 246.021L733.053 222.926C733.143 222.819 733.129 222.659 733.022 222.569Z",
    fill: "#397DFF",
  },
  ...
  {
    type: "path",
    d: "M1172.43 119.19C1161.04 107.84 1141.32 122.2 1150.1 137.037",
    stroke: "#FF783E",
    strokeWidth: 12,
    strokeLinecap: "round",
  ...
  {
    type: "path",
    d: "M1209.36 520.058C1200.77 520.837 1192.24 522.07 1183.8 523.845C1177.31 525.218 1179.8 535.21 1186.31 533.855C1193.88 532.265 1201.5 531.082 1209.21 530.374C1215.74 529.778 1215.98 519.458 1209.36 520.058Z",
    fill: "#BAA5F5",
    stroke: "#1B1E1A",
    strokeWidth: 1.5,
  },
  {
    type: "path",
    d: "M816.237 177.713C818.964 185.892 822.12 193.912 825.782 201.719C828.608 207.725 837.761 203.006 834.952 196.986C831.668 189.98 828.767 182.829 826.312 175.491C824.233 169.269 814.133 171.4 816.237 177.713Z",
    fill: "#BAA5F5",
    stroke: "#1B1E1A",
    strokeWidth: 1.5,
  },
  {
    type: "path",
    d: "M946.737 711.212C949.464 719.39 952.62 727.411 956.282 735.218C959.108 741.223 968.261 736.505 965.452 730.485C962.168 723.479 959.267 716.328 956.812 708.99C954.733 702.768 944.633 704.899 946.737 711.212Z",
    fill: "#B82C6F",
    stroke: "#1B1E1A",
    strokeWidth: 1.5,
  },
  {
    type: "path",
    d: "M823.961 771.534C820.994 776.919 818.34 782.573 816.071 788.532C814.331 793.123 821.749 798.298 823.506 793.714C825.543 788.368 827.863 783.26 830.521 778.421C832.776 774.321 826.252 767.377 823.961 771.534Z",
    fill: "#FF783E",
    stroke: "#1B1E1A",
    strokeWidth: 1.5,
  },
  {
    type: "path",
    d: "M1155.56 507.934C1152.64 507.082 1150.25 508.853 1149.16 511.444C1147.18 516.21 1145.2 520.99 1143.2 525.751C1142.12 528.327 1144.25 531.431 1146.71 532.149C1149.63 533.001 1152.02 531.229 1153.11 528.638C1155.09 523.873 1157.07 519.107 1159.07 514.331C1160.15 511.755 1158.02 508.651 1155.56 507.934Z",
    fill: "#FF783E",
    stroke: "#1B1E1A",
    strokeWidth: 1.5,
  },
  {
    type: "path",
    d: "M816.771 232.868C816.612 235.909 818.885 237.83 821.654 238.29C826.748 239.127 831.856 239.965 836.948 240.817C839.703 241.276 842.237 238.494 842.371 235.933C842.53 232.892 840.257 230.971 837.487 230.511C832.394 229.674 827.3 228.837 822.193 227.984C819.438 227.525 816.905 230.306 816.771 232.868Z",
    fill: "#FF783E",
    stroke: "#1B1E1A",
    strokeWidth: 1.5,
  },
  {
    type: "path",
    d: "M1155.78 596.226L1147.07 574.282C1146.03 571.668 1142.31 571.051 1140.07 572.243C1137.37 573.689 1137.01 576.679 1138.03 579.241C1140.95 586.559 1143.84 593.872 1146.74 601.186C1147.78 603.8 1151.51 604.417 1153.74 603.225C1156.45 601.778 1156.8 598.789 1155.78 596.226Z",
    fill: "#A6CFE7",
    stroke: "#1B1E1A",
    strokeWidth: 1.5,
  },
        ...
        {SPRINKLES.map((s, i) => {
          const stP = Math.max(
            ...
            Math.min(
              1,
              (p - i * 0.03) /
                Math.max(0.01, 1 - SPRINKLES.length * 0.03),
            ),
          );
          ...
          return (
            <path
              ...
              strokeMiterlimit={10}
              strokeLinecap={
                (s.strokeLinecap as "round" | "butt" | "square") || undefined
              }
              strokeLinejoin={
                (s.strokeLinejoin as "round" | "miter" | "bevel") || undefined
              }
              style={{
                transform: `rotate(${rt}deg) scale(${sc})`,
...
// from("#plane", { dur: 1, ease: "sine.inOut", scale: 0.2,
//   motionPath: { path, align: "#path_2", autoRotate: 180, start: 1, end: 0 } }, "flight")
// to(".innerplane", { dur: 0.2, opacity: 0 }, 2)  — relative to delay, absolute = DELAY+2
...
const PLANE_PATHS = {
  ...
  inner1:
    "M1286.78 195.303L1452.45 134.639L1293.52 213.708L1286.78 195.303Z",
  inner2:
    "M1446.47 137.211L1293.72 213.209L1287.24 195.519L1446.47 137.211ZM1458.42 132.07L1286.32 195.088L1293.32 214.21L1458.42 132.07Z",
};

// Motion path keypoints sampled from #path_2, mapped to screen coords
...
const PlaneLayer: React.FC<{ p: number; fadeP: number }> = ({ p, fadeP }) => {
  ...
  // Original: start: 1, end: 0 — plane traverses path backwards
  ...
  const cx =
    PLANE_MOTION[idx].x +
    (PLANE_MOTION[idx + 1].x - PLANE_MOTION[idx].x) * frac;
  const cy =
    PLANE_MOTION[idx].y +
    (PLANE_MOTION[idx + 1].y - PLANE_MOTION[idx].y) * frac;

  // autoRotate: angle from path direction + 180
  const dx =
    PLANE_MOTION[Math.min(idx + 1, PLANE_MOTION.length - 1)].x -
    PLANE_MOTION[idx].x;
  const dy =
    PLANE_MOTION[Math.min(idx + 1, PLANE_MOTION.length - 1)].y -
    PLANE_MOTION[idx].y;
...
const HandLayer: React.FC<{ enterP: number; wiggleP: number }> = ({
  enterP,
  wiggleP,
}) => {
  ...
  // CustomWiggle { wiggles: 6 } — damped sine, 6 oscillations
  const wiggle =
    wiggleP > 0
      ? 30 * Math.sin(wiggleP * Math.PI * 12) * (1 - wiggleP)
      : 0;
...
// ── 3D Polyhedron (#main) ────────────────────────────────────────────────────
// Original: <image href="3D-poly.png" x="758" y="320" width="344" height="370" />
// GSAP: from("#main", { dur: 2, opacity: 0, y: -2000, ease: "myBounce" }, 0.5)
// GSAP: to("#main", { dur: 2, scaleX: 1.4, scaleY: 0.6, ease: "myBounce-squash", transformOrigin: "center bottom" }, 0.5)

const HeroPoly: React.FC<{ bounceP: number; squashP: number }> = ({
  bounceP,
  squashP,
}) => {
  ...
  const imgW = (344 / OW) * 1920;
  const imgH = (370 / OH) * 1080;
...
// ── Rotated box element ──────────────────────────────────────────────────────
// Original: <rect id="box" x="539" y="469.42" width="87.0018" height="80.7347"
//   transform="rotate(-83.3705 539 469.42)" fill="url(#pattern)" />
// Textured with noise pattern — we overlay the noise PNG
  ...
  return (
    ...
    >
      <defs>
        ...
        >
          <image
            href={staticFile(`${ASSET_DIR}/noise.png`)}
            width="500"
            height="500"
          />
        </pattern>
...
// ── Confetti (Physics2D) ─────────────────────────────────────────────────────
// Original: 22 .confetti img elements (11 unique images x 2 each)
// GSAP: set(".confetti img", { opacity: 1 }, "explode+=.2")
// GSAP: to(".confetti img", { dur: 2, rotation: "random(-360,360)", scale: "random(0.5,1)",
//   physics2D: { velocity: "random(800,2000)", angle: "random(150,360)", gravity: 3000, acceleration: 100 }
// }, "explode+=.2")
...
function genConfetti(count: number): ConfettiPiece[] {
  return Array.from({ length: count }, (_, i) => {
    ...
    const imgFile = CONFETTI_IMAGES[i % CONFETTI_IMAGES.length];
    return {
      src: staticFile(`${ASSET_DIR}/${imgFile}`),
      vx: speed * Math.cos(angleRad),
      vy: speed * Math.sin(angleRad),
      rotEnd: rr(i * 11, -360, 360),
      scaleEnd: rr(i * 19, 0.5, 1),
      size: 30,
    };
  });
}
...
const ConfettiField: React.FC<{ p: number; durSec: number }> = ({
  p,
  durSec,
}) => {
  if (p <= 0) return null;
  ...
  return (
    <>
      {CONFETTI.map((pc, i) => {
        const x = centerX + pc.vx * t;
        const y = centerY + pc.vy * t + 0.5 * CONFETTI_GRAVITY * t * t;
        const rotation = interpolate(p, [0, 1], [0, pc.rotEnd]);
        const scale = interpolate(p, [0, 1], [0.1, pc.scaleEnd]);
...
// GSAP: from([free.chars, all.chars], { dur: 0.7, y: "random([-500,500])",
//   rotation: "random([-30,30])", ease: "expo.out",
...
export const GsapSmooth: React.FC = () => {
  ...
  // 1. Text chars: from([free.chars, all.chars], { dur: 0.7, stagger: { amount: 0.3 } })
  //    at timeline t=0 → absolute = DELAY
  const textP = prog(frame, fps, DELAY, 0.7 + 0.3);
  const staggerAmount = 0.3;

  // 2. Poly bounce: from("#main", { dur: 2, y: -2000, ease: "myBounce" }, 0.5) → DELAY + 0.5
  // 3. Poly squash: to("#main", { dur: 2, scaleX: 1.4, scaleY: 0.6 }, 0.5) → same timing
  const polyP = prog(frame, fps, DELAY + 0.5, 2);

  // 4. Text slide: to("#free", { dur: 2, xPercent: -20, ease: "elastic.out(1,0.3)" }, "explode")
  const freeSlideP = prog(frame, fps, EXPLODE, 2);
  // 5. Text slide: to("#all", { dur: 2, xPercent: 50, ease: "elastic.out(1,0.3)" }, "explode")
  const allSlideP = prog(frame, fps, EXPLODE, 2);

  // 7. DrawSVG #path: from("#path", { dur: 0.5, drawSVG: 0 }, "explode")
  const drawPath1P = prog(frame, fps, EXPLODE, 0.5);

  // 8. DrawSVG #path_2: from("#path_2", { dur: 0.8, drawSVG: 0 }, "flight")
  const drawPath2P = prog(frame, fps, FLIGHT, 0.8);

  // 14. Bang + spin: from("#bang, #spin", { dur: 0.7, ease: "back.out(4)" }, "explode+=.1")
  const bangP = prog(frame, fps, EXPLODE + 0.1, 0.7);

  // 13. Wiggle: from("#wiggle", { dur: 0.7, ease: "back.out(4)" }, "explode+=.4")
  const wiggleP = prog(frame, fps, EXPLODE + 0.4, 0.7);

  // 15. Sprinkles: from(".sprinkle", { scale: 0, rotation: 360, ease: "back.out" }, "explode")
  const sprinklesP = prog(frame, fps, EXPLODE, 0.7);

  // Box: same timing as sprinkles
  const boxP = sprinklesP;

  // 16. FFD: from("#ffd", { xPercent: -800, opacity: 0, ease: "back.out" }, "explode")
  const ffdP = prog(frame, fps, EXPLODE, 0.7);

  // 11-12. Confetti: set + to at "explode+=.2", duration 2
  const confettiP = prog(frame, fps, EXPLODE + 0.2, 2);

  // 6. Plane visibility: set("#plane", { opacity: 1 }, "flight")
  // 9. Plane motion: from("#plane", { dur: 1, ease: "sine.inOut", ... }, "flight")
  const planeP = prog(frame, fps, FLIGHT, 1);
  // 10. Inner plane fade: to(".innerplane", { dur: 0.2, opacity: 0 }, 2) → DELAY + 2
  const planeFadeP = prog(frame, fps, DELAY + 2, 0.2);

  // 17-18. Hand enter + wiggle
  ...
  return (
    ...
    >
      ...
      {/* 3D polyhedron — real PNG now */}
      <HeroPoly bounceP={polyP} squashP={polyP} />

      {/* "free " text — #free { left: 10%, top: 45%, font-size: 8vw } */}
      <div
        style={{
          position: "absolute",
          left: "10%",
      ...
      >
        <AnimText
          ...
          charProgress={(i) => {
            ...
            const cp = Math.max(
              0,
              Math.min(1, (textP * total - charStagger) / dur),
            );
            return expoOut(cp);
          }}
        />
      ...
      {/* "for all" text — #all { left: 41%, top: 45% } */}
      <div
        style={{
          position: "absolute",
          left: "41%",
      ...
      {/* Confetti — real PNG images with Physics2D trajectories */}
      <ConfettiField p={confettiP} durSec={2} />

      {/* Plane — SVG overlay for transforms */}
      <svg
