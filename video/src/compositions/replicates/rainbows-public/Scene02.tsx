...
const Showcase: React.FC<{
  ...
  return (
    <div style={{ position: "absolute", inset: 0, opacity: op, transform: `translateX(${tx}px)` }}>
      ...
      {cards}
      {/* Label */}
      <div style={{ position: "absolute", right: 60, top: "50%", transform: "translateY(-50%)", maxWidth: 600, textAlign: "right" }}>
        <div style={{ fontSize: label.length > 10 ? 56 : 68, fontWeight: 700, color: C.navy, fontFamily: F.h, letterSpacing: -1.5, lineHeight: 1.05 }}>
          {label}
        </div>
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: C.bg }}>
      ...
      {frame >= 144 && frame < 220 && (() => {
        ...
        const op = en * (1 - ex);
        const phoneLabels = ["Frontrunning\nDesk", "Insider\nFlow", "Manipulation\nBook"];
      ...
      {/* ── 6. "70%" — glass payload reveal (212-258) ── */}
      {frame >= 204 && frame < 266 && (() => {
        const lf = frame - 212;
        const en = sp(lf);
        ...
        return (
          ...
          >
            {/* Three.js glass sparkle behind the number */}
            <div style={{ position: "relative", width: 600, height: 280 }}>
              ...
              {/* Foreground "70%" — sits over the glass sparkle */}
              <div
                style={{
                  position: "absolute",
                  inset: 0,
                  ...
                  color: C.blue,
                  letterSpacing: -8,
                  textShadow: "0 4px 30px rgba(40,69,224,0.25)",
