 ...
 * Minimal, institutional, hypnotic.
 */
...
const FRAGMENT = /* glsl */ `
  ...
  void main() {
    ...
    float b1 = blob(p, vec2(sin(t*0.12)*0.3, cos(t*0.1)*0.2), 0.35, t, 0.0);
    float b2 = blob(p, vec2(cos(t*0.1+1.0)*0.25-0.15, sin(t*0.08+2.0)*0.25), 0.28, t, 10.0);
    float b3 = blob(p, vec2(sin(t*0.09+3.0)*0.2+0.2, cos(t*0.11+1.5)*0.2-0.1), 0.22, t, 20.0);
    ...
    vec3 deepGreen = vec3(0.0, 0.541, 0.353);
    vec3 darkForest = vec3(0.039, 0.18, 0.11);
    vec3 mint = vec3(0.9, 1.0, 0.94);
    ...
    color += vec3(0.0, max(bgNoise, 0.0), max(bgNoise * 0.7, 0.0));
...
export const GradientMorph: React.FC = () => <ShaderScene fragmentShader={FRAGMENT} />;
