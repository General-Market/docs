...
function buildParticles(accent: string, W: number, H: number): Particle[] {
...
const ParticlesBg: React.FC<{ accent: string }> = ({ accent }) => {
  ...
  const particles = React.useMemo(() => buildParticles(accent, W, H), [accent, W, H]);
...
function buildMatrixColumns(W: number): MatrixColumn[] {
...
const MatrixBg: React.FC = () => {
  const frame = useCurrentFrame();
  const { width: W, height: H } = useVideoConfig();
  const columns = React.useMemo(() => buildMatrixColumns(W), [W]);
...
function buildBokeh(W: number, H: number): BokehCircle[] {
...
const BokehBg: React.FC = () => {
  const frame = useCurrentFrame();
  const { width: W, height: H } = useVideoConfig();
  const circles = React.useMemo(() => buildBokeh(W, H), [W, H]);
