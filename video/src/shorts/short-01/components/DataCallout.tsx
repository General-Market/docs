 ...
 */
const fitText = (text: string, maxFontSize: number, maxScale: number, maxWidth: number) => {
  const words = text.split(/\s+/);
  const longestWord = words.reduce((a, b) => (a.length > b.length ? a : b), "");

  // Width of longest word at a given fontSize:
  // charCount * fontSize * CHAR_RATIO + (charCount - 1) * LETTER_SPACING
  // This must be <= maxWidth / maxScale (container width)
  const containerWidth = Math.floor(maxWidth / maxScale);
...
export const DataCallout: React.FC<Props> = ({ callout }) => {
  ...
  if (callout.instant) {
    ...
    scale = startScale + (endScale - startScale) * slamProgress;
    opacity = Math.min(1, slamProgress * 2);
  }
  ...
  const { fontSize, containerWidth } = fitText(callout.text, 110, maxScale, layout.calloutMaxWidth);

  return (
    <div
      style={{
        position: "absolute",
        top: layout.calloutTop + (callout.yOffset ?? 0),
        left: 0,
        right: 0,
        display: "flex",
