...
// Per-emotion sizing — mapped to existing chibi PNGs (short-01 metrics).
// Sizes match the actual PNG the emotion resolves to via emotionToFile.
const CHIBI_METRICS: Record<ChibiEmotion, { size: number; bottomPad: number }> = {
  content:     { size: 875,  bottomPad: 0 },    // → thumbsup
  impressed:   { size: 890,  bottomPad: 0 },    // → idea
  envious:     { size: 1060, bottomPad: 91 },   // → confused
  determined:  { size: 1055, bottomPad: 89 },   // → confident
  confident:   { size: 1055, bottomPad: 89 },   // → confident
  curious:     { size: 915,  bottomPad: 0 },    // → thinking
  smug:        { size: 1050, bottomPad: 143 },  // → shrug
  proud:       { size: 1060, bottomPad: 103 },  // → proud
  mesmerized:  { size: 890,  bottomPad: 0 },    // → idea
  excited:     { size: 900,  bottomPad: 0 },    // → excited
  victorious:  { size: 900,  bottomPad: 0 },    // → excited
  shocked:     { size: 866,  bottomPad: 0 },    // → panic
  defeated:    { size: 1050, bottomPad: 120 },  // → tired
  manic:       { size: 900,  bottomPad: 0 },    // → excited
  frustrated:  { size: 1060, bottomPad: 91 },   // → confused
  exasperated: { size: 1050, bottomPad: 120 },  // → tired
  scheming:    { size: 915,  bottomPad: 0 },    // → scheming
  hopeful:     { size: 890,  bottomPad: 0 },    // → idea
  crushed:     { size: 887,  bottomPad: 0 },    // → scared
  resigned:    { size: 1050, bottomPad: 120 },  // → tired
  wise:        { size: 1047, bottomPad: 90 },   // → teaching
};
...
export const VoiceSyncChibi: React.FC<Props> = ({
  ...
  const chibiSize = metrics.size;
  const bottomPadY = metrics.bottomPad;
  ...
  if (postEntrance && !isExiting) {
    let activeWord: Caption | null = null;
    let wordProgress = 0;
    let intensity = 1;

    for (const word of captions) {
      const wordStart = msToFrame(word.startMs, fps);
      const wordEnd = msToFrame(word.endMs, fps);
      if (globalFrame >= wordStart && globalFrame <= wordEnd) {
        activeWord = word;
        const wordDuration = wordEnd - wordStart;
        wordProgress = wordDuration > 0
          ? (globalFrame - wordStart) / wordDuration
          : 0;
        const wordDurationMs = word.endMs - word.startMs;
        intensity = Math.min(1.5, Math.max(0.5, wordDurationMs / 300));
        break;
      }
    }

    if (activeWord) {
      const pulse = Math.sin(wordProgress * Math.PI);
      ...
      switch (animation) {
        ...
        case "snap": {
          const s = spring({
            frame: Math.round(wordProgress * 15),
            fps,
            config: { damping: 8, stiffness: 300, mass: 0.5 },
            durationInFrames: 15,
          });
          animScaleX = interpolate(s, [0, 0.3, 1], [0.95, 1 + 0.08 * intensity, 1]);
          animScaleY = interpolate(s, [0, 0.3, 1], [1 + 0.04 * intensity, 1 - 0.07 * intensity, 1]);
          break;
        }
        case "punch": {
        ...
        case "tilt": {
          const dir = activeWord.startMs % 2 === 0 ? 1 : -1;
          animRotation = pulse * 5 * intensity * dir;
          animTranslateX = pulse * 6 * intensity * dir;
          break;
        }
        case "shake": {
          const shakeIntensity = pulse * intensity;
          animTranslateX = noise2D("sx", globalFrame * 0.5, 0) * 10 * shakeIntensity;
          animTranslateY = noise2D("sy", 0, globalFrame * 0.5) * 6 * shakeIntensity;
          break;
        }
        case "dim": {
        ...
        case "idle": {
          ...
          animRotation = pulse * 1.8 * intensity;
          animTranslateX = pulse * 4 * intensity * (activeWord.startMs % 2 === 0 ? 1 : -1);
          break;
        }
        case "wobble": {
        ...
        case "heartbeat": {
          const p1 = wordProgress < 0.5
            ? Math.sin((wordProgress / 0.5) * Math.PI)
            : 0;
          const p2 = wordProgress >= 0.4 && wordProgress <= 0.9
            ? Math.sin(((wordProgress - 0.4) / 0.5) * Math.PI) * 0.6
            : 0;
          const s = 1 + p1 * 0.07 * intensity + p2 * 0.05 * intensity;
          animScaleX = s;
          animScaleY = s;
          break;
        }
        case "drift": {
          const dir = activeWord.startMs % 2 === 0 ? 1 : -1;
          animTranslateX = pulse * 18 * intensity * dir;
          animRotation = pulse * -2 * intensity * dir;
          break;
        }
        case "blink": {
          if (wordProgress < 0.3) {
            const bt = wordProgress / 0.3;
