...
function generateCrackLines(count: number, seed: number): CrackLine[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const points: CrackPoint[] = [{ x: IMPACT_X, y: IMPACT_Y }];
    let angle = baseAngle;
...
function generateFragments(count: number, seed: number): Fragment[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const cx = IMPACT_X + Math.cos(ringAngle) * ringDist;
    const cy = IMPACT_Y + Math.sin(ringAngle) * ringDist;
...
export const ScreenBreak: React.FC<ScreenBreakProps> = ({
  ...
  const cracks = useMemo(() => generateCrackLines(10, 42), []);
  const fragments = useMemo(() => generateFragments(24, 77), []);
