...
## 2026-06-10 — a 35-second network blip wrote empty reports for 16 niches; both runs exited 0
- **Symptom:** the deep monthly + daily reprocess "completed" rc=0, but every niche after ~10:50:32 UTC stored `tweets: 0, native_x_articles: 0` and overwrote the day's reports with empty rankings. Ledger showed 514 consecutive status-408 rows ~50ms apart — far too fast for HTTP timeouts.
- **Cause chain:** `twapi._get()` converts ANY network `OSError` (Wi-Fi drop → instant "network unreachable") into a synthetic 408 with an empty body; `metered_call` recorded it as a $0/0-result page; `metered_search` treats an empty page as end-of-results and breaks cleanly; the niche then writes an "empty" report and the runner moves on. A ~1-minute local outage let two concurrent runs blast through 16 remaining niches in 35 seconds, all "successful".
