...
use crate::helpers::{load_tracked_symbols, spawn_resilient, shutdown_signal};

pub(crate) async fn run_serve(args: config::ServeArgs) -> Result<(), Box<dyn std::error::Error>> {
    // Init tracing
    ...
    crate::db::run_migrations_with_seed(&pool, "data-node/migrations").await?;
    ...
    if args.reset_session {
        ...
        crate::db::reset_collector_cursors(&pool)
            .await
    ...
    let sim_cache_inner = crate::simulation::SimDataCache::load(&pool).await
        .unwrap_or_else(|e| {
            tracing::warn!(error = %e, "Failed to load sim data cache, simulations will fail");
            std::sync::Arc::new(crate::simulation::SimDataCache {
                all_dates: vec![],
    ...
    let symbol_map = crate::api::load_symbol_map(&args.symbol_map)?;
    ...
    let chain_cache_early = Arc::new(crate::chain_cache::ChainCache::new());
    ...
    tokio::spawn(async move {
        crate::kline_collector::run(kline_pool, kline_symbol_map).await;
    });
    ...
    // Start liquidity collector in background (all symbols from symbol-map)
    {
        ...
        tokio::spawn(async move {
            crate::liquidity_collector::run(liq_pool, all_symbols, liq_poll_interval, liq_retention).await;
        });
    ...
    let cg_limiter = {
        let is_pro = args.coingecko_api_key.as_deref()
            ...
            .unwrap_or(false);
        if is_pro {
            crate::coingecko::RateLimiter::coingecko_pro()
        } else {
            crate::coingecko::RateLimiter::coingecko_demo()
        }
    };
    ...
    let logos_dir = std::path::PathBuf::from(&args.logos_dir);
    {
        ...
        let cg_state = Arc::new(crate::cg_collector::CgCollectorState::new());
        ...
        tokio::spawn(async move {
            crate::cg_collector::run(cg_pool, cg_state, cg_key, cg_poll, cg_logos_dir, cg_lim).await;
        });
    ...
    if args.listing_sync_interval > 0 {
        ...
        tokio::spawn(async move {
            crate::listing_sync::run_daily(listing_pool, listing_interval).await;
        });
    ...
    if args.dl_poll_interval > 0 {
        ...
        tokio::spawn(async move {
            ...
            crate::dl_collector::run_once(&dl_pool).await;
            info!("DefiLlama first sync done, reloading sim cache...");
            match crate::simulation::SimDataCache::load(&dl_reload_pool).await {
                Ok(new_cache) => {
            ...
            crate::dl_collector::run(dl_pool, dl_poll).await;
        });
    ...
    if args.fng_poll_interval > 0 {
        ...
        tokio::spawn(async move {
            crate::fng_collector::run(fng_pool, fng_poll).await;
        });
    ...
    // backtester chart stays current.
    {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                match crate::simulation::SimDataCache::load(&reload_pool).await {
                    Ok(new_cache) => {
    ...
    crate::market_data::error_tracker::init_global();
    crate::market_data::sync_registry::init_global();
    ...
    if let Some(ref key) = args.finnhub_api_key {
        ...
        spawn_resilient("finnhub", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::finnhub::FinnhubClient::from_env() {
                    Ok(source) => {
    ...
    if let Some(ref key) = args.fred_api_key {
        ...
        spawn_resilient("fred", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::fred::FredMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::ScheduledSyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
                    Err(e) => {
    ...
    if let Some(ref key) = args.bls_api_key {
        ...
        spawn_resilient("bls", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::bls::BlsMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::ScheduledSyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    if let Some(key) = args.treasury_api_key.as_ref().filter(|k| !k.trim().is_empty()) {
        ...
        spawn_resilient("treasury", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::treasury::TreasuryMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::ScheduledSyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    // 5. ECB (euro rates) — no API key, always on
    {
        ...
        spawn_resilient("ecb", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::ecb::EcbMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::ScheduledSyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    // 5b. BoE (FX rates) — no API key, always on
    {
        ...
        spawn_resilient("boe", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::boe::BoeMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::ScheduledSyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    if let Some(ref key) = args.eia_api_key {
        ...
        spawn_resilient("eia", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::eia::EiaMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::ScheduledSyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    if let Some(key) = nasdaq_key_valid {
        ...
        // CFTC
        {
            ...
            spawn_resilient("cftc", pw.clone(), move || {
                ...
                let pw = pw.clone();
                async move {
                    match crate::market_data::sources::nasdaq::CftcMarketSource::from_env() {
                        Ok(source) => {
                            let engine = crate::market_data::ScheduledSyncEngine::new(pool_c, Box::new(source), bh, pw);
                            engine.run().await;
                        }
        ...
        // CHRIS (continuous futures)
        {
            ...
            spawn_resilient("chris", pw.clone(), move || {
                ...
                let pw = pw.clone();
                async move {
                    match crate::market_data::sources::nasdaq::ChrisMarketSource::from_env() {
                        Ok(source) => {
                            let engine = crate::market_data::ScheduledSyncEngine::new(pool_c, Box::new(source), bh, pw);
                            engine.run().await;
                        }
        ...
        // OPEC
        {
            ...
            spawn_resilient("opec", pw.clone(), move || {
                ...
                let pw = pw.clone();
                async move {
                    match crate::market_data::sources::nasdaq::OpecMarketSource::from_env() {
                        Ok(source) => {
                            let engine = crate::market_data::ScheduledSyncEngine::new(pool_c, Box::new(source), bh, pw);
                            engine.run().await;
                        }
        ...
        // IMF
        {
            ...
            spawn_resilient("imf", pw.clone(), move || {
                ...
                let pw = pw.clone();
                async move {
                    match crate::market_data::sources::nasdaq::ImfMarketSource::from_env() {
                        Ok(source) => {
                            let engine = crate::market_data::ScheduledSyncEngine::new(pool_c, Box::new(source), bh, pw);
                            engine.run().await;
                        }
    ...
    // Previously gated behind nasdaq_api_key by mistake.
    {
        ...
        spawn_resilient("bchain", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::nasdaq::BchainMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    if args.openmeteo_sync_interval > 0 {
        ...
        spawn_resilient("openmeteo", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::openmeteo::OpenMeteoMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    // SEC EDGAR 13F Filings (no auth, User-Agent only)
    {
        ...
        spawn_resilient("sec_edgar", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::sec_edgar::SecEdgarMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::ScheduledSyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    // SEC EFTS Filing Counts (no auth required)
    {
        ...
        spawn_resilient("sec_efts", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::sec_efts::SecEftsMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::ScheduledSyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    // SEC Form 4 Insider Trading (no auth required)
    {
        ...
        spawn_resilient("sec_insider", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::sec_insider::SecInsiderMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::ScheduledSyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    if let Some(ref id) = args.finra_client_id {
        if let Some(ref secret) = args.finra_client_secret {
            ...
            spawn_resilient("finra", pw.clone(), move || {
                ...
                let pw = pw.clone();
                async move {
                    match crate::market_data::sources::finra::FinraMarketSource::from_env() {
                        Ok(source) => {
                            let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                            engine.run().await;
                        }
    ...
    // FINRA Daily Short Volume (public endpoint, no API key needed)
    {
        ...
        spawn_resilient("finra_short_vol", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::finra_short_vol::FinraShortVolMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    // Congress (requires CONGRESS_API_KEY — skip silently if not set)
    {
        ...
        spawn_resilient("congress", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::congress::CongressMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::ScheduledSyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    // npm package downloads
    {
        ...
        spawn_resilient("npm", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::npm::NpmMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    // PyPI package downloads
    {
        ...
        spawn_resilient("pypi", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::pypi::PypiMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    // crates.io Rust package downloads
    {
        ...
        spawn_resilient("crates_io", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::crates_io::CratesIoMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    // Steam concurrent player counts
    {
        ...
        spawn_resilient("steam", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::steam::SteamMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    // Hacker News story scores
    {
        ...
        spawn_resilient("hackernews", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::hackernews::HackerNewsMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    // 4chan board activity
    {
        ...
        spawn_resilient("fourchan", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::fourchan::FourchanMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    // AniList anime/manga popularity
    {
        ...
        spawn_resilient("anilist", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::anilist::AniListMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    // TWSE (Taiwan Stock Exchange)
    {
        ...
        spawn_resilient("twse", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::twse::TwseMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    // Polymarket prediction markets
    {
        ...
        spawn_resilient("polymarket", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::polymarket::PolymarketMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    // DefiLlama (chain TVL, protocol TVL, DEX volumes)
    {
        ...
        spawn_resilient("defillama_market", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::defillama::DefiLlamaMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    // Zillow — free public CSV data, no API key needed
    {
        ...
        spawn_resilient("zillow", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::zillow::ZillowMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    if let Some(ref client_id) = args.twitch_client_id {
        if let Some(ref client_secret) = args.twitch_client_secret {
            ...
            spawn_resilient("twitch", pw.clone(), move || {
                ...
                let pw = pw.clone();
                async move {
                    match crate::market_data::sources::twitch::TwitchMarketSource::from_env() {
                        Ok(source) => {
                            let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                            engine.run().await;
                        }
    ...
    if let Some(ref key) = args.tmdb_api_key {
        ...
        spawn_resilient("tmdb", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::tmdb::TmdbMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    if let Some(ref key) = args.lastfm_api_key {
        ...
        spawn_resilient("lastfm", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::lastfm::LastfmMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    if let Some(ref key) = args.backpacktf_api_key {
        ...
        spawn_resilient("backpacktf", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::backpacktf::BackpackTfMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    // GitHub — works with or without token (unauthenticated = lower rate limits)
    {
        ...
        spawn_resilient("github", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::github::GithubMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    if let Some(ref token) = args.cloudflare_radar_token {
        ...
        spawn_resilient("cloudflare", pw.clone(), move || {
            ...
            let pw = pw.clone();
            async move {
                match crate::market_data::sources::cloudflare::CloudflareRadarMarketSource::from_env() {
                    Ok(source) => {
                        let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    // Works with Pro key, Demo key (CG- prefix), or no key (free tier fallback)
    {
        ...
        spawn_resilient("coingecko", pw.clone(), move || {
            ...
            async move {
                match crate::market_data::sources::coingecko::CoinGeckoMarketSource::from_env(cg_lim) {
                    Ok(source) => {
                        let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                        engine.run().await;
                    }
    ...
    // Volcano — USGS (no key)
    {
        ...
        spawn_resilient("volcano", pw.clone(), move || {
            ...
            async move {
                match crate::market_data::sources::volcano::VolcanoMarketSource::from_env() {
    ...
    // Earthquake — USGS (no key)
    {
        ...
        spawn_resilient("earthquake", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::earthquake::EarthquakeMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // Space Weather — NOAA (no key)
    {
        ...
        spawn_resilient("spaceweather", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::spaceweather::SpaceweatherMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    if let Some(ref key) = args.nasa_firms_key {
        ...
        spawn_resilient("wildfire", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::wildfire::WildfireMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // Flights — adsb.lol (no key, single-call strategy)
    {
        ...
        spawn_resilient("flights", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::flights::FlightsMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // Military Aircraft — adsb.lol (no key, community ADS-B API)
    {
        ...
        spawn_resilient("mil_aircraft", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::mil_aircraft::MilAircraftMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // Epidemic — disease.sh (no key)
    {
        ...
        spawn_resilient("epidemic", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::epidemic::EpidemicMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // Sports — ESPN (no key)
    {
        ...
        spawn_resilient("sports", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::sports::SportsMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // ISS Position — Open Notify (no key)
    {
        ...
        spawn_resilient("iss", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::iss::IssMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // Weather Alerts — NWS (no key, User-Agent only)
    {
        ...
        spawn_resilient("weather_alerts", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::weather_alerts::WeatherAlertsMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // Animals — GBIF + iNaturalist (no key)
    {
        ...
        spawn_resilient("animals", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::animals::AnimalsMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // GTFS-RT Transit (NYC MTA Subway, BART — no API key needed)
    {
        ...
        spawn_resilient("gtfs_rt", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                let source = crate::market_data::sources::gtfs_rt::GtfsRtMarketSource::new();
                let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw);
                engine.run().await;
            }
        });
    ...
    // USASpending.gov — US federal defense spending (no key needed)
    {
        ...
        spawn_resilient("usa_spending", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::usa_spending::UsaSpendingMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    if std::env::var("CHATURBATE_WM").is_ok() {
        ...
        spawn_resilient("chaturbate", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::chaturbate::ChaturbateMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // PandaScore Esports — always-on (token hardcoded in source, overridable via env)
    {
        ...
        spawn_resilient("pandascore", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::pandascore::PandascoreMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // USGS Water — no key needed
    {
        ...
        spawn_resilient("usgs_water", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::usgs_water::UsgsWaterMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // NOAA Tides — no key needed
    {
        ...
        spawn_resilient("noaa_tides", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::noaa_tides::NoaaTidesMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // CityBikes — no key needed
    {
        ...
        spawn_resilient("citybikes", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::citybikes::CityBikesMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // NDBC Ocean Buoys — no key needed
    {
        ...
        spawn_resilient("ndbc", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::ndbc::NdbcMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // NOAA Ocean Meteorology — no key needed
    {
        ...
        spawn_resilient("noaa_met", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::noaa_met::NoaaMetMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // NWPS River Gauges — no key needed
    {
        ...
        spawn_resilient("nwps", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::nwps::NwpsMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    if std::env::var("AIRNOW_API_KEY").is_ok() {
        ...
        spawn_resilient("airnow", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::airnow::AirnowMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // CourtListener — always-on (public API, works without auth at lower rate limits)
    {
        ...
        spawn_resilient("courtlistener", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::courtlistener::CourtListenerMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // OpenAlex Scholarly Works — no key needed (email optional via OPENALEX_EMAIL env)
    {
        ...
        spawn_resilient("openalex", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::openalex::OpenAlexMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // Crossref DOI Registry — no key needed (email optional via CROSSREF_EMAIL env)
    {
        ...
        spawn_resilient("crossref", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::crossref::CrossrefMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // PubMed Biomedical Research — no key needed (API key optional via NCBI_API_KEY env)
    {
        ...
        spawn_resilient("pubmed", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::pubmed::PubMedMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // Queue-Times — theme park wait times (no key)
    {
        ...
        spawn_resilient("queue_times", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::queue_times::QueueTimesMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // ParkAPI Parking Garages — no key needed
    {
        ...
        spawn_resilient("parking", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::parking::ParkingMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    if std::env::var("TOMTOM_API_KEY").is_ok() {
        ...
        spawn_resilient("tomtom_traffic", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::tomtom_traffic::TomtomTrafficMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    if std::env::var("TOMTOM_API_KEY").is_ok() {
        ...
        spawn_resilient("tomtom_evcharge", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::tomtom_evcharge::TomtomEvchargeMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    if let Some(ref key) = args.bestbuy_api_key {
        ...
        spawn_resilient("bestbuy", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::bestbuy::BestBuyMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    if let Some(ref app_id) = args.adzuna_app_id {
        if let Some(ref app_key) = args.adzuna_app_key {
            ...
            spawn_resilient("adzuna", pw.clone(), move || {
                let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
                async move {
                    match crate::market_data::sources::adzuna::AdzunaMarketSource::from_env() {
                        Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // FAA Airport Delays — no key needed (US government API)
    {
        ...
        spawn_resilient("faa_delays", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::faa_delays::FaaDelaysMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // Yahoo Finance Drink Markets — no key needed (unofficial API)
    {
        ...
        spawn_resilient("yahoo_drinks", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::yahoo_drinks::YahooDrinksMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // Deutsche Bahn Train Delays — no key needed (free public API)
    {
        ...
        spawn_resilient("db_trains", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::db_trains::DbTrainsMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // NYC 311 Complaints — no key needed (free Socrata Open Data API)
    {
        ...
        spawn_resilient("nyc311", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::nyc311::Nyc311MarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // McBroken Ice Cream — no key needed (public API)
    {
        ...
        spawn_resilient("mcbroken", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::mcbroken::McBrokenMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // TfL London Tube Status — no key needed (free public API)
    {
        ...
        spawn_resilient("tfl_tube", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::tfl_tube::TflTubeMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // NYC MTA Subway Alerts — no key needed (free GTFS-RT feed)
    {
        ...
        spawn_resilient("mta_subway", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::mta_subway::MtaSubwayMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // Ryanair Flight Delays — no key needed (free OpenSky + Ryanair schedule APIs)
    {
        ...
        spawn_resilient("ryanair", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::ryanair::RyanairMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // US Power Outages — no key needed (free PowerOutage.us/ODIN API)
    {
        ...
        spawn_resilient("power_outages", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::power_outages::PowerOutagesMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // World Bank — public API, no key needed (ScheduledSyncEngine for daily fetch)
    {
        ...
        spawn_resilient("worldbank", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::worldbank::WorldBankMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    // Lichess Chess — no key needed (public API)
    {
        ...
        spawn_resilient("lichess", pw.clone(), move || {
            let pool_c = pool_c.clone(); let bh = bh.clone(); let pw = pw.clone();
            async move {
                match crate::market_data::sources::lichess::LichessMarketSource::from_env() {
                    Ok(source) => { let engine = crate::market_data::SyncEngine::new(pool_c, Box::new(source), bh, pw); engine.run().await; }
    ...
    {
        let tracker = crate::market_data::error_tracker::global();
        // Finnhub
    ...
    if let Some(ref urls_str) = args.oracle_health_urls {
        ...
        if !urls.is_empty() {
            crate::oracle_health_collector::validate_oracle_urls(&urls);
            let node_count = urls.len();
            let pool_clone = pool.clone();
            let interval = args.oracle_health_poll_interval;
            tokio::spawn(async move {
                crate::oracle_health_collector::run_oracle_health_collector(pool_clone, urls, interval).await;
            });
    ...
    let live_cache = Arc::new(LiveTickerCache::new());
    {
        ...
        tokio::spawn(async move {
            ...
            crate::live_cache::run_fast_poller(fast_cache, tracked, fast_interval).await;
        });
    ...
    let source_registry = crate::source_registry::SourceRegistry::load(&args.sources_display_file)
        .unwrap_or_else(|e| {
            tracing::warn!(
            ...
            crate::source_registry::SourceRegistry { sources: vec![], categories: vec![] }
        });
    ...
    crate::api::spawn_health_stats_refresh(pool.clone(), Arc::clone(&health_stats_cache), interval_map);

    // Start batch engine (sources from registry)
    let batch_state = Arc::new(crate::batch_engine::BatchEngineState::new());
    {
    ...
    let app_state = Arc::new(AppState {
        ...
        cache: crate::api::PriceCache::new(5), // 5-second TTL
        live_cache,
        l3_provider,
        settlement_provider,
        deployment,
    ...
    macro_rules! spawn_poller {
        ($name:expr, $secs:expr, $fn:path) => {{
            ...
            tokio::spawn(crate::collector_loop::run_collector_loop($name, Duration::from_secs($secs), move || {
                let s = Arc::clone(&s);
                async move { $fn(&s).await }
            }));
        }};
    }
    spawn_poller!("nav",         10, crate::chain_pollers::poll_nav_once);
    spawn_poller!("oracle",       2, crate::chain_pollers::poll_oracle_once);
    spawn_poller!("balances",     1, crate::chain_pollers::poll_user_balances_once);
    spawn_poller!("allowances",   3, crate::chain_pollers::poll_user_allowances_once);
    spawn_poller!("orders",       1, crate::chain_pollers::poll_user_orders_once);
    spawn_poller!("positions",    3, crate::chain_pollers::poll_user_positions_once);
    spawn_poller!("cost_basis",   5, crate::chain_pollers::poll_user_cost_basis_once);
    // Backend chain state pollers (for oracle/AP HTTP endpoints)
    spawn_poller!("pending_orders",     1, crate::chain_pollers::poll_pending_orders_once);
    spawn_poller!("batched_orders",     2, crate::chain_pollers::poll_batched_orders_once);
    spawn_poller!("oracle_registry",   10, crate::chain_pollers::poll_oracle_registry_once);
    spawn_poller!("cycle_metadata",     2, crate::chain_pollers::poll_cycle_metadata_once);
    spawn_poller!("registry_metadata", 10, crate::chain_pollers::poll_registry_metadata_once);
    spawn_poller!("settlement_state",   2, crate::chain_pollers::poll_settlement_state_once);
    spawn_poller!("pending_rebalances", 5, crate::chain_pollers::poll_pending_rebalances_once);
    spawn_poller!("system_snapshot",    5, crate::chain_pollers::poll_system_snapshot_once);
    spawn_poller!("aum_ranking",       10, crate::chain_pollers::poll_aum_ranking_once);
    ...
    spawn_poller!("batch_markets_reload", 60, crate::chain_pollers::poll_batch_markets_reload);
