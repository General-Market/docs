...
fn stale_reason_for(source_id: &str) -> &'static str {
    match source_id {
        ...
        "binance_spot" => "Spot pairs with no trades in the last sync window",
        "binance_futures_funding" => "Funding rates publish on 8h cycles — long quiet windows are normal",
        "binance_options" => "Option contracts near expiry or far from the money trade thin",
...
async fn sse_stream(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            if topics.contains(&"morpho-markets".to_string()) {
                let gen = cache.morpho_markets_gen.get();
