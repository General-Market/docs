...
impl UsgsWaterMarketSource {
    ...
    async fn fetch_state(&self, state: &str) -> Vec<StationReading> {
        ...
        // Sort by discharge descending, take top N
        readings.sort_by(|a, b| b.discharge.cmp(&a.discharge));
        readings.truncate(MAX_STATIONS_PER_STATE);

        debug!(
            "State {}: {} stations with valid discharge readings (capped at {})",
            state,
            readings.len(),
            MAX_STATIONS_PER_STATE
        );
...
impl MarketDataSource for UsgsWaterMarketSource {
    ...
    async fn fetch_prices(&self, asset_ids: &[String]) -> Result<Vec<PriceUpdate>> {
        ...
        // Build a set of requested site numbers for fast lookup
        let requested: HashSet<&str> = asset_ids
            .iter()
        ...
        for (i, state) in STATES.iter().enumerate() {
            ...
            for reading in &readings {
                ...
                // Only include stations that were requested
                if !requested.contains(reading.site_no.as_str()) {
                    continue;
                }
        ...
        info!(
            "Fetched {}/{} prices from USGS Water across {} states",
            results.len(),
            asset_ids.len(),
            STATES.len()
        );
    ...
    async fn discover_upstream_assets(&self) -> Result<Vec<AssetEntry>> {
        ...
        for (i, state) in STATES.iter().enumerate() {
            ...
            for reading in &readings {
                // Skip duplicate sites (a station might appear in adjacent state queries)
                if !seen_sites.insert(reading.site_no.clone()) {
                    continue;
                }

                all_entries.push(AssetEntry {
                    asset_id: format!("usgs_water_{}", reading.site_no),
                    symbol: format!("WATER/{}", reading.site_no),
                    name: reading.site_name.clone(),
