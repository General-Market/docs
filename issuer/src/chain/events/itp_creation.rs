...
pub use common::types::{ItpCreatedEvent, ItpCreationRequest};
...
/// - weights (uint256[])
/// - assets (address[])
pub fn parse_create_itp_requested(log: &Log) -> Result<ItpCreationRequest, ParseError> {
...
pub fn validate_itp_creation_request(request: &ItpCreationRequest) -> Result<(), ValidationError> {
    // Check lengths match
    if request.weights.len() != request.assets.len() {
        return Err(ValidationError::LengthMismatch {
            weights_len: request.weights.len(),
            assets_len: request.assets.len(),
        });
    }
    ...
    // Check max assets
    if request.weights.len() > MAX_ASSETS {
        return Err(ValidationError::TooManyAssets {
            count: request.weights.len(),
            max: MAX_ASSETS,
        });
    }

    // Check weights sum to 1e18
    let sum: U256 = request.weights.iter().fold(U256::zero(), |acc, w| acc + *w);
    ...
    for (i, weight) in request.weights.iter().enumerate() {
    ...
    for (i, asset) in request.assets.iter().enumerate() {
        if asset.is_zero() {
    ...
    // Check symbol length
    if request.symbol.len() > MAX_SYMBOL_LENGTH {
        return Err(ValidationError::SymbolTooLong {
            length: request.symbol.len(),
...
/// Data:
/// - admin (address)
pub fn parse_itp_created(log: &Log) -> Result<ItpCreatedEvent, ParseError> {
    if log.topics.len() < 4 {
        return Err(ParseError::MissingTopic {
            index: log.topics.len(),
        });
...
mod tests {
    ...
    fn test_validate_success() {
        ...
        assert!(validate_itp_creation_request(&request).is_ok());
    }
    ...
    fn test_validate_length_mismatch() {
        ...
        let result = validate_itp_creation_request(&request);
        assert!(matches!(
            result,
