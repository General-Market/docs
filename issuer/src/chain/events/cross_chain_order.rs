...
/// - Full order params from getCrossChainOrder() call (limitPrice, slippageTier, deadline, createdAt)
...
pub struct CrossChainOrder {
    ...
    /// Note: This field is fetched via getCrossChainOrder(), not from the event
...
/// Partial order data from event only (before fetching full params)
///
...
impl CrossChainSellOrderEvent {
    ...
    pub fn from_log(log: &Log) -> Result<Self, CrossChainOrderParseError> {
...
impl CrossChainOrderEvent {
    ...
    /// Convert to full CrossChainOrder by adding data from getCrossChainOrder() call
    pub fn into_full_order(
        self,
        limit_price: U256,
        slippage_tier: u8,
        deadline: U256,
...
impl CrossChainOrder {
    ...
    ///
    /// Note: Prefer `is_expired_at()` when a block timestamp is available,
    ...
    pub fn is_expired_at(&self, timestamp: u64) -> bool {
        ...
        if let Some(deadline_secs) = self.deadline.try_into().ok() {
    ...
    ///
    /// Returns None if valid, or Some(reason) if invalid
    pub fn validate_at(&self, timestamp: u64) -> Option<&'static str> {
        if self.user.is_zero() {
            return Some("user address is zero");
        }
        if self.amount.is_zero() {
    ...
    pub fn dedup_key(&self) -> (u64, U256) {
        (self.chain_id, self.order_id)
    }
    ...
    pub fn has_suspicious_amount(&self) -> bool {
        self.amount < U256::from(SUSPICIOUS_AMOUNT_THRESHOLD)
    }

    /// Log a warning if the amount looks suspiciously small
    ///
    /// Story 7-6b: Detect potential 6-decimal vs 18-decimal confusion.
    pub fn log_if_suspicious_amount(&self) {
        if self.has_suspicious_amount() {
            warn!(
                order_id = %self.order_id,
                amount = %self.amount,
...
mod tests {
    ...
    fn test_is_expired_at() {
        ...
        assert!(order.is_expired_at(2000));
    ...
    fn test_validate_at() {
        ...
        assert_eq!(order.validate_at(2000), Some("order has expired"));
        // Not expired at timestamp 500
        assert_eq!(order.validate_at(500), None);
    }
    ...
    fn test_validate_zero_user() {
        ...
        assert_eq!(order.validate(), Some("user address is zero"));
    }
    ...
    fn test_validate_zero_amount() {
        ...
        assert_eq!(order.validate(), Some("amount is zero"));
    }
    ...
    fn test_suspicious_amount_detection_6_decimal() {
        ...
        assert!(order.has_suspicious_amount());
    }
