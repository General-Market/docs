...
## Vision Testnet Integration: Prediction Market as Signal Source

Vision runs on Index L3, an Arbitrum Orbit chain (chainId `111222333`). Batch markets on Vision settle Twitch-related questions every 2, 5, or 10 minutes — depending on the source configuration. Key differences from off-chain analytics:

| Parameter | External analytics (StreamCharts) | Vision testnet |
|---|---|---|
| Pricing mechanism | Editorial forecast + regression | Parimutuel batch auction, L3 settled |
| Margin | Nonexistent (free, ad-supported) | ~0% (pool rebalances) |
| Participants | Passive readers | Bots and algorithmic traders |
| Reaction to news | Hours (editorial lag) | Seconds (on-chain) |
| Signal | Static forecast | Live aggregated belief |

When Vision testnet prices diverge from external analytics — that's a potential edge. On Twitch this gap is especially rich because streamer state changes fast (a raid, a shock announcement, a host migration), and on-chain bots update within a single block; editorial forecasts won't.

### Connecting to Vision Testnet via web3.py
...
# Addresses come from `envs/testnet/deployment.json` — canonical source.
INDEX_ADDRESS = os.getenv("INDEX_ADDRESS")
VISION_RESERVE_ADDRESS = os.getenv("VISION_RESERVE_ADDRESS")


class VisionTestnetClient:
    """
    Client for reading Twitch batch market state from Vision testnet.
    No private keys required for read-only access.
    """

    def __init__(self, rpc_url: str = VISION_RPC):
        self.w3 = Web3(Web3.HTTPProvider(rpc_url))
        if not self.w3.is_connected():
            raise RuntimeError(f"Cannot reach Vision RPC at {rpc_url}")

        with open("abi/VisionReserve.json") as f:
            abi = json.load(f)
        self.reserve = self.w3.eth.contract(
            address=Web3.to_checksum_address(VISION_RESERVE_ADDRESS),
            abi=abi,
        )

    def list_active_markets(self, source_prefix: str = "twitch") -> list[dict]:
        """List active Twitch batch markets."""
        raw = self.reserve.functions.listActiveBatches().call()
        markets = []
        for batch_id, name, yes_pool, no_pool, settles_at in raw:
            if name.startswith(source_prefix):
                ...
                markets.append({
                    ...
                    "no_pool": int(no_pool),
                    "yes_implied": (
                        float(yes_pool) / float(total) if total else 0.5
                    ),
                    "no_implied": (
                        float(no_pool) / float(total) if total else 0.5
                    ),
                    "settles_at": int(settles_at),
                })
        return markets

    def get_market_price(self, batch_id: int) -> dict:
        """
        Return implied YES/NO probability from the on-chain parimutuel pool.
        L3 USDC uses 18 decimals (per project docs — do not assume 6).
        """
        yes_pool, no_pool, settles_at = self.reserve.functions.getBatchState(
            batch_id,
        ).call()
        total = yes_pool + no_pool
        if total == 0:
            return {"yes": 0.5, "no": 0.5, "liquidity_usdc": 0.0}
        return {
            "yes": float(yes_pool) / float(total),
            "no": float(no_pool) / float(total),
            "liquidity_usdc": float(total) / 1e18,  # 18 dec on L3
            "settles_at": int(settles_at),
        }


# === Usage ===
vision = VisionTestnetClient()
active = vision.list_active_markets(source_prefix="twitch")
for m in active[:5]:
    print(f"  📊 {m['name']}")
    print(f"     YES: {m['yes_implied']:.1%} | NO: {m['no_implied']:.1%}")
    print(f"     Pool: ${(m['yes_pool'] + m['no_pool']) / 1e18:,.0f} USDC "
          f"(L3, 18 dec)")
```

### Retrieving Historical Vision Prices

```python
class VisionHistorical:
    """
    Pull historical batch resolution prices from the data-node
    oracle API. data-node exposes per-batch history at
    /api/vision/batches/{source}/history.
    """

    DATA_NODE = os.getenv("DATA_NODE_URL", "https://api.generalmarket.io")

    def get_price_history(
        self, source: str, batch_id: int,
        lookback_hours: int = 48,
    ) -> pd.DataFrame:
        try:
            r = requests.get(
                f"{self.DATA_NODE}/api/vision/batches/{source}/history",
                params={
                    "batch_id": batch_id,
    ...
    def get_pool_depth(self, batch_id: int) -> dict:
        """
        Pool depth — parimutuel liquidity on each side.
        High depth → more reliable crowd signal.
        """
        try:
            r = requests.get(
                f"{self.DATA_NODE}/api/vision/batches/{batch_id}/depth",
                timeout=15,
            )
            r.raise_for_status()
            book = r.json()

            yes_usdc = float(book.get("yes_pool", 0)) / 1e18
            no_usdc = float(book.get("no_pool", 0)) / 1e18
            ...
            return {
                ...
                "imbalance": (
                    (yes_usdc - no_usdc) / total if total else 0
                ),
                "spread_pct": 0.0,  # parimutuel has no spread
            }
