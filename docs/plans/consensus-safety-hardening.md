...
**Design invariant**: Oracles are stateless. Every piece of in-memory state MUST be reconstructable from on-chain data via a small number of RPC calls. No local disk persistence (except WAL for mid-cycle recovery), no event replay from genesis, no peer gossip required for recovery.
...
| `activeBitmask` | Derived from `getActiveOracleEndpoints()` (Phase -1) or from snapshot (Phase 2+3) | 1 | Rebuilt at boot |
...
| WAL (P2P-6) | Local disk, per-oracle file | 0 | Survives crash |
...
- Alternative checked: file-based flag (`/tmp/oracle-pause`) — rejected because it requires SSH to 20 nodes. On-chain is one tx.
...
**Implementation**:
- At startup, if `num_oracles > 1` and `--registry-sync` is not set, refuse to start with: `"ERROR: --registry-sync required for multi-oracle deployments"`
- Update production startup scripts
- Update `start-oracles.sh` (local dev) to include `--registry-sync`
...
**Problem**: `num_oracles`, `signature_threshold`, `node_index`, and `oracle_registry_index` all come from CLI flags. After oracle additions/removals, CLI values are stale. `node_index = (node_id - 1)` (bootstrap/consensus.rs:68) is wrong after any oracle removal. `LeaderElector::new()` panics if `node_index >= num_oracles`.
...
```
Concrete `EthersChainReader` calls: `activeOracleCount()`, `registryNonce()`, `getAggregatedPubkey()`.
...
let on_chain_active = chain_reader.get_active_oracle_count().await
    .unwrap_or_else(|_| {
        warn!("Failed to query on-chain activeCount, falling back to --num-oracles CLI arg");
        self.params.num_oracles as u64
    });
let threshold = compute_threshold(on_chain_active as usize);  // BFT 67%
```

**Step 3: Derive `oracle_registry_index` and `node_index` from chain** (replaces build_keys lines 68-69):
```rust
// In build_keys():
let (oracle_registry_index, node_index) = match self.derive_indices_from_chain().await {
    Ok(indices) => indices,
    Err(e) => {
        warn!("Failed to derive indices from chain: {e}. Falling back to CLI.");
        let idx = self.params.on_chain_oracle_id.unwrap_or((self.node_id - 1) as u8);
...
async fn derive_indices_from_chain(&self) -> Result<(u8, u8), BootstrapError> {
    ...
    // Dense index = count of active oracles with ID < ours
    let node_index = oracles.iter()
        .filter(|i| i.status == 1 && i.id < my_oracle.id)
        .count() as u8;

    info!(oracle_registry_index, node_index, "Derived indices from on-chain registry");
...
**Optional contract addition**: `getOracleIdByAddress(address) -> (bool found, uint256 oracleId)` for simpler self-discovery. Not blocking — BLS pubkey matching via `getActiveOracleEndpoints()` works.
...
async fn build_key_registry(&self) -> Option<Arc<InMemoryKeyRegistry>> {
    ...
    if let Ok(oracles) = self.chain_reader.get_oracle_registry().await {
        let active_oracles: Vec<_> = oracles.iter()
            .filter(|i| i.status == 1)
            .collect();
...
**IMPORTANT**: `generate_peer_id(oracle.id)` is used consistently here — same as the fixed `RegistrySyncHandler` (Phase 0c expanded).

**PREREQUISITE — `Oracle` struct change**: The current `Oracle` struct (`common/src/types/oracle.rs`) does NOT have an `id` field. It has `addr`, `ip`, `bls_pubkey`, `status`, `registered_at`. Add `pub id: u64` to the struct. Additionally, `get_oracle_registry()` (`oracle/src/chain/reader.rs`) currently calls `getOracles()` and skips inactive oracles, returning a dense list without on-chain IDs. Change it to call `getActiveOracleEndpoints()` instead, which returns `(ids[], ips[], pubkeys[])` — preserving on-chain IDs. Populate `oracle.id` from the `ids[]` array. This is a hard prerequisite for Phases -1c, -1d, 0b, 0c, and P2P-4 — all of which depend on `oracle.id` for correct peer_id generation and index derivation. Without this change, after any oracle removal, `generate_peer_id(dense_index)` produces different peer_ids than `generate_peer_id(on_chain_id)`, causing BLS verification failures and total consensus stall.
...
pub fn compute_threshold(num_oracles: usize) -> usize {
    ...
    (num_oracles * 2 / 3) + 1  // floor(2n/3) + 1
}
```
...
```
- `RegistrySyncHandler`: computes `node_index` = count of active oracles with on-chain ID < `self.oracle_id`. **Requires** `Oracle.id` field (Phase -1d prerequisite) — the handler iterates `get_oracle_registry()` which now returns on-chain IDs via `getActiveOracleEndpoints()`.
- `RegistrySyncHandler`: computes `oracle_registry_index` = `self.oracle_id` (unchanged)
- `apply_pending_config_update()`: uses `update.node_index` for `LeaderElector::new()`, updates `self.config.oracle_registry_index` from `update.oracle_registry_index`
- If this node IS the removed oracle (`self.oracle_id` not in active set): log ERROR, self-halt gracefully (don't panic)
- **Crash recovery**: On restart, bootstrap (Phase -1c) re-derives correct `node_index` and `oracle_registry_index` from chain. The `pending_config_update` cell is ephemeral — loss on crash is harmless because bootstrap replaces it.
...
**Bitmap fix**: All bitmap computations use `config.oracle_registry_index` (the on-chain ID), NOT `peer_id[0]`. The `peer_id[0]` value is irrelevant for bitmaps — it's only for P2P identity.

**Key registry fix**: `RegistrySyncHandler.process_event()` MUST use `generate_peer_id(oracle.id as u32)` (on-chain ID) instead of raw `peer_id[0] = idx as u8`. The `idx` from `enumerate()` on the active-only list is a dense index, NOT the on-chain ID — after any oracle removal these diverge. Using the dense index would produce different peer_ids than Phase -1d's bootstrap, causing BLS verification failures. This requires the `Oracle` struct change from Phase -1d (adding `id` field, switching to `getActiveOracleEndpoints()`).

Locations to fix:
- `protocol.rs` ~line 2549: ITP creation bitmap -> use `oracle_registry_index` from config or key registry lookup
- `registry_sync/mod.rs` ~line 420: `peer_id[0] = idx as u8` -> `let peer_id = generate_peer_id(oracle.id as u32);` (on-chain ID, NOT enumeration index)
...
During config propagation window (~5s), accept proposals from the leader according to EITHER old or new `num_oracles`:
```rust
...
**Current bug**: MirrorOracleRegistry.sol (line 130) uses `abi.encode("REGISTRY_SYNC", nonce, ...)`. Rust `build_registry_sync_message_hash()` (registry_sync/mod.rs:642-697) uses `abi.encodePacked` semantics (raw 13-byte tag, no ABI padding, total 237 bytes). These produce different hashes — BLS signature verification will fail.
...
**Recommendation**: Option 1 — change Solidity to `abi.encodePacked`. Simpler, already tested in Rust, and MirrorOracleRegistry is a proxy (upgradeable). The plan's Phase 4 will later change BOTH to `abi.encode` with chain binding (`block.chainid`, `address(this)`), at which point both sides update together.

**Implementation**:
```solidity
// MirrorOracleRegistry.sol — change line 130:
bytes32 messageHash = keccak256(
    abi.encodePacked("REGISTRY_SYNC", nonce, newAggPubkey, newActiveCount, newThreshold)
);
```
Update the test files that construct the same hash (MirrorOracleRegistry.t.sol, MorphoPermissionlessLiquidation.t.sol, ITPNAVOracle.t.sol).
...
The **leader** constructs `signersBitmask` from `aggregator.get_signatures().keys()`, mapping each signer's `PeerId` to their `oracle_registry_index` bit position.

**Griefing vector (accepted)**: A malicious leader can exclude a legitimate signer from the bitmask (inflating their `oracleMissedCount`). This is bounded:
...
- `oracleMissedCount` is advisory only — NEVER used for automated slashing (see P3)

### P3. `oracleMissedCount` is advisory only
...
- Change `_verifyBLS(bytes32 messageHash, bytes calldata blsSignature)` to:
  ...
  ```
- **Storage**: BLSVerifier layout FROZEN at 1 slot (`_blsOracleRegistry`). NO `__gap` added. All new state lives in OracleRegistry. If BLSVerifier ever needs storage, each child contract's `__gap` must be reduced by the same amount — design a migration at that time.
...
**`removeOracleByVote()`** in OracleRegistry.sol (NOT in BLSVerifier path):
...
**Operational constraint**: After `addOracle()`/`removeOracle()`, admin MUST call `setAggregatedPubkey(pubkey, N)` before further mutations. Enforced on-chain:
```solidity
function addOracle(...) external onlyAdmin {
    if (lastSnapshotNonce != registryNonce) revert OracleRegistry__PendingSnapshot();
    // ...
}
function removeOracle(...) external onlyAdmin {
    if (lastSnapshotNonce != registryNonce) revert OracleRegistry__PendingSnapshot();
    // ...
}
```
`_emitStateChange()` emits `SnapshotPending(nonce)`. Oracle nodes log WARNING if gap persists > 10 blocks (full state machine deferred to implementation).
...
- `_computeActiveBitmask()` iterates `_oracles[0.._oracleCount]`, sets bit `i` if `status == 1`. Gas scales with total oracles ever registered (not just active). Acceptable for 20-100 oracles. Called only in admin tx (`setAggregatedPubkey`), not user-facing.
- `getActiveBitmask() external view` — public view for node bootstrap (calls `_computeActiveBitmask()`)

### Oracle node: `VersionedKeyRegistry`
...
### Contract changes (in OracleRegistry.sol)
...
Unchanged from v3. PoP with `abi.encode("INDEX_BLS_POP", block.chainid, address(this), oracleAddr, blsPubkey)`.

---

## Item 7: Global Key Uniqueness

Unchanged from v3. `_pubkeyHashToOracleId` stores `oracleId + 1` (sentinel collision fix).
...
**Additional Rust-side change**: `build_registry_sync_message_hash()` in `registry_sync/mod.rs` must be updated to match the new Solidity encoding (`abi.encode` with `block.chainid` + `address(this)`). The handler must know the MirrorOracleRegistry's chain ID and address (add to `RegistrySyncConfig`).
...
// start-oracles.sh run 3-20 oracles on 127.0.0.1.
...
// In accept loop, before spawning handler:
{
    ...
    // Skip per-IP check for loopback (local dev runs N oracles on 127.0.0.1)
...
- Loopback IPs exempt — local dev with 3-20 oracles on 127.0.0.1 works out of the box
...
Uses `DashMap` (already in `oracle/Cargo.toml` dependency tree) to avoid RwLock write contention from 20 concurrent reader_loops:
...
The check goes in `ConsensusProtocol::handle_message()` (`protocol.rs`), **NOT** in `ConsensusMessageHandler` (`messages.rs`). Reason: `ConsensusProtocol` has access to `self.leader_elector`, `self.config` (num_oracles), and can be given `Arc<PeerScorer>`. `ConsensusMessageHandler` is a thin stateless router with none of these.
...
1. Compute `leader_index = cycle_number % num_oracles`
...
```rust
/// Ordered list of oracle PeerIds, index matches oracle index in registry
peer_registry: Arc<RwLock<Vec<PeerId>>>,
```

**Population strategy** (OracleRegistry stores ETH addresses, not PeerIds):

1. **On-chain discovery (preferred)**: Use `get_oracle_registry()` (which now returns on-chain IDs via `getActiveOracleEndpoints()` — see Phase -1d prerequisite). Build the registry using `generate_peer_id(oracle.id as u32)` for each active oracle. This handles non-contiguous IDs correctly after oracle removal. The static peer list (`ORACLE_PEERS` env) is used ONLY for TCP connection establishment (IP:port), NOT for identity mapping.

2. **Static peer fallback (local dev only)**: For local dev with `--test-key-seeds` where on-chain discovery is unavailable, parse the peer list and build the registry using `generate_peer_id(node_id)` for each `node_id` 1..=num_oracles. This assumes contiguous IDs and will break after any oracle removal — acceptable for local dev only.

**WARNING**: The previous approach of building the peer_registry from static peers with `generate_peer_id(node_id)` for `node_id` 1..=N assumes contiguous IDs. After any oracle removal, IDs have gaps (e.g., 1, 3, 4, ..., 20). Dense index 1 would map to `generate_peer_id(2)` but the actual node has on-chain ID 3 and peer_id `generate_peer_id(3)`. The leader check would reject legitimate leaders. Always prefer on-chain discovery in production.
...
Per-oracle WAL path prevents file collision when multiple oracles share the same CWD (as in `start.sh` and `start-oracles.sh` which run 3-20 oracles from the repo root). The `{node_id}` is substituted at startup from `--node-id`.
...
Phase -1 (Operational Infrastructure):
  -1a. On-chain consensus pause mechanism (OracleRegistry.consensusPaused)
       - RPC failure = paused (fail-safe)
  -1b. Mandatory --registry-sync for multi-oracle
       - initial_scan_blocks: 10,000 -> 86,400
  -1c. Bootstrap from on-chain state
       - ChainReader trait: get_active_oracle_count, get_registry_nonce, get_aggregated_pubkey
       - Derive oracle_registry_index + node_index from chain (BLS pubkey match)
       - threshold from compute_threshold(on_chain_active)
       - Deprecate --signature-threshold CLI flag
  -1d. Bootstrap key registry from chain
       - build_key_registry() queries get_oracle_registry(), populates InMemoryKeyRegistry
...
Phase 0 (Bug Fixes):
  ...
  0c. Fix peer_id[0] inconsistencies — bitmaps use oracle_registry_index, key registry uses generate_peer_id(oracle.id)
  0d. Leader identity verification with dual-view tolerance
  0e. Fix abi.encode vs abi.encodePacked mismatch in MirrorOracleRegistry.sync()
...
Phase 4 (Key Architecture — contract-only):
  #8 Key Separation + MirrorOracleRegistry chain binding (abi.encode + chainid + address)
...
```
                    Oracle struct change (add id field, switch to getActiveOracleEndpoints)
                              |
                              v
Phase -1 ──> Phase 0 ──> Phase 1 ──> Phase 2+3 ──> Phase 4
                |
                +──> P2P (independent, parallel — but P2P-4 needs Oracle.id)

Within Phase 0:  0e (abi.encodePacked fix) ──MUST DEPLOY BEFORE──> -1b (--registry-sync enforcement)
```

Phase P2P has no contract dependencies. Can be deployed independently via binary update + rolling restart (no pause needed). Phase 0d (leader verification) from the contract plan is merged into P2P-4. P2P-4's on-chain discovery path requires the `Oracle.id` field (Phase -1d prerequisite).
...
All new CLI flags must be threaded into both `start.sh` and `scripts/start-oracles.sh`.

### New CLI flags

| Flag | Default | Purpose |
|------|---------|---------|
| `--wal-path <PATH>` | `./consensus-{node_id}.wal` | Per-oracle WAL file path |
...
In the oracle launch loop (line ~816):
...
### start-oracles.sh changes

Add to each oracle invocation:

```bash
    --wal-path logs/consensus-1.wal \
```

(Similarly for oracles 2 and 3.)

### vital-e2e-test.sh

No changes needed — it calls `start-oracles.sh` which inherits the WAL flags.
...
Deploy to 1 oracle first. Monitor for:
...
If clean after 24h, deploy to remaining oracles in batches of 3-5.
...
**Happy path**:
1. Admin calls `OracleRegistry.setConsensusPaused(true)` — one on-chain tx
...
4. Upgrade contracts via UUPS proxy: OracleRegistry, MirrorOracleRegistry, Investment, BLSCustody, ArbBridgeCustody, L3BridgeCustody, FeeRegistry, BridgeProxy
...
6. **Seed initial snapshot**: Admin calls `OracleRegistry.setAggregatedPubkey(currentPubkey, 1)` to write the bootstrap `RegistrySnapshot`. Without this, `getSnapshotAtNonce()` returns empty data and the first `_verifyBLS` call after unpause will revert (zero pubkey).
7. Upload new oracle binary to all 20 nodes via bastion
8. Restart all nodes (sequential is fine — they won't cycle while paused)
9. Wait for all `/ready` endpoints to return 200
10. Admin calls `OracleRegistry.setConsensusPaused(false)`
...
After any crash or restart, an oracle node:
1. Loads BLS keypair from file
2. Queries on-chain registry -> builds `InMemoryKeyRegistry` with all active oracles
3. Derives `oracle_registry_index` (BLS pubkey match) and `node_index` (dense index)
4. Queries `activeOracleCount()` -> computes threshold
5. Queries `registryNonce()` -> creates bootstrap snapshot in `VersionedKeyRegistry`
6. Constructs `LeaderElector(node_index, num_oracles)`
...
**New node joining**: Identical to crash recovery. Zero prior local state required. The node's BLS key must already be registered on-chain via `addOracle()`.
...
| Automated slashing | Not added | `oracleMissedCount` is advisory only. Governance must cross-reference on-chain tx history. |
| NAT traversal | Not added | All oracles on public IPs. No hole punching needed. |
...
| File | Items | Notes |
|------|-------|-------|
| `contracts/src/registry/OracleRegistry.sol` | -1a, #1, #2, #3, #6, #7, #8 | `__gap` 34 -> 29 (5 slots) |
| `contracts/src/interfaces/IOracleRegistry.sol` | -1a, #1, #2, #3, #6, #7, #8 | All changed/new signatures |
| `contracts/src/registry/MirrorOracleRegistry.sol` | 0e, #1, #2, #8 | `__gap` 45 -> 43 |
| `contracts/src/interfaces/IMirrorOracleRegistry.sol` | #1, #2 | New views |
...
| `common/src/types/oracle.rs` | -1d | Add `id: u64` field to `Oracle` struct |
| `oracle/src/chain/reader.rs` | -1d | Switch `get_oracle_registry()` from `getOracles()` to `getActiveOracleEndpoints()`, populate `Oracle.id` |
| `common/src/traits/chain_reader.rs` | -1c | New ChainReader methods |
| `oracle/src/consensus/protocol.rs` | #0, #1, #3 | ConfigUpdate struct, leader verification, bitmap fix |
| `oracle/src/consensus/aggregator.rs` | #0, #3 | Delete calculate_threshold, non_signers_bitmask |
| `oracle/src/consensus/keys.rs` | #2 | VersionedKeyRegistry with pruning |
| `oracle/src/leader/election.rs` | #0 | Accept dense node_index from ConfigUpdate |
| `oracle/src/chain/events/registry_sync.rs` | #0, #1, #2 | ConfigUpdate cell, bootstrap snapshot |
| `oracle/src/chain/writer.rs` | #1, #3 | All build_*_tx gain reference_nonce + signers_bitmask |
| `oracle/src/registry_sync/mod.rs` | #0, #4 | ConfigUpdate computation, peer_id fix |
| `oracle/src/bootstrap/consensus.rs` | -1c, -1d | Chain-based key registry + indices |
| `oracle/src/bootstrap/types.rs` | -1c | generate_peer_id used consistently |
| `oracle/src/main.rs` | -1a, -1e | /ready endpoint, pause check |
| `scripts/deploy-ceremony.sh` | -1f | **NEW** — production orchestration |
| `scripts/start-oracles.sh` | -1b | Add --registry-sync flag |

### Part B: P2P Layer Hardening

| File | Action | Lines Added (est.) |
|------|--------|-------------------|
| `oracle/src/p2p/rate_limit.rs` | **New** | ~60 |
| `oracle/src/p2p/peer_scoring.rs` | **New** | ~150 |
| `oracle/src/p2p/wal.rs` | **New** | ~200 |
| `oracle/src/p2p/metrics.rs` | **New** | ~80 |
| `oracle/src/p2p/connection.rs` | Edit | ~30 (rate limit + ban check pre-decode, scorer calls) |
| `oracle/src/p2p/transport.rs` | Edit | ~40 (conn limits, scorer tick, disconnect_peer method) |
| `oracle/src/p2p/mod.rs` | Edit | ~10 (exports) |
| `oracle/src/consensus/state.rs` | Edit | ~1 (add `Hash` derive to ConsensusPhase) |
| `oracle/src/consensus/messages.rs` | Edit | ~10 (content_hash function, RejectedNonLeader variant) |
| `oracle/src/consensus/protocol.rs` | Edit | ~60 (P2P leader check, equivocation check, WAL writes, replay mode) |
| `oracle/src/bootstrap/p2p.rs` | Edit | ~15 (thread PeerScorer, WAL config) |
| `oracle/src/bootstrap/mod.rs` | Edit | ~5 (WAL path + sync mode params) |
| `oracle/src/main.rs` | Edit | ~10 (CLI flags, WAL open + replay) |
| `oracle/Cargo.toml` | Edit | ~1 (add `crc32fast`) |
| `start.sh` | Edit | ~3 (WAL path flag in oracle loop) |
| `scripts/start-oracles.sh` | Edit | ~6 (WAL path flag per oracle) |
| **Total** | | **~670 lines** |
...
| BLSVerifier | 0 | 0 | **0 (FROZEN)** | Layout locked. New state goes to OracleRegistry. |
...
- `MirrorOracleRegistry.sync()` signersBitmask unvalidated (documented)
