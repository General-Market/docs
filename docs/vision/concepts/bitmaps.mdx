...
After committing the hash on-chain, the player must reveal the actual bitmap bytes to issuers before the tick resolves. The sealed prediction becomes public. What was private becomes shared. What was hope becomes arithmetic.
...
If a player commits a bitmap hash on-chain but fails to reveal the actual bytes to issuers before resolution, the player is **voided** for that tick. They committed to having an opinion but never shared it. Voided players:
