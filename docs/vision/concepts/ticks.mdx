...
Every ten minutes, the world is measured. Prices move or they do not. Winners are paid with losers' money. The protocol takes 0.3%. Time continues.
...
When a tick becomes due for resolution, the issuer engine runs its pipeline. It is mechanical, deterministic, and utterly fair -- which is to say, it has no mercy:
...
After a tick ends, there is a pause. Ten minutes of grace. Issuers wait for the **reveal window** (default: 600 seconds) before resolving. This gives players time to submit their bitmap reveals to issuers.
