...
Resolution types are stored as raw `uint8` values on-chain. The Solidity enum defines the first 8 types; extended types (8-13) are handled by the issuer resolver:
...
// are stored as uint8 and resolved by the issuer
```

## How Resolution Works

At tick resolution, the issuer computes the percentage change for each market. The arithmetic is simple. The consequences are not:
