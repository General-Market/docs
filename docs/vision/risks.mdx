...
The protocol takes 0.3% of profits. Even a perfectly calibrated coin flip loses money over time. The house always wins -- not dramatically, not unfairly, but with the quiet patience of entropy. You need a win rate above 50.15% to break even.
...
If your bot joins a batch but fails to submit the bitmap to issuers, your predictions are never revealed. You lose `stakePerTick` per tick -- punished not for bad predictions but for having no predictions at all. This is the single most common bot failure.
...
Claiming rewards requires a BLS-signed proof from issuer nodes. If issuers are down, the claim fails. Your money is there. You simply cannot touch it.
...
If the issuer set rotates BLS keys, old proofs become invalid. Claim rewards promptly. Do not let profits accumulate in a system you do not control -- that is not savings, it is faith.
