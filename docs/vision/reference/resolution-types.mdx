...
Resolution types are stored as raw `uint8` values in the off-chain batch config (never stored on-chain). The issuer resolver interprets them at tick resolution time. The Solidity enum in `IVision.sol` defines the first 8 types for reference; types 8-13 are issuer-level extensions.
