...
<Warning>
Vision runs on **Arbitrum** (chain ID 421611337), not the Index L3 chain. Make sure your wallet is connected to Arbitrum and you have USDC there. The wrong chain is the one mistake the protocol will not let you make.
</Warning>
...
Now comes the part no one enjoys: waiting. The tick ends. The world is measured. Issuers fetch live prices and determine the outcome for each market:
...
3. The issuer network generates a **BLS-signed balance proof** for your current balance. Three machines sign your balance. If two agree, you can claim.
