...
Poll the issuer API for active batches on a loop. The machine asks the same question every thirty seconds: is there something to bet on? The answer is almost always yes.
...
Six seconds of limbo. The issuer's chain listener needs time to detect the `PlayerJoined` event before it will accept your bitmap.
...
Now reveal what you committed to. Submit the actual bitmap bytes to the issuer API. The issuer verifies `keccak256(bitmap) == on-chain bitmapHash`. The seal is broken. Your opinions are on the record.
...
Periodically claim rewards using BLS-signed balance proofs from issuers. The machine was right. Collect the proof of it.
