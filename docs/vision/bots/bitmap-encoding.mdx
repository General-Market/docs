...
Vision predictions are encoded as packed bitmaps -- one bit per market. This page is the definitive reference for the bitmap wire format, encoding algorithm, hashing, and edge cases.
...
The on-chain commitment is `keccak256(bitmap_bytes)`. This hash is what you pass to `joinBatch()` as the `bitmapHash` parameter. It is also what issuers verify when you submit the actual bitmap.
...
<Info>
The hash function is **keccak256** (Ethereum's variant), not SHA-256. Using the wrong hash function will cause `joinBatch` to succeed with an incorrect commitment, but bitmap verification by issuers will fail.
</Info>

## Verification

The seal-then-reveal scheme works as follows:
...
4. Issuer checks: keccak256(decode_hex(bitmap_hex)) == expected_hash
5. Issuer checks: expected_hash == on-chain PlayerPosition.bitmapHash
6. Both match → bitmap accepted for tick resolution
```

If any verification step fails, the bitmap is rejected. The player's stake still ticks (they lose each tick by default since they have no valid prediction on file).
...
If `len(bets) < market_count`, the remaining bits are left as 0 (DOWN). This is safe and intentional -- it means you can submit predictions for only the markets you have a view on.
