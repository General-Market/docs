...
The on-chain commitment is `keccak256(bitmap_bytes)`. You publish the hash before revealing the opinion. A sealed confession -- the blockchain knows you have a view but not what it is. This hash is what you pass to `joinBatch()` as `bitmapHash`, and what issuers verify when you later reveal the actual bitmap.
