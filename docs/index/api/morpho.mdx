...
ITP shares are collateral. The oracle that prices them is BLS-verified and issuer-signed. If the oracle lies, liquidations break. So it does not lie.
...
```
  ...
  +------------------+     +------------------+     +------------------+
  |  Issuer Network  |     |  BLS Aggregation |     |  ITPNAVOracle    |
  |  (3 issuers)     | --> |  (2/3 threshold) | --> |  (on-chain)      |
  |                  |     |                  |     |                  |
