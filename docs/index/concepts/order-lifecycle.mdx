...
<Steps>
  ...
  <Step title="Order is PENDING">
    Your order enters `PENDING`. It sits on-chain, visible to the oracle nodes. It will stay there for about one second. In that second, three machines decide whether to trust each other. This is consensus.
  </Step>
  <Step title="COLLECT Phase">
    The oracle nodes detect pending orders during **COLLECT** (200ms). All orders are gathered into a candidate batch. The clock is already running.
  </Step>
  ...
  <Step title="Signature Aggregation">
    The leader collects signatures. Once `ceil(2n/3)` of registered oracles have signed, the signatures collapse into a single compact proof. Many voices, one signature.
  </Step>
