...
Written in Rust (68K LOC), the issuer nodes are the consensus layer of the protocol. They batch orders, coordinate BLS signing across peers, and submit confirmed batches on-chain. One-second cycles. Round-robin leaders. No meetings.
...
Round-robin. Deterministic. Based on the node's index in `IssuerRegistry.sol`. The leader rotates every second.
...
| **Discovery** | Static config + on-chain registry via `IssuerRegistry.sol` |
...
The issuers and the AP do not talk. They cannot. This is not a limitation -- it is the architecture.

<Warning>
  Issuer nodes **cannot communicate directly** with the AP. The AP discovers confirmed batches exclusively through on-chain `TradeRequest` events. This boundary exists because if the signers and the executor could coordinate off-chain, the system would have a single point of collusion.
</Warning>
