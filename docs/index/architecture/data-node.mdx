---
title: "Data Node"
description: "98+ data source aggregation, market data API, backtesting engine, and real-time feeds"
---
...
```
 ...
 ║  └─────────────┘         │ (L3 + Settl.)  │      │ issuer_health      │     ║
 ║                          ├────────────────┤      ├────────────────────┤     ║
 ║                          │ FNG / Issuer   │─────►│ fng_index          │     ║
...
Beyond the 91 external sources, the data node runs its own collectors -- polling chains, computing NAV, tracking issuer health. The system watches itself.
...
| **Issuer Health** | `issuer_health_collector.rs` | Issuer node status, latency, consensus health | 5m |
...
```
    ...
    └────────────┬─────────────┘                    │
         ...
         │  MorphoPositions, FillRecords, Issuers       │
         └──────────────────────┬──────────────────────┘
                                │
                                ▼
                          REST API serves
...
```
    ...
    │  │ batch_tables         │              │ issuer_health_snaps  │    │
    │  └──────────────────────┘              └──────────────────────┘    │
    │                                                                     │
    └─────────────────────────────────────────────────────────────────────┘
```
