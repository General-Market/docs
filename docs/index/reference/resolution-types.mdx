...
Resolution types are stored as raw `uint8` values in the off-chain batch config (never on-chain). The issuer resolver interprets them at tick resolution time.
