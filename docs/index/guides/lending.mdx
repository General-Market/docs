...
```
  ...
  |     Issuers compute NAV --> BLS aggregate --> push on-chain  |
...
ITP collateral is priced by the `ITPNAVOracle`. Three issuers compute NAV from live market prices. Two must agree. The price is then pushed on-chain with a BLS signature. Trust is replaced by cryptography. This is the entire premise.
...
<Note>
The NAV oracle updates every issuer cycle. Between updates, Morpho uses the last BLS-verified price. There is a brief lag between market reality and on-chain truth. In calm markets, this lag is irrelevant. In volatile markets, it is the difference between safety and liquidation.
</Note>
