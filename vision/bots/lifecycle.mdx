...
This page documents every step a Vision bot performs, from initial registration through reward claiming and withdrawal. Each step includes the exact function call or API request, expected behavior, and error handling.
...
│     + resubmit to issuers        │    + POST /vision/bitmap
└────────┬─────────────────────────┘
         │
         ▼
┌──────────────────┐
...
Register your bot on the Vision contract. This is a one-time operation.
...
Poll the issuer API for active batches on a loop.
...
| Error | Cause | Fix |
|-------|-------|-----|
| HTTP 500 | Database error on issuer | Retry after backoff |
| Timeout | Issuer unreachable | Check `VISION_API_URL`, retry |
...
For each market in the batch, decide UP or DOWN. See [Example Strategies](/vision/bots/strategies) for algorithms.
...
The issuer's chain listener needs time to detect the `PlayerJoined` event before it will accept your bitmap submission.
...
**Why 6 seconds?** The issuer's chain listener polls for new events on a ~3-5 second interval. A 6-second wait gives comfortable margin.

<Info>
If you submit the bitmap too early, the issuer responds with HTTP 404: "Player not found in batch." Retry after a few seconds.
</Info>

---

## Step 8: Submit Bitmap to Issuers

Submit the actual bitmap bytes to the issuer API. The issuer verifies `keccak256(bitmap) == on-chain bitmapHash`.
...
You can change your predictions mid-batch by updating the on-chain hash and resubmitting the bitmap.
...
# Update on-chain, then resubmit to issuers
...
**On-chain effect:** Overwrites `PlayerPosition.bitmapHash` with the new hash. The old bitmap is discarded by issuers when the new one is submitted.
...
<Tip>
Updating your bitmap mid-batch lets you adapt to changing market conditions. However, the update only takes effect for the next tick -- the current tick uses the previously submitted bitmap.
</Tip>
...
Periodically claim rewards using BLS-signed balance proofs from issuers.
...
**On-chain effect:**
- BLS signature is verified against the issuer registry's aggregated public key.
- If `newBalance > oldBalance`, the difference (minus 0.3% fee on profit) is transferred to the player.
...
| `InvalidBLSSignature` | Bad or insufficient BLS signatures | Re-fetch proof from issuers |
...
Exit a batch entirely and recover your remaining balance.
...
- Fee (0.3%) charged on profit only: if `finalBalance > totalDeposited`, fee is on the difference.
...
Here is the full lifecycle assembled into a single polling loop:
...
<Info>
The bot maintains a local `joined_batches` set to avoid duplicate joins. On restart, it checks on-chain positions via `getPosition()` to rebuild this set.
</Info>
...
| Claim | `InvalidBLSSignature` | Re-fetch proof from issuers |
