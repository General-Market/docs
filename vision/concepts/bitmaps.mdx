...
A **bitmap** is a compact binary encoding of a player's predictions across all markets in a batch. Each bit represents one market: `1` means UP, `0` means DOWN. Bitmaps are committed on-chain as a keccak256 hash and revealed off-chain to issuers before resolution.
...
Bitmaps are committed on-chain as a **keccak256 hash** before the actual bitmap bytes are revealed. This prevents other players from seeing your predictions and front-running.
...
4. **Player reveals bitmap off-chain** by sending the actual bytes to issuers via `POST /vision/bitmap`.
...
<Warning>
The keccak256 hash of the submitted bitmap bytes must match the on-chain `bitmapHash`. If they do not match, the issuer rejects the submission and the player's predictions are voided for that tick.
</Warning>

## Reveal Process

After committing the hash on-chain, the player must reveal the actual bitmap bytes to issuers before the tick resolves.

### Submitting to Issuers

```bash
curl -X POST https://issuer.example.com/vision/bitmap \
...
After a tick ends, there is a configurable **reveal window** (default: 600 seconds) before bitmaps are made public. During this window, issuers use the bitmaps for resolution but do not expose them via the API. After the window expires, anyone can query revealed bitmaps:
...
<Tip>
Players can update their bitmap at any time by calling `updateBitmap(batchId, newBitmapHash)` on-chain, then revealing the new bitmap to issuers. The new bitmap takes effect starting from the next tick.
</Tip>
...
For example, a batch with 3 markets only uses bits 0-2 of byte 0. Bits 3-7 are ignored regardless of their value.

### Unrevealed Bitmaps

If a player commits a bitmap hash on-chain but fails to reveal the actual bytes to issuers before resolution, the player is **voided** for that tick. Voided players:
...
If a batch has zero markets (`marketIds` is empty), the bitmap is 0 bytes. This is a valid but degenerate case -- no markets resolve and all players' balances remain unchanged.
