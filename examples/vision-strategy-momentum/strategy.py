...
Bets with the recent price trend. If an asset went up over the lookback
window, bet UP; if it went down, bet DOWN.
...
LOOKBACK_TICKS = 5  # Number of recent ticks to measure trend
...
def generate_bets(batch_state: dict) -> list[bool]:
    """
    Generate momentum-based bets for a batch.
    ...
    Returns:
        List of booleans: True = UP (momentum positive), False = DOWN.
    """
    ...
    for market_id in market_ids:
        ...
        if len(history) >= 2:
            # Momentum: compare latest price to oldest in the window
            trend = history[-1] - history[0]
            bets.append(trend > 0)  # UP if price increased
        else:
