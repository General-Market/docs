...
contract DeployFullSystemE2E is Script {
    // ============ CONSTANTS ============
    ...
    function _deployBridge() internal {
        ...
        // 6. Set signer threshold (2/3 for local E2E)
        BridgeProxy(bridgeProxyAddr).setSignerThreshold(2);
    ...
    function _wireContracts() internal {
        ...
        // Set BridgeProxy on ArbBridgeCustody BEFORE agg pubkey is set (Phase 7),
        // because setBridgeProxy requires BLS verification when pubkey is non-empty.
        ArbBridgeCustody(arbBridgeCustodyProxy).setBridgeProxy(bridgeProxyAddr, "");
        console.log("  ArbBridgeCustody wired to BridgeProxy");
    }
    ...
    function _registerIssuers() internal {
        ...
        // Set precomputed aggregated G2 pubkey for 3 test issuers (seeds [0,0x42], [1,0x42], [2,0x42])
        // Generated by: cargo test -p issuer print_aggregated_pubkey_for_e2e -- --nocapture
        bytes memory aggPubkey = hex"";
        ...
        console.log("  Aggregated pubkey set on IssuerRegistry and CollateralRegistry");
    }
    ...
    function _registerIssuer(uint256 idx, address issuer, string memory ipPort) internal {
        ...
        // Real BLS G2 pubkeys from deterministic seeds (seed[0]=idx, seed[1]=0x42)
        // Generated by: cargo test -p common print_node_pubkeys_for_deployment -- --nocapture
        bytes memory blsPubkey;
        if (idx == 0) {
            blsPubkey = hex"";
        } else if (idx == 1) {
            blsPubkey = hex"";
        } else if (idx == 2) {
            blsPubkey = hex"";
        } else {
            revert("Only 3 issuers supported in E2E deploy");
        }
