    ...
    F: PriceFetcher + 'static,
{
    ...
    pub async fn handle_message(&self, from: PeerId, message: P2PMessage) -> Result<(), Error> {
        ...
        match result {
            ...
            // Bundle (single-aggregated-BLS) handlers retired with the dead-msg
            // sweep. Variants remain on MessageHandleResult for now; ignore them
            // here. Restore from git history if the bundle path returns.
            ...
            | MessageHandleResult::ProcessMintBridgedSharesBundleSign { .. } => {
                debug!("bundle message variant retired — ignoring");
            }
