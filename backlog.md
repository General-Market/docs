...
## 20260518-1620-w9zk oracle-3 fleet-key drained (binance rounds frozen)
[FAILED] User reported binance_spot/binance_options/binance_futures_funding stopped pushing new rounds on generalmarket.io/source/<id>. Latest binance_spot round had settled at 12:49 UTC; options + funding at 11:38 UTC. Three+ hour gap on a 5-minute tick. Misleading symptom: oracle kept signing `VisionCreateBatchSign` every ~70s for all three sources, so the consensus surface looked healthy.
[DECISION] Root cause is gas, not consensus. Oracle-3 leader fleet-key `0xdF00D68d…` held 0.005116506 GM on L3. Each create_batch / settle_batch wants 0.005–0.009 GM. The wallet had been bleeding for hours. Oracle-3 happens to own the three heaviest sources by market_count (binance_spot=431, binance_options=800, binance_futures_funding=531) plus flights/mcbroken/polymarket/tmdb — so its fleet key drains faster than oracle-1's or oracle-2's. Five of the six fleet keys were under 2 GM.
[DECISION] Fix: cast send 50 ether from ISSUER_2 (`0xC0d3ca67…`, ~100M GM) to each of the six fleet keys. No restart needed — oracle's retry sweep picked up the failed batches inside one heartbeat. binance_options batch 59939 created 14:20:03 UTC, binance_spot 59940, binance_futures_funding 59942.
...
## 20260520-2210-jrvs Jarvis built — one Python process forwarding waitlist + Discord + Gmail to Telegram
[DECISION] Single asyncio process, three watchers, one Telegram chat. Lives at `jarvis/` in the index repo, runs as Docker container `jarvis` on VPS 3 attached to `dokploy-network`. State (`state/*.json`) bind-mounted from `/root/jarvis/state/`. Restart policy `unless-stopped`. Replaces the would-have-been-three-microservices design with one process under coarse supervision.
[DECISION] Waitlist watcher polls `submissions` (the join form), not `whitelisted_wallets` (the redemption step). Submissions has the personal payload — email, twitter, telegram, wallet, volume, reach, protection_from, notes — the user actually wants in the notification.
[FAILED] First iteration of the Gmail watcher fetched all UNSEEN messages on each poll. Max's inbox had 27,810 unread. The IMAP fetch hung on a multi-GB body batch, no logs, no Telegram delivery. Restart cycle silent.
[DECISION] Bootstrap fix: on first run with no `state/gmail.json`, capture `max(UID)` and forward nothing. Subsequent polls only consider UIDs strictly greater than the cursor that are also UNSEEN. Cap at `MAX_PER_POLL=10` per cycle. Telegram is not a backfill device.
[DECISION] HTML formatting with emoji prefixes — 🚪 waitlist, 💬 Discord, 📧 Gmail. Emojis here are by explicit user request and don't override the Cioran no-emoji rule elsewhere in the codebase.
[DECISION] Outreach/warmup spam filter in `gmail_poller.py`. Regex list (`3CYMYP8`, `SPF DKIM DMARC`, `PlusVibe`, `Lemwarm`, `allaccs`, trailing `| AB12345 XY67890` tracker pattern) plus sender-domain blocklist (`joinconversionstack.com`, `veloryxgroup.com`). Skipped messages logged at INFO, not forwarded. Extend by appending to `OUTREACH_SUBJECT_PATTERNS` or `OUTREACH_SENDER_DOMAINS`.
[DECISION] Deleted 8 obviously-junk waitlist rows (smoke tests, Max's own otc_max/max_otc entries, four "eze@*" gibberish rows). Three real signups remain: rhadokrypt, cube/yolonomics, jetaime. Backup at `/root/jarvis-waitlist-purge-20260520.json` on VPS 3.
[REFERENCE] Discord bot is the "General" app (id `1506406898339942550`), in guild `1473683174222598353` (*General Market*), watching channel `1473683175484952881` (*#general*) — only one. **Message Content Intent** must stay on in the developer portal or `msg.content` returns empty.
[REFERENCE] Waitlist DB lives in Docker container `waitlist-pg` on `dokploy-network` (VPS 3). Hostname only resolves inside the overlay — Jarvis runs on the same network for this reason. DSN in `jarvis/.env` (also gitignored).
[REFERENCE] Twitter integration deferred. Free tier API is dead, Basic ($100/mo) gets mentions only, DMs require Enterprise. Cheap path: turn on Twitter email notifications and let the existing Gmail watcher relay them.

## Session: 20260524-1450-or8c (Vision round/PnL bug + oracle redeploy outage)

- [DECISION] `/source/defillama-bridges` showed a settled prior-round loss attached to the open round, twice, before it could settle. Root cause: oracle profile builder (`api.rs`) grouped settled ticks by source NAME and swept a source's whole history onto whichever batch it processed first (the live, unsettled one) via `rounds_by_source.remove(&source)`. Fix: attach each tick to its own `batch_id` (`rounds_by_batch` only). Q0/Q1 read the same table grouped vs not, so every tick has an aggregate row and surfaces through its own ProfileBatch — no orphans.
- [DECISION] Frontend (`SourceDetailHumanTrading.tsx`): only the NOW timeline box carries the "IN" badge (was painting LAST+NEXT off a single commit); `PositionsCard` now gets `effectiveCommit` (optimistic) instead of chain-only `currentCommit` (kills the ~5s display lag); the header P&L is labelled "Realized" and hidden until a round settles (so a past loss can't read as the live bet losing); ticks deduped by `(batch, round)`.
- [FAILED] `redeploy-oracle.sh` took the three prod oracles DOWN. Step 2 `docker rm -f`'d the containers, then step 3 failed on `docker-compose.override.yml: no such file or directory`. The override is generated by testnet.sh, not in git, and had been cleaned off the VPS on May 21. A created container kept its config so the oracles ran fine until removed — then couldn't be recreated.
- [DECISION] Recovered by regenerating the override from canonical sources (`scripts/gen-oracle-override.py` reads `active-deployment.json` + `system.env`, mirrors testnet.sh) and bringing oracles up on the existing image. BLS intact (3 signers, threshold 2) — key files survived and the BLS keys are seed-derived, not from those files.
- [DECISION] Hardened `redeploy-oracle.sh`: build FIRST while old containers serve, swap only after the new image exists (a failed build can no longer take prod down); regenerate the override if missing and abort before any destructive step if it still can't be produced.

## 20260524-1830-ac7x — AntiCheat inter-scene transitions

[DECISION] Upgrade AntiCheat transitions by extending the existing zoomThroughBlur primitive with a per-layer directional SLIDE vector (dx/dy in vw/vh) rather than swapping to stock @remotion/transitions slide()/wipe()/flip(). Reason: the film's cuts are built on a non-reversing zoom-through-blur that hides the geometric jump inside the blur peak and publishes --acx-fg/bg-scale vars consumed by BgLayer. Stock presentations would (a) discard the blur-continuity grammar, (b) break the CSS-var plumbing, (c) read as a generic slate. Adding a travel vector to the same primitive gives the premium directional read the brief asks for while staying cohesive. Timings/durations/beats untouched — only visual character of the presentation changes.
[DECISION] No scene-entrance upgrades needed — audited all 7 scenes; every entrance is already real spring/3D/char-reveal/drop-settle/whip motion, none is a bare opacity fade. Brief's "upgrade merely-opacity entrances" clause has no target here.

## 20260525-1530-p4k9 — Vision market participants: API hang + all-Cancelled curated subsources

[DECISION] Fixed the "Loading participants…" hang on curated-subsource market pages (e.g. /source/defillama-ai-agents/market/protocol_agentfi). Root cause: AssetActivityCard queried settlements via `dataNodeSourceId = toInternalId('defillama-ai-agents') = 'defi'`, but vision_settlements keys curated-subsource settlements by the batchSubsourceKey ('defillama-ai-agents'), not the parent firehose id. So the query hit the giant main-`defi` partition: wrong batch's participants AND a >12s JSONB detoast scan that exceeded the proxy's 12s per-attempt budget → 503 → stuck spinner. Fix: added `toBatchSourceId()` in lib/vision/source-ids.ts; routed only the settlements hook through it (price history still uses the shared parent id). Verified: subsource-keyed call returns 200 in 0.19s/40 rows vs 12.3s for the parent id.
[FINDING] Deeper backend bug, NOT fixed here: every curated DefiLlama subsource batch resolves 100% Cancelled. All 17 subsources (ai-agents, bridges, derivatives, dexs, lending, risk-curators, …) show 0 resolved / N cancelled outcomes from 2026-05-22 → 2026-05-24, while the main `defi` source resolves normally (7.27M Flat, 372K Up, 356K Down). Cancelled batches refund every player and carry empty `by_asset`, so the participant matrix is always empty even with the corrected source id. The oracle never has start/end prices for curated-subsource batches — needs separate investigation of the curated-subsource resolution/price-fetch path (batch_engine.rs + oracle lifecycle).
[FINDING] Sibling latent bug (cosmetic): /api/vision/config/[source] tries toInternalId(source) first and only falls back to the raw subsource name if the parent returns empty markets. For curated subsources the parent `defi` config is non-empty, so the threshold band pill shows the main-defi batch's band, not the subsource's. Not fixed — cosmetic, out of scope for the participants report.

## 20260525-1210-h7k3 — Human sources rollout (all firehose sources → human page)

[DECISION] Turn every non-human Vision source into a "human source" like DefiLlama. The human-page machinery already existed (`audience:'human'` → `SourceDetailHumanTrading`), and that component already had a top-10-by-value fallback for the no-allowlist case. So the rollout = config flip + generalizing the fallback + live re-rank + a vault tab. 99 sources flipped (4 'bot' + 95 no-audience) → 117 human + 1 redirect.
[DECISION] Popularity metric is a cascade, not raw `value`. 81 of 99 sources are non-price (temperature, delay-minutes, AQI) where "value" is a measurement, and even crypto's `value` is unit price (a $90k coin isn't "popular"). `lib/vision/popularity.ts`: marketCap → volume24h → value, deterministic (assetId tiebreak). Honest label: header reads "Top 10 by <valueLabel>" ("Top 10 by Score", "Top 10 by TVL"), never the word "popular".
[DECISION] Live re-rank = FLIP reorder in place (measure → invert → transition), keyed by assetId, respects prefers-reduced-motion. Display set updates live; the bettable set stays the frozen `tradableMarkets` (in-active-batch) subset. Display-but-not-bettable cards keep their locked state.
[DECISION] Vault content relocated to its own tab. `SourceTabNav` gains a 'vault' tab gated on `hasVaultForSource`; new route `/source/[sourceId]/vault` renders FeaturedVaultHero + VaultShowcase (the composition previously on the bot-page overview). Existing `/vault/[vaultAddress]` detail route untouched.
[DECISION] Phased: Phase 1 (frontend) ships on existing batches — every displayed top-10 is bettable because the firehose batch holds all healthy assets. Phase 2 (data-node `get_healthy_assets` → top-N) scopes the round itself to 10. Phase 1 carries no oracle risk; shipped in commit a08f9b48f and is live (hackernews now renders "Top 10 by Score").
[DECISION] Phase 2 is consensus-SAFE (verified independently): the scary `BatchConfigOrchestrator` (±30% count / <5% unknown tolerances + MAX_CREATIONS_PER_HOUR=3 global cap) is DEAD CODE — `run()` is an inert `loop { sleep }`, leader/follower commented out. The live path is `BatchLifecycleManager`: single shared data-node, settlement fetches markets by the batch's PINNED config_hash (lifecycle.rs:905), so a mid-round top-N re-rank cannot mis-settle. Code landed in d7b93f6a0.
[FAILED/BLOCKED] Phase 2 NOT activated on prod. VPS1's data-node is 119 commits behind mono main AND its working tree has local uncommitted edits to `data-node/src/batch_engine.rs` (the exact file Phase 2 changes), plus serve.rs/source_registry.rs/oracle files + an untracked compose override. A `git pull`+rebuild would drag in 118 unrelated commits and destroy unsaved prod work / hit a merge conflict. Backend deploys here are deliberate, not auto (only the frontend auto-deploys via Dokploy). Phase 2 must wait for a human-reconciled backend deploy: inspect VPS1's local batch_engine.rs edits, reconcile, then rebuild data-node-shadow.service. No oracle restart needed (oracles pick up new configs by hash).

## 20260525-1100-pm9q — ParimutuelExplainer Remotion video
[DECISION] New explainer `ParimutuelExplainer` at `video/src/compositions/parimutuel/` (theme.ts, primitives.tsx, beats.tsx, ParimutuelExplainer.tsx), registered at the root of Root.tsx. Teaches the batch anti-cheat as a chain of state transformations over ONE continuous BlueField (the field never cuts; only the content on top transforms): one question → oracle threshold (+2%) → A+B=C (YES+NO stakes merge into the pot) → B→A (losers fund winners) → the ballot (one ticket of 10,000 varied yes/no lines, swept + stamped at once) → everyone together (ballots converge from all edges into one batch → 10,000 sealed pools) → cost-to-cheat ($5k × 10k = $50M+, "not worth it") → three guarantees (sealed / settled by the oracle / split among winners) → landing aphorism "Cheating costs more than it pays." 9 beats, ~49s @ 30fps, landscape 1920×1080. Reuses anticheat-edit props (tokens palette, BlueField, RevealChars, IdleZoom). Silent + music bed: 03-eternal-odyssey (trimBefore 89.211s, its "hope rising" turn) at 0.34 + tech-hum atmos at 0.12.
[FEEDBACK] First cut buried the batch concept — single-market atom beats then a thin "×10,000" grid read as "imagine 10k of that," not "the unit of trade IS a 10,000-line sheet and everyone pours into the same batch." Fix: replaced the thin grid with two beats — `BallotBeat` (many DIFFERENT markets on one ticket, marked at once) carries "plenty of markets, at once"; `TogetherBeat` (incoming ballots from every edge → shared batch → sealed-pool grid) carries "and everyone does, together." The anti-cheat cost argument only lands once the viewer feels the simultaneity + isolation.
[GOTCHA] `RevealChars` renders each glyph as an inline-block span, so a multi-word headline wraps MID-WORD when it exceeds the container width (landing showed "it pa / ys."). Fix: split long headlines into explicit per-line RevealChars blocks + `whiteSpace: "nowrap"` on each line and on the Beat title.

## 20260525-1210-h7k3 — Phase 2 re-scoped to 5-minute sources + DEPLOYED

[DECISION] Phase 2 top-N betting cap was too broad ("all firehose sources"). Re-scoped: the cap fires only when a source's tick is 5 minutes (sync_interval_secs == 300). Rationale: at a slow tick the top-10 barely moves between rounds, so capping a deep source (crypto 6824, github 1067) to 10 only thins liquidity; the cap earns its place where the board churns fast. Only 9 sources are 300s: binance_spot/options/futures_funding, tmdb, pandascore, weather_alerts, db_trains, paris_metro, tfl_tube. Gate in batch_engine.rs: `curated_allowlist.is_none() && sync_interval_secs == TOP_N_TIMEFRAME_SECS(300) && healthy.len() > TOP_N_PER_BATCH`. Commit 7e907163c.
[DECISION] Frontend human-page rollout (Phase 1, commit a08f9b48f) stays for ALL non-human sources — NOT narrowed to 5-min. hackernews (the seed example) is 600s, must stay a human page. The human page shows a top-10 DISPLAY and bets on whatever's in the active batch; for non-5min sources that's the full firehose, for 5-min sources it's the capped top-10. Consistent.
[DEPLOYED] Phase 2 deployed to VPS1 data-node via surgical `git checkout origin/main -- data-node/src/batch_engine.rs` (box was 119 commits behind with local edits == main on the other 5 backend files; only batch_engine.rs lacked the change). Rebuilt as user max (cargo, ~3.5min), restarted data-node-shadow.service at 09:04:28 UTC. Binaries backed up: .bak-pre-phase2-* (original) and .bak-uncond-cap-* (unconditional-cap interim).
[VERIFIED] Gate proven both directions at the data layer. Post-restart (>09:04:28 UTC): tmdb (5-min) scoped to top-N at 09:07:04; defi/polymarket=10000, github=1067, hackernews=300, stocks=158 all recomputed post-restart and KEPT full set (not capped). 5-min sources binance_*/db_trains/tfl_tube/tmdb/weather_alerts all =10. data-node stable (NRestarts=0), snapshots HTTP 200, no "no recommended config" oracle errors. /recommended full-cycle confirmation pending (~16min cadence).
[GOTCHA] /batches/recommended + /signed return 0 for ~10-16min after any data-node restart — they publish only after the batch engine's full cycle completes. Existing rounds settle fine meanwhile (oracle reads each round's PINNED config_hash, not /recommended). Verify top-N via DB batch_configs.asset_count + "Firehose batch scoped to top-N" log, not /recommended, in the warmup window.

## 20260525-1610-r3k8 — Curated subsources now RESOLVE (three chained bugs fixed)

[DECISION] "Loading participants" on /source/defillama-*/market/* was THREE chained bugs, all now fixed + verified in prod:
  1. FRONTEND (commit 5d17fe825): AssetActivityCard queried settlements via toInternalId('defillama-ai-agents')='defi' — wrong, giant partition → >12s JSONB scan → proxy 503 → spinner. Fixed with toBatchSourceId() (settlements use batchSubsourceKey; price history still uses parent 'defi'). Proven 12.3s→0.19s.
  2. DATA-NODE / ORACLE PRICE LOOKUP (commit beebfd307): oracle resolve_and_settle + save_start_prices POST {source: <batch source>} to /vision/snapshot/targeted, filtered WHERE l.source=$1. For curated subsources the batch source is the subsource key but prices ingest under parent 'defi' → 0 rows → empty start+end prices → EVERY market Cancelled (all 17 subsources, 100%, since 05-22). Fixed in data-node SourceRegistry::price_source_for (subsource key → internal_ids[0]); snapshot_targeted resolves before the query. Surgically patched onto prod VPS1 working tree (no pull — 121 commits behind + 6 dirty files from other sessions), `cargo build --release --bin data-node`, restarted data-node-shadow. Verified: defillama-ai-agents/protocol_agentfi snapshot 0 rows → real price; batch 122713 got start_prices=10 AND end_prices=10.
  3. ORACLE STORAGE FLAG (commit f1f2513d5): vision_settlements (the table the participants UI reads) was FROZEN globally since 05-24 14:41 — the 05-24 oracle-redeploy recovery regenerated docker-compose.override.yml via gen-oracle-override.py, which never emitted USE_NEW_STORAGE=1. record_settlement_v2_jsonb is gated on that env → settled rounds stopped being mirrored. Fixed gen-oracle-override.py to emit the flag durably; added USE_NEW_STORAGE=1 to all 3 oracle env blocks in the running override; rolling-recreated oracles (both -f files merged) keeping BLS threshold 2/3 alive.
[FAILED] First oracle-1 recreate used `docker compose -f docker-compose.yml up -d` (single -f) — this SUPPRESSES auto-merge of docker-compose.override.yml, so the container came up base-only (no --node-id, no command, no volumes) and crash-looped "node-id is required". Oracles 2/3 stayed healthy (consensus intact). Recovered by recreating with BOTH `-f docker-compose.yml -f docker-compose.override.yml`. RULE: with an override present, never name only the base file — use plain `docker compose up -d` (auto-merge) or pass both -f.
[DECISION] All fixes are forward-only. Already-settled Cancelled batches are BLS-signed + immutable. Verified end-to-end: post-fix curated subsources resolve 10/10 non-Cancelled (risk-curators, rwa, liquid-staking, top-chains, dexs, lending, prediction-market, launchpad, onchain-capital-allocator, telegram-bot, trading-app, wallets) and land in vision_settlements with fresh settled_at.

[VERIFIED-E2E 20260525-1210-h7k3] Phase 2 confirmed across frontend + data + swarm bot.
- Frontend public API (generalmarket.io/api/vision/batches): binance_spot market_count=10, hackernews=300. Pages render human-trading view ("Top 10 by Price"/"Top 10 by Score", HTTP 200).
- Data: binance_spot 10-market batch = top-value pairs (btc/eth/bnb/bch/wbtc/xaut/paxg/wbeth/yfi/zec usdt); built by same popularity ORDER BY as the frontend display, so displayed-10 == bettable-10 by construction. hackernews config full at 300.
- Swarm bot = `vision-swarm` container on VPS1 (python swarm_runner.py, 10 bots). Live logs show it joining 10-market rounds (capped 5-min sources: rounds 123004/123008/123012/123018/123019/123014) AND full rounds (hackernews 300, others 998/500/37/22). It reads each batch's config by hash → picked up capped batches with NO restart. (Stale-batch 404 bitmap-resubmit warnings on old IDs 112023-4 are pre-existing, unrelated.)
- Other bots present on VPS1: itp-trader-bot, bot-bot-fund-*, bot-bot-opt-* (fund/vault bots), testnet-itp-bot.

[SETTLEMENT-VERIFIED 20260525-1210-h7k3] Next-round settlement of the capped 5-min batches confirmed CORRECT.
- All 8 live 5-min sources settle 10 markets / 0 cancelled / 10 resolved (real Up/Down outcomes w/ stakes, threshold_bps + pct_change_bps): binance_spot, binance_options, binance_futures_funding, tmdb, db_trains, tfl_tube, weather_alerts, esports. Settlement reads the round's PINNED config_hash so the frozen top-10 resolves even as the live top-10 rotates. The cap does not break settlement.
- hackernews (non-5min, 300 markets): 297 resolved / 3 cancelled (stale assets) — normal.
- FINDING (PRE-EXISTING, not Phase 2): crypto settles 100% Cancelled (6824/6824). Root cause = CoinGecko API rate-limiting (data-node log: repeated "CoinGecko rate-limited, waiting"), so the coingecko/crypto collector can't fetch fresh prices → config recovered-stale (age ~6.5h, started ~03:00 UTC, hours before the 09:04 deploy) → settlement correctly cancels/refunds markets with no fresh price. crypto is 600s, correctly NOT capped. Cancelling is the right behavior given no data; the real fix is the CoinGecko rate-limit (API key / fetch cadence), a separate operational issue. Flagged to user.

## Session 20260525-1247-hnw1 — HackerNews winnability

[DECISION] HN up_x% unwinnable: root cause = batch picks markets by value(score) DESC (oldest, plateaued stories) + threshold = 10-deep EMA that lags above a decelerating cumulative curve.
[DECISION] B1 — youth-cap HN universe to <6h (HN_MAX_STORY_AGE_SECS), youngest-N fallback (HN_MIN_UNIVERSE=20). Markets land on still-climbing stories. select_young_stories() pure + unit-tested.
[DECISION] B2 — new BatchStrategy::ENGAGEMENT_CUMULATIVE (ema_change_span 3) for HN so the threshold chases the deceleration down. ENGAGEMENT (span 10) kept for twitch (non-monotonic).
[DECISION] F1 — lin/log toggle on MarketCandleChart (localStorage gm:vision:chart-log), default log for cumulative sources. Price-scale mode drives priceToCoordinate so overlays follow; autoscale floors min >0 under log.
[DECISION] F2 — LatestBandPill states a concrete target ("must reach 2,540 (+8%) to win", projected from current value) instead of "±X% band"; chart threshold label "to win +8%".
[FAILED] eslint not runnable in this checkout — no committed flat config, next lint is interactive/deprecated. Relied on tsc --noEmit (clean).
[PENDING] Backend committed to mono but NOT deployed — VPS 1 data-node is ~121 commits behind w/ local batch_engine.rs edits; needs surgical patch + rebuild + restart.

[DEPLOY 20260525-1210-h7k3] coingecko/crypto removal — final state.
- Backend: DONE+VERIFIED. crypto in DISABLED_SOURCES (commit 41eb65f4d, data-node rebuilt+restarted 12:03). /recommended has NO crypto; no new crypto batch_config (last 05:11) or settlement (last in-flight drained 12:06). 5-min gate intact, snapshots 200.
- Frontend listing: DONE+VERIFIED. coingecko absent from /api/vision/sources (count 0) via HIDDEN_SOURCE_IDS + the data-node dropping crypto.
- Frontend page: NOT 404. /source/coingecko returns 200 rendering SourceDetailV2 ("CoinGecko Crypto" + vault/trending chrome + "Check back/No data" empty states). Confirmed on a CLEAN rebuild that a nonexistent source ALSO returns 200 — so the `/source/[sourceId]` route does NOT hard-404 unknown/hidden sources (the existing `!hasVaultForSource && audience!=='human'` gate doesn't fire for coingecko). Garbage top-level PATHS do 404, so notFound() works generally — it's the source route that renders permissively. A true page-404 needs a code change: `if (isHiddenSource({sourceId, internalIds: source?.internalIds})) notFound()` atop frontend/app/[locale]/(app)/source/[sourceId]/page.tsx — affects all ~32 hidden sources' deep links. PENDING user decision.
- DOKPLOY GOTCHA (cost ~1h): repeated webhook triggers spawned 4+ concurrent `git clone` into the SAME /etc/dokploy/applications/<app>/code dir; they corrupted each other and stalled at ~52M (a healthy clone grows to ~3.2G). The first swapped container was built from a corrupted clone (behaved stale). Fix: kill build procs BY PID (NOT `pkill -f "...clone string..."` — it matches and kills your own ssh session), wipe the code dir, trigger ONE webhook deploy, watch the dir grow cleanly to ~3.2G before the nixpacks build.

[DECISION 20260525-1343-pf7d] pumpfun "tokens of the day" — new-token signal + daily freeze.
- "New token" = first_seen >= UTC midnight today. first_seen persisted in new table `pumpfun_first_seen (asset_id, first_seen)` — recorded once per asset by the batch engine (insert-on-first-sighting, never updated), so it survives restarts. Chose a persisted DB timestamp over Jupiter/DexScreener launch timestamps because those payloads do not reliably carry a token creation time, and the in-memory discovery cache (client.rs) is lost on restart.
- 24h volume signal: wired Jupiter stats24h (buyVolume+sellVolume) through the pumpfun client into market_prices_latest.volume_24h (was hardcoded 0.0). Ranking = top 10 NEW tokens by volume_24h DESC, asset_id ASC tiebreak.
- Daily freeze: new table `pumpfun_daily_tokens (utc_date, rank, asset_id)`. Computed once per UTC day, reused for all 144 rounds; recomputed only when the UTC date rolls. Restart mid-day reuses the same set (read from DB). Fewer than 10 new tokens → fill to 10 with next-best recent (last 24h) high-volume tokens not already chosen, so the batch always has exactly 10 markets. Deterministic: pure function of the DB snapshot + a frozen daily set.
- One source of truth: UI reads the SAME batch's markets (via batchConfig.markets order) as "Tokens of the day", through a `displayFollowsBatch` source flag — no separate UI ranking. Degrades gracefully (falls back to popularity ranking if the batch is empty / backend half not yet deployed).

[DEPLOY 20260525-1210-h7k3] Hidden-source not-found (commit f007bc53a) DEPLOYED + verified.
- Added isHiddenSource->notFound() atop the source route; rewrote app/not-found.tsx to suggest live markets (validated against registry: Polymarket, Hacker News, binance, etc. + "Browse all markets" -> /sources).
- VERIFIED: /source/coingecko now renders the helpful not-found page ("This market isn't here" + "Markets that are open" + suggestions + "Browse all markets"), publicly and via container. binance_spot/hackernews still render their real pages. coingecko absent from listing (count 0).
- KNOWN NUANCE: HTTP status is 200, not 404 (soft-404). This route streams (export const revalidate=60), so Next commits the 200 before notFound() resolves — it renders the not-found UI but can't change the status. A nonexistent source is also 200. The user-facing page is correct (helpful suggestions). A true 404 STATUS would need a route rendering-mode change (e.g. force-dynamic or restructuring the gate before stream start) with a perf tradeoff — deferred unless SEO/crawler hard-404 is required.

[VERIFIED 20260525-1210-h7k3] Hidden-source not-found confirmed via <title> (raw-HTML grep is useless — the not-found boundary markup is embedded in EVERY page's RSC payload, so "Markets that are open" appears everywhere; use the server <title> to tell which branch rendered). coingecko -> "Source Not Found | General Market" (not-found, helpful suggestions). binance_spot -> "Binance Spot | Vision", hackernews/polymarket -> "<name> | Vision" (real pages, NO regression). Garbage source -> "Source Not Found". Soft-404 (HTTP 200) stands due to streaming; page CONTENT + TITLE correct.

[DIAGNOSIS 20260525-1740-pf01] /source/pumpfun "Markets indexing. Check back tomorrow." after ~2 min — TWO independent bugs found from live prod data.
- BUG 1 (frontend, FIXED here): sources-display.json pumpfun prefixes were ["pumpfun_"] but real assetIds are pf_*. The client snapshot route /api/vision/snapshot?source=pumpfun filters snapshots by startsWith(prefix) (route.ts:124) -> strips all 220 pf_ rows -> returns 200 with prices:[] -> react-query replaces good data -> curatedMarkets empties -> showEmpty. SSR prefetch (prefetch.ts:79) applies NO prefix filter, so cold paint hydrates ~220 rows; the 60s client refetch then wipes it = show-then-vanish. Changed prefixes -> ["pf_"]. Verified: pf_ filter keeps 220/220 vs pumpfun_ keeps 0/220.
- BUG 2 (data-node, NOT fixed — needs VPS deploy decision): the 10 frozen daily-token batch ids have ZERO snapshot rows and ZERO history points (history-bulk returned {} for all 10; live tokens return 89-106 pts). Cause: pumpfun client tracks a rolling set capped at 600 by MARKET CAP (client.rs:83), but get_pumpfun_daily_tokens (batch_engine.rs:967) freezes top-10 NEW by 24H VOLUME for the whole UTC day. Volume-picked tokens fall out of the mcap-600 tracked set within hours -> unpriced -> absent from snapshot/history. Today is worse: cold-start seed put all ~2900 tokens in pumpfun_first_seen, so today's "new" set was picked from dead historical tokens. Pumpfun client has no DB/pool access and no knowledge of the frozen set (fetch_assets/fetch_prices use Jupiter+in-mem cache only), so the two pipelines can't see each other.
- Even with BUG 1 fixed the page stays empty for displayFollowsBatch because pinnedAssetIds = batch ids (marketIds.length>0 always wins over popularity fallback), and those ids resolve to nothing in the snapshot.

[RESOLVED 20260525-1740-pf01] /source/pumpfun fixed end-to-end + VERIFIED live.
- Frontend: sources-display.json prefix pumpfun_ -> pf_ (commit 4ba39d330). Dokploy redeployed; /api/vision/snapshot?source=pumpfun now returns 230 rows (was 0).
- Data-node (VPS 1, surgical): client.rs <- origin/main (full pin code) + serve.rs one-line PumpfunMarketSource::new(pool_c.clone()) hand-patched to PRESERVE the local is_human CuratedSubsource edit (did NOT git-checkout serve.rs). Built --bin data-node as max (3m35s, warnings only — the sfdata-node bin is pre-existing-broken on this 121-behind tree, irrelevant to prod). Restarted data-node-shadow.service. Backup at VPS:/tmp/pf-deploy-backup.
- The pin RESURRECTED today's frozen set: all 10 daily tokens went is_active=true + repriced (fetched_at=18:35:51, right after restart) and now appear in the snapshot (inSnapshotNow 10/10, renderableInBrowser 10/10). They were never dead — they had aged out of the MAX_TRACKED_TOKENS=600 mcap cap and been deactivated by sync_assets.
- DECISION REVERSED: force-re-pick was NOT executed. The user picked it believing the tokens were dead; the DB proved them alive and the pin fixed them. Re-picking would rewrite pumpfun_daily_tokens and desync the displayed set from the live on-chain round #125636 (already committed to these 10). Side benefit: the round can now resolve (priced) instead of cancelling.

[DONE 20260525-1210-h7k3] Apple-style 404 + shared search-bar icons (commit 5c38fc77c) DEPLOYED + visually verified.
- Extracted the search bar's round-logo into shared `frontend/components/ui/SourceLogo.tsx` (28px disc on #f5f5f7, next/image + letter fallback); search bar (SourceSearch) and not-found both use it now — same component, not a lookalike.
- not-found.tsx restyled to docs/apple-style-table.md: SF Pro stack, small grey "404" eyebrow over a 48px semibold "This market isn't here." (not the old big-black slab), #1d1d1f/#6e6e73 text, 17px body -0.022em, "MARKETS THAT ARE OPEN" round-logo chips, blue #0071E3 "Browse all markets" pill + "Back to General Market ›" blue link.
- VISUAL VERIFIED via Playwright screenshot of /source/z: Apple layout live, circular icons render (some next/image lazy-load mid-screenshot), 0 console errors, no error box.
- Search bar: SourceLogo swap clean, no JS errors. NOTE (pre-existing, NOT this work): /sources monitoring page logs 6 image 404s for /source-imgs/icons/<id>.png (crypto, defillama-derivatives, binance_*, tubes) — the known manual-icon-path bug (see vision-profile-positions-gotchas), separate from the registry-logo path SourceLogo uses.
- Dokploy clone-storm recurred twice from rapid pushes; cleared each time (kill build procs by PID, wipe /code, ONE webhook, watch grow to 3.2G) per dokploy-concurrent-clone-pileup memory.

## 20260526-1730-pq4k — Perps graveyard timeline research
[DECISION] Research compiled for an animated timeline on the history of perps protocols and their downfall via the cost of liquidity. Four parallel research agents (first-gen/orderbook+vAMM, Solana/alt-L1, pool-model, macro thesis+Hyperliquid). Synthesis at marketing/perps-graveyard-timeline-research.md. Thesis: ~$400M disclosed funding across ~17 protocols bought launches, not durable liquidity; Hyperliquid won 70-80% with zero VC by OWNING liquidity (HLP) instead of renting it with token emissions. Four failure modes identified (vAMM/insurance-fund drain, mercenary token emissions, empty orderbook, pool-as-counterparty). Several figures flagged TO-VERIFY before any number goes on screen (RabbitX/Gains TVL, aggregate VC total, exploit sizes).

## 20260526-1439-bf7c — BatchFlowReel center card → AntiCheatEdit blue field
[DECISION] Re-skinned the BatchFlowReel center card to match the AntiCheatEdit schematic card at 3:24 ("Two views of one book"). Only the card's OWN background changed — swapped the white ground + accelerating points-plexus inside the floating window for the real AntiCheatEdit `BlueField` (deep-blue radial gradient + drifting white line-grid + sweeping scan-line cross), reused directly from anticheat-edit/props so the background is byte-identical. The blurred-blue broll BEHIND the card is left untouched (per explicit user constraint). Card frame ink flipped to a subtle light edge over #020E2B. To match the card's style, on-field text moved to white: added C.ink/inkSoft/inkDim tokens; hero numbers (multiply/unlock/payout) dropped the gradient-clip for solid white + navy shadow (matches StatSolo); BeatTitle sub, row-name labels (LineRow/TraderChip), and the mono caption labels all lightened. Text on white-glass surfaces (product UI, packets, payout chips) kept its dark ink — it reads fine on blue. Fonts were already shared (common/fonts). Removed the now-dead PointsNetwork + PILL_GRADIENT import. Verified: tsc clean, eslint clean, 8 beats rendered legible.

## 20260526-1439-bf7c — CORRECTION: wrong reference card
[FAILED] First pass replicated the DARK blue-field schematic card ("Two views of one book", ~3:24) — wrong. Reverted via `git checkout` (the 5 batch-flow files were clean at d27805914, so revert was lossless).
[DECISION] Correct reference is the WHITE flow-visibility venue-bar card at 3:37 (`LogoBarChart` / `topicChart("order-flow-vis")` in anticheat-edit/overlays). Its signature is the GROUND, not dark ink: light #F0F2F4 paper + a fine Base-blue dot lattice (`DotGrid` from anticheat/, 14px spacing, r1.6, α0.22, accent #0052FF, animated wave + drifting bands) + `DotGridVignette`. Swapped the BatchFlowReel center card's accelerating points-plexus for the real `DotGrid` + #F0F2F4 ground (reused the component for an identical texture); kept the card's existing dark-on-light + blue/violet content (already close to the reference). Broll behind the card untouched. Removed the dead PointsNetwork. DotGrid runs on its own 30fps clock at 60fps but `speed=0.9` stays calm across the 47s reel (verified frame 2600). tsc + eslint clean.

## 20260526-1905-tw3a — Three Walls: three micro-videos
[DECISION] Spec written at docs/superpowers/specs/2026-05-26-three-walls-microvideos-design.md. Three standalone Remotion micro-videos in the BatchFlowReel style (navy backlight + floating panel + #F0F2F4 paper + DotGrid lattice, glass cards, blue→violet pills, EASE curves), grouped in a Root <Folder name="ThreeWalls">. Each is ONE continuous mechanical animation stating one reason the orderbook can't reach 1B assets — pure motion, no title cards, no hero numerals. Build order: FiniteLiquidity (1200f/20s) → TechnicalOverload (300f/5s, true 3D R3F cube 100k→1B + explosion) → AsphyxiationByWinners (540f/9s). User decisions locked: natural lengths (not 4-6s); only two on-screen captions ("if giving choice to traders", "2 years later") + happy/unhappy/mmm-yellow face states, otherwise wordless; #1 is a real 3D box. #3 ends on the govt-protection-box twist (MMs stop switching only when liquidity is fenced by force). #2 now has a resolution beat (General layer forces breadth → traders happy, MM neutral) — supersedes the earlier "problem only" framing. R3F gotcha noted: NO drei <Environment> (blank-render bug); 100k = InstancedMesh, 1B = textured shell (can't instance 1e9); verify with --concurrency=1.

## 20260527-0210-bf7k — BatchFlowReel throughput climax rebuilt
[DECISION] Replaced the static "multiply" counter beat (~32s) with a built four-station throughput story the camera flies right across, then pulls back: (1) a person fires one trade, (2) it fans into 10,000 lines, (3) a day-timeline stamps it ×100 → 1,000,000, (4) dezoom reveals ten *sources* (Pump.fun, Polymarket, Twitch, Binance, Hacker News, DefiLlama, Deutsche Bahn, Cloudflare, 4chan, Steam) each = 1,000,000 → grand total 10,000,000.
[DECISION] Ends on 10,000,000 — dropped the old "$1B liquidity unlocked / more than Hyperliquid" beat (user call). The ×10 factor is ten different data sources, NOT the ten markets you picked.
[DECISION] New presentational components live in batch-flow/throughput.tsx (PersonIcon, TradeFan, DayTimeline, SourceStack); data in data.ts (THROUGHPUT_SOURCES + factors). camera() now returns {x,y,scale} to pan+dezoom for the stack reveal.
[FAILED-minor] TradeFan count sits slightly left/low of the burst's visual centre — legible, flagged for a later nudge.
[CLEANUP] Removed orphan beats-scale.tsx (pre-rewrite twin, imported by nothing).

## 20260527-0345-bfm2 — BatchFlowReel: morph open, person-traders, graph settle, continuous climax
[DECISION] User review of the current reel. Four changes agreed (build in phases): (1) 1s→3s the "one question" card MORPHS into the Fartcoin dashboard card (it is literally a blown-up Fartcoin card) — kill the crossfade; dashboard assembles around it. (2) 13s traders: circle-with-number → filled person glyph (reuse throughput PersonIcon), re-laid as a clean row above the POOL (was a cramped ring overlapping the box). (3) 15-30s settle: rebuild the ten lines as a LINKED graph (each line links the others), one oracle sweep settles the whole graph, losers→winners on the edges; drop both floating concept cards (GlassLine, GlassFlow). (4) climax: the throughput station-hop (MX1 person / MX2 fan / MX3 day+stack — "3 separate scenes") fused into ONE continuous growing-graph dezoom: 1 node → linked mesh of 10,000 (one tx) → tiled ×100 across a day (1,000,000) → ten source-meshes (10,000,000). Keeps the 10M / ten-source ending from 20260527-0210-bf7k (no $1B beat). Motif: it's all one self-similar graph that grows; the eye only falls back, never cuts.
[PHASE 1] morph + person-traders. Files: ui.tsx (ProductUI staggered/morph entrance), BatchFlowReel.tsx (morph + trader row), flow.tsx (drop TraderChip use).

[PHASE 2 DONE] Settle-as-graph shipped (commit bdebfc476): pool → ten linked donut-nodes on spokes + hub; one radar oracle sweep settles each (loser arc drains into winner); hub tallies YOU COLLECT $14.17 net +$4.17; dropped GlassLine+GlassFlow cards and the DX detour; new graph.tsx. tsc/eslint clean, verified f900/1130/1392.
[PHASE 3] Climax: user picked NESTED ZOOM-OUT. One node fans to a 10,000-node graph (one transaction) → camera pulls back, that whole graph is one cell of ~100 (a day, 1,000,000) → pulls back again, one of ten source-fields (10,000,000). Camera ONLY pulls back (vertical pull-back through a 3-level tree, LOD by zoom). Replaces the MX1/MX2/MX3 station-hop (the "3 separate scenes"). New ThroughputClimax component; camera simplified to a single BX→CX pan then internal self-zoom.

## 20260530-1720-fee1 — Pitch-deck fee corrected to Binance-perp level
[DECISION] The decks/dataroom claimed "~50 bps of settled volume" with $10M/mo × 50bps = $50k MRR math. Per Max, that fee is wrong — General should take "the same as a Binance perp" (~0.05% of volume). Corrected in marketing/general-deck-v2.html (added a biz-math fee line — v2 had none), general-deck.html:669, yc-pitch.html:1043, and frontend/content/room/business-model.mdx (fee → 0.05%, dropped the volume→revenue arithmetic per "drop the explicit math").
[DECISION] Public docs NOT changed. They state "0.05% fee on profits" which is the LIVE contract truth (PROTOCOL_FEE_BPS=5, on profit not volume). Max's Binance-style fee (on volume) is a FUTURE on-chain change ("changed on next deployment"). Changing docs now would misdescribe the deployed contract.
[PENDING] On next on-chain deployment: change the fee model to Binance-perp-style (~0.05% on volume, not profit), bump PROTOCOL_FEE_BPS / the fee base in the contract, then sync ALL public docs (introduction, risks, getting-started, fees.mdx, glossary, contracts.mdx PROTOCOL_FEE_BPS row, bots/overview, architecture/block-lifecycle, docs/vision/*) from "0.05% on profits" to the new model.
[FLAG] "$50k MRR" milestone still stated (no formula) in the-ask.mdx:39,45, index.mdx:31, general-deck.html:748 — now orphaned from any supporting math. Left intact pending Max's call (keep as aspirational target, or re-anchor).

## 20260605-1845-xtrg — X niche research engine close-out
[DECISION] Niche research completed for 6 cells (trenches EN/CN, perps EN/CN/JP/KR) using twitterapi.io with project budget enforcement. Final authoritative spend: $1.2905 of $15 cap. Deliverables: 6 niche maps + cross-cell format playbook in marketing/niche-research/.
[DECISION] Query validation repaired non-EN banks against live X results instead of trusting drafted idiom. Dead/weak terms were lowered or swapped (notably KR trading-method terms and CN bundle-detection wording). Current banks all produced live validation hits.
[DECISION] Followings endpoint shape differed from assumed schema: returned top-level `followings` and snake_case profile counts. Patched twapi.py so graph pass records `followed_by` edges and preserves follower counts; added regression tests.
[FAILED] Many non-EN followings and some deep-pulls returned HTTP 408 while still burning small credits. Stopped retry loops after one pass; used successful graph edges only. This kept spend low and avoided paying into endpoint instability.
[DECISION] Format playbook centers data drops, numbered lists, tutorials, and daily recaps. Pure meme/question formats can distribute but do not compound trust; unclassified viral bait was treated as an anti-pattern, not a core format.

## 20260610-1820-docs — Vision/Index docs rebuilt from scratch on the CRX handbook layout
[DECISION] Full docs rebuild shipped (417ec4e55 content, ea038c83c platform, da95c8cc0 verify round). Five sections (get-started/vision/bots/index/developers), 46 pages, plain .md + gm* fences replacing the MDX system; CRX handbook system ported and rebranded (Apple-blue accent, Geist Sans + JetBrains Mono); 7 static SVG diagrams; GmTry live API explorer; 92 redirects from /docs/blocks; link checker (553 links + fence schemas) gated into prebuild (GM_SKIP_LINKS=1 escape). Pipeline: spec/method/fact-sheets in docs/docs-rebuild/ → 12 parallel writers → 6 section verifiers + harmonizer. Product naming: Vision is the product, a "block" = one prediction batch; "Blocks" as a product name retired.
[DECISION] bot.py was broken as shipped and got fixed in-pipeline: 5-param joinBatchDirect ABI (selector mismatch → revert), dead default address 0x821D7c (zero bytecode), faucet blind to the waitlist gate. Now: 4-param ABI, live 0x36a28967, /api/bot/faucet fallback. config.toml/.env.example committed clean; root AGENTS.md Vision section facts corrected (file stays untracked). example-vision-bot/ and examples/vision-bot-python/ remain stale.
[FLAG] Product bugs found by verifiers, not fixed (UI code untouched): (1) human-trading join silently bets DOWN on every unpicked market in the full config (~5,091 on twitch) — the "$0.50 × 10 markets" split shown is a display fiction (SourceDetailHumanTrading.tsx buildBets:799); (2) VaultActions.tsx:334,979 divides performanceFeeRate (bps) by 1e16 → vault fees render ~0%; (3) "previous batch settles the moment this one closes" i18n string + RoundTimeline math encode the retired N+2 settle order (live: settle ~one tick after creation); (4) FirstTradeGuide.tsx teaches retired proportional-by-score pooling; (5) deployment.json VisionVault row has zero bytecode — live clone impl is 0x761add2bec841a8cf623ee2437cd8cd45ec0a1b9; (6) /vision/balance handler exists but is never routed (also still advertised in learn/build-prediction-market-bot.mdx + public/vision-bot/AGENTS.md + llms-full.txt); (7) nav-series rejects "1m" while its error text offers it; (8) Discord invite split: app uses xsfgzwR6, linktree QbasycShP.

## 20260611-docs-titles — One register system for every docs name surface
[DECISION] All 47 handbook pages swept for titles/navTitles/descriptions/groups: explanation pages get plain-statement titles ("How payouts work", not "How do I win?"), how-tos get imperatives, reference pages get exact noun phrases; slogans retired ("Two chains, one balance" → "Settlement and the bridge"); cute description phrases replaced with facts and numbers ("the truth about the balance endpoint" → "why the balance endpoint always 404s"; fees desc now carries 0.05%/0.1 USDC). Groups renamed literal: Play→Gameplay, Money→Your money, Standing→Leaderboard & risks, Build→Build a bot, Start→Foundations. In-page H2s stay question-form (house pattern) — only vague ones renamed (The numbers→The fee schedule), with gmsummary left-sides updated to match (anchor coupling). Cross-refs are hardcoded strings in gmcards/gmseealso JSON and Next: links — renames swept href-keyed, not blind global replace ("Strategies", "Vaults" are too generic to replace by text alone). Link checker green (570 links).

## 20260612-crx-taker-only — CRX app goes taker-only; accounts arrive funded by a keeper
[DECISION] Provisioning architecture per Max's calls: $100k of EACH of the 4 tokens (USDC/sUSDS/EURC/USDT, $400k face), a queue BETWEEN frontend and keeper (relayer POST /provision, file-backed like profiles.rs), and mint-STRAIGHT-to-balance on-chain — one DAO-gated engine2 call `provisionAccount(party, tokens, amounts)` that mints each mock to the core and credits the general balance (backing stays real, withdraw works), grants TAKER idempotently. No depositFor, no mint-approve-deposit dance: 2 txs per account (gas top-up + provisionAccount).
[DECISION] Engine2-only redeploy (no core/engine1 swap) was safe because `git log contracts/src/CRXStorage.sol` showed no layout change since the deployed build (withdrawSigned e309f16) — the core's catch-all fallback chains the new selector core→engine1→engine2 without a core stub on-chain. Checked before deploying; the UpgradeWithdrawSigned full-recipe is only needed when the layout moved.
[DECISION] Maker removal scope: ~40 files deleted from the ui app (MakerDesk/RfqInbox/QuoteTicket/RoleSwitcher/onboarding wizard/maker handbook), but the relayer's maker machinery STAYS — the bot fleet answering taker RFQs are makers. Frontend `Role` type kept two-sided ("maker" survives as the on-chain position SIDE in store/explorer); only the workspace hardcodes taker. useRole() returns a constant; ~23 read-only consumers untouched.
[FAILED] `cargo test provision` (no --lib) ran 0 tests silently — the filter only hit doc-tests. Use `cargo test --lib provision`. Also: same-second created_at collisions made HashMap order decide "oldest first" — tie-break by address.
[DECISION] Concurrent-session git discipline both repos: crx-mono shared files (app.rs/handlers.rs) carried the email-auth session's in-flight wiring — committed together after cargo check green (their own commits had swept my lib.rs the same way). ui repo: temp-index (GIT_INDEX_FILE read-tree HEAD + add my paths + commit-tree + guarded update-ref) kept their staged LoginCard/EmailLoginCard work out of my commit; when origin moved mid-push, recreated the commit on the new base instead of pull --rebase (which would have checked out over their uncommitted tree).
[FLAG] /api/faucet (100k USDC + gas, server-keyed) and /developers/get-funded stay live for API/bot devs — redundant with provisioning for app users. Retire or repoint when the developer docs get their own sweep.
[FLAG] Old maker profiles in the relayer's profiles.json still say role "maker" — nothing reads role for routing anymore (LoginGate routes everyone to the taker desk), but the data lingers.

## 20260612-crx-email-2fa — Email login + mandatory Google-Authenticator 2FA shipped on app.crxfx.com
[DECISION] The relayer is the single account authority (not Next, not Clerk): file-backed accounts.json beside profiles.json, argon2id passwords, totp-rs (SHA1/6/30s ±1 step + replay guard), 8 keccak-hashed single-use recovery codes, AES-256-GCM TOTP secrets at rest (ACCOUNT_ENC_KEY, falls back to keccak(JWT_SECRET) with a warn). Session JWT keeps sub = linked wallet address so every existing protected route and bot stayed untouched; new scope claim (signup/mfa/session) with missing-scope→session for legacy tokens. Email session 8h; wallet/bot JWT stays 1h.
[DECISION] Anti-enumeration: signup on a taken email returns an identical 200 + decoy signup token whose codes can never verify, and the real owner gets a warning mail; login errors are a uniform "invalid credentials". Verified live: signup-scoped token on GET /account → 401 "token scope is not a session".
[DECISION] Mail rides Resend's HTTP API from the relayer (reqwest), sender CRX <no-reply@notify.crxfx.com> — notify.crxfx.com is the only Resend-verified domain (the bare-crxfx.com default in the plan would have bounced). RESEND_API_KEY copied server-side from the crxfx-frontend container into /opt/crx/.env; key absent = codes logged (dev mode).
[FAILED] Spec reviewer caught password_login clearing only the email lockout counter on success, never the IP counter — one-line fix + red-green regression test (892fb5e). Final whole-feature review caught what per-task reviews couldn't: WalletGuard still gated every (app) route on wagmi, so email-only sessions bounced in a redirect loop and the recovery→/settings?section=security path was unreachable; AND clearCurrentSession() had zero callers (no sign-out). Fixed in d7b276e: guard admits a live session, AccountChip sign-out clears it + hard-navigates.
[DECISION] Best-practice sweep (2663d7f relayer / 33ef87b ui, redeployed same day): the timing side-channel is FIXED — unknown emails now verify against a OnceLock dummy argon2id hash, equal cost both paths (live probe: ~0.4s on unknown email); plus PASSWORD_MAX_LEN=128 (argon2 DoS ceiling), per-email signup mail throttle (60s, checked BEFORE the existence branch so the 429 is enumeration-uniform; live probe 200→429), GCM decrypt floor 13→28 bytes, expired-session pointer sweep in loadCurrentSession, getAccount on the shared error idiom. Full lifecycle E2E (25 assertions: signup→code→QR→confirm→wallet-link→login→replay→pw-change→recovery→re-enroll→lockout) passes against the patched binary locally; relayer unit tests 83 green.
[FLAG] Mid-flight product shift interplay: the taker-only purge + funded-on-arrival provisioning landed DURING this build. The signup wizard still requires the user to bring + link their own wallet (challenge signature); with keeper-provisioned fresh addresses, a future simplification is to provision a wallet AT signup instead of linking one. Needs Max's call.
[FLAG] e2e probe residue: three unverified decoy/pending accounts (crx-e2e-probe-{1,2,3}@example.com) in the relayer's accounts.json — harmless (unverified = replaceable, can't log in), no cleanup endpoint exists.

## 20260612-crx-wallet-link-relocation — Wallet linking leaves the signup wizard, moves into Settings
[DECISION] Per Max (screenshot of step 5): signup is now FOUR steps ending at recovery codes; the account enters the app wallet-less. Session model: JWT sub = wallet address when linked, account UUID when not. Wallet-less sessions reach exactly five AccountUser-gated routes (/account, /account/link-wallet, /account/password, /account/recovery-codes, /account/totp/enroll); SessionUser/CallerAddress still require an address subject, so the trade path rejects them structurally — verified by adversarial review listing every Auth::verify consumer. /auth/totp/confirm (signup scope) mints the first wallet-less session beside the recovery codes; /auth/link-wallet (signup-scoped) DELETED; /account/link-wallet (session-scoped) binds a challenge-verified wallet and returns an upgraded address-subject session the client swaps in (and sweeps the stale "unlinked" localStorage record). Settings → Security owns the link flow. Lifecycle E2E extended to 28 assertions, green; relayer 84 tests.
[FAILED] Deploy of HEAD failed (E0560): the forwards session's `entrySpot: quote.lockedRate` hunk rode into our handlers.rs commit while its Terms struct field stayed UNCOMMITTED in crx-interfaces — origin compiled locally (their tree edits present) but not from a clean export. Lesson: a foreign hunk that rides along can carry a hidden dependency on files that did NOT ride along; before deploying a shared-tree repo, clean-export-check (`git archive HEAD | tar -x && cargo check`) — exactly what deploy-stack.sh does, do it BEFORE burning the 10-min build. Fixed by dropping the orphan line (dbfe42f); the line returns with their interface+contract change in one commit. NOTE: their Terms field changes the EIP-712 typehash — must ship with the contract redeploy or live binds break.
[FAILED] Two deploy-stack.sh runs raced in /opt/crx-build on VPS2 (concurrent session re-rsynced + relaunched mid-build); rm -rf + --delete killed my build's log out from under the poller. Convergent ending: their roll shipped an image containing our commit (verified live by the 401 on /account/link-wallet, not by any deploy log). Lesson: verify deploys by probing the live wire, never by the deploy script's own output; one build dir, no lock — coordinate or re-run when quiet.
[FLAG] Re-link while already linked is allowed at the relayer (replaces own wallet; orphans the old address's desk state) but unreachable through the UI (Settings only offers the flow when wallet is null). Tighten server-side if it ever matters.

## 20260612-crx-connect-nudge — Every connect-wallet dead end routes to Settings → Signer; the uncommitted-tree root cause
[DECISION] One affordance (ConnectNudge block + inline) behind all 8 "connect a wallet" prompts; arrival fires a one-shot teal beacon on the Signer card (crx-signer-beacon, the lock-ignite gesture in --accent-ring). The nudge is armed in sessionStorage, NOT inferred from #signer — hash-triggered would replay the bloom on every manual rail click; sessionStorage fires exactly once per routed arrival. The Signer panel also gains the real ConnectPicker when no wallet is connected, so the door every nudge leads to actually opens (it previously showed only a disabled "Switch signer").
[DECISION] Root cause of "I asked 20 times and the note isn't made": the work EXISTED — AccountCluster's email-session ⇅ pill, the 284-line RecentActivityPanel rework, DepositWithdraw anatomy — sitting UNCOMMITTED in ~/Downloads/ui while prod rebuilt 25× the same day from committed-only code. When a user reports a repeatedly-requested feature missing, check `git status` of the working tree BEFORE re-implementing: the fix may be finished and unpushed. Shipped as e758e68 + ae9a2e9.
[DECISION] Prod-state verification recipe for the email-session UX: WalletGuard trusts localStorage only, so planting crx-session-current=unlinked + crx-session:unlinked={address:null,token:…,expiresAt:future} in the browser renders the real desk on app.crxfx.com without credentials — used to reproduce the missing pill on live and to verify the fix after rotation.
[FLAG] Stale-index noise: ~30 files showed MM (staged≠HEAD) whose worktree content equalled HEAD — leftover index state from a prior session's commit flow. `git add -A` resolved them to no-ops; don't read MM as "two versions in flight" without diffing.
[FLAG] HedgeDesk CTA when wallet connected but no maker reachable now says "Quotes offline" (was a lying disabled "Connect wallet").

## 20260613-vps1-disk — VPS1 at 86% disk / swap-full: a stalled price-storage migration storing everything twice
[DECISION] Root cause was not logs or junk. `index_prices` was 372GB of the 414GB used; `market_prices` (live plain table, 164GB incl. an 82GB covering index) AND `market_prices_v2` (TimescaleDB hypertable, 111GB) held the SAME price stream — a dual-write migration (`docs/plans/2026-05-16-vision-storage-redesign.md`, `USE_NEW_STORAGE=1`) that shipped phase 1 and stalled. Reads never cut over (v2 parent idx_scan=0); v2 was written, never read.
[DECISION] The migration's whole payoff is impossible here: `SHOW timescaledb.license` = **apache** → no native compression, no policies. The plan's projected 10–15× shrink / 600× capacity win cannot run on this build. retention.rs even says so ("Apache-edition-safe; no compression"). So v2 was stored raw — same size as the table it mirrors. Net effect: one dataset stored twice, neither compacted, the old table bloating via DELETE-based retention faster than autovacuum reclaims on a load-9 box.
[DECISION] Freed 111GB by `DROP TABLE market_prices_v2, prices_v2` (index_prices 372→261GB, disk 86%→63%). Safe because grep proved nothing reads them, parent idx_scan=0, and the plan lists DROP as the sanctioned phase-2 rollback. DID NOT touch `vision_settlements` (37GB) — the SETTLEMENT migration completed (old `*_archive` tables already dropped in a partial phase-5; api.rs reads vision_settlements at :798/:3263); it is the only copy.
[FAILED] First instinct (and the approved option's wording) was `USE_NEW_STORAGE=0` globally. WRONG: the same flag, in the ORACLE crate (lifecycle.rs), gates the `vision_settlements` write that api.rs serves — flipping it everywhere silently stops settlement recording. The flag is read per-process: the 3 consensus oracles are Docker containers with their own `USE_NEW_STORAGE=1` (independent of host env); in the data-node crate the flag gates ONLY the price mirror (write_channel.rs, verified by grep). So it was scoped to the data-node service alone.
[FAILED] systemd drop-in `Environment=USE_NEW_STORAGE=0` did NOT win — process still showed 1. Rule: **`EnvironmentFile=` overrides `Environment=` regardless of order.** `system.env` (line 167) set it to 1. Fix: drop-in `EnvironmentFile=<dir>/storage.env` (drop-in files append last; among env-files the later wins) at `/etc/systemd/system/data-node-shadow.service.d/`. Did NOT edit shared `system.env` — the oracle compose may use it as env_file and a container restart would then pick up 0 and break settlements.
[DECISION] Did NOT rebuild the data-node binary to remove the dead mirror in code. `/home/max/index` on VPS1 is 380 commits behind origin with heavy uncommitted WIP from other sessions (serve.rs, traits.rs, batch_engine.rs…); the running binary builds from THAT tree, so a `cargo build` would ship someone else's unfinished work. The runtime drop-in (no rebuild, re-execs the existing binary) stops the dual-write without that risk. `data-node-shadow.service` is price ingestion only — NOT consensus (those are the testnet-oracle-1/2/3 containers) — so its restart is a few-seconds ingestion gap, auto-resumed.
[FLAG] retention.rs still lists `("market_prices_v2",30)` and `("prices_v2",90)` in HYPERTABLE_DROP_TARGETS → one harmless drop_chunks "does not exist" warn per hour (not gated by the flag). Remove those two entries + the price-mirror block in write_channel.rs when the divergent tree is reconciled and a clean data-node build ships.
[FLAG] Two real follow-ups remain. (1) The 82GB covering index on `market_prices` is largely DELETE-bloat; now that 180GB is free, `pg_repack market_prices` can reclaim ~50–80GB (online, brief locks — schedule off-peak). (2) Strategic: the price table is still DELETE-retention (bloat + XID-wraparound prone) and Apache edition blocks the compression the plan was built on. Permanent fix = switch TimescaleDB Apache→Community (free license) then finish the cutover, OR redesign retention. Max's call.
[FLAG] Verified post-change: price ingestion healthy (lag ~9s, ~11k rows/min), settlements flowing (last 23s ago, 383/hr), v2 spam zero. RAM available 0.8→3.2GB, swap 100%→74%. 1-min load spiked to 14 right after (two restarts → full source re-fetch burst + my queries); expected to settle below the 9.4 baseline since price writes are now halved.
