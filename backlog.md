...
[DECISION] New explainer `ParimutuelExplainer` at `video/src/compositions/parimutuel/` (theme.ts, primitives.tsx, beats.tsx, ParimutuelExplainer.tsx), registered at the root of Root.tsx. Teaches the batch anti-cheat as a chain of state transformations over ONE continuous BlueField (the field never cuts; only the content on top transforms): one question → oracle threshold (+2%) → A+B=C (YES+NO stakes merge into the pot) → B→A (losers fund winners) → ×10,000 sealed pools → cost-to-cheat ($5k × 10k = $50M+, "not worth it") → three guarantees (sealed / settled by the oracle / split among winners) → landing aphorism "Cheating costs more than it pays." 1281f / 42.7s @ 30fps, landscape 1920×1080. Reuses anticheat-edit props (tokens palette, BlueField, RevealChars, IdleZoom). Silent + music bed: 03-eternal-odyssey (trimBefore 89.211s, its "hope rising" turn) at 0.34 + tech-hum atmos at 0.12.
